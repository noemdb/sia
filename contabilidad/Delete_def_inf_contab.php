<?php include ("../class/conect.php");  include ("../class/funciones.php");$cod_informe=$_GET["txtcod_informe"]; echo "ESPERE POR FAVOR ELIMINANDO....";  $fecha_hoy=asigna_fecha_hoy();
$conn=pg_connect("host=".$host." port=".$port." password=".$password." user=".$user." dbname=".$dbname."");if (pg_last_error($conn)) { ?> <script language="JavaScript"> muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');  </script>  <?php }
  ...
  if ($filas==0){?> <script language="JavaScript">  muestra('CODIGO DE NO EXISTE'); </script> <?php }
	 ...
	 else{  ?><script language="JavaScript"> muestra('ELIMINO EXITOSAMENTE'); </script>  <?php  $sfecha=formato_aaaammdd($fecha_hoy);
