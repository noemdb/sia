...
if($tipo_asiento=='ANU'||$tipo_asiento=='ANC'||$tipo_asiento=='CHQ'||$tipo_asiento=='NDB'||$tipo_asiento=='AND'){$error=1; ?> <script language="JavaScript"> muestra('TIPO DE ASIENTO NO VALIDO');</script><?php }
if ($error==0){ $conn=pg_connect("host=".$host." port=".$port." password=".$password." user=".$user." dbname=".$dbname."");
 ...
 if($error==0){$campo502="NNNNNNNNNNNNNNNNNNNN";  
   ...
   if ($filas==0){$error=1;?>  <script language="JavaScript">  muestra('TIPO DE ASIENTO NO EXISTE');  </script>  <?php }
      ...
      if ($error==0){ $sql="SELECT * FROM CON017 where codigo_mov='$codigo_mov' order by nro_linea";
         ...
         if($balance>0.001){$error=1; echo $t_debe.' '.$t_haber.' '.formato_monto($balance),"<br>"; ?> <script language="JavaScript"> muestra('MOVIMIENTOS CONTABLE NO CUADRA');</script><?php }
         If(($t_debe==0)or($t_haber==0)){$error=1; ?> <script language="JavaScript"> muestra('MONTO DE MOVIMIENTOS NO VALIDO');</script><?php }
      }	  
	  ...
	  if ($error==0){ $sql="SELECT * FROM CON017 where codigo_mov='$codigo_mov' and modulo='E' order by nro_linea";  $res=pg_query($sql); 
         while($registro=pg_fetch_array($res)){ 
			...
			if (verifica_disponibilidad($conn,$cod_presup,$fuente_financ,$fecha,$monto_c)==0){$error=0;}
               else{$error=1;?><script language="JavaScript">muestra('ERRRO EN EL CODIGO PRESUPUESTARIO:<?php  echo $registro["cod_presup"]; ?> FUENTE:<?php  echo $registro["fuente_financ"]; ?> ');</script><?php }
	  ...
	  if ($error==0){ $sql="SELECT * FROM CON010 where codigo_mov='$codigo_mov' order by debito_credito desc,cod_cuenta";
         ...
         if(($t_debe==0)or($t_haber==0)){$error=1; echo $t_debe.' '.$t_haber; ?> <script language="JavaScript"> muestra('MONTO DEL COMPROBANTE CONTABLE NO VALIDO');</script><?php }
         $balance=$t_mov-$t_debe;		 
		 if($balance>0.001){$error=1; echo $t_debe.' '.$t_mov.' '.formato_monto($balance),"<br>"; ?> <script language="JavaScript"> muestra('COMPROBANTE CONTABLE NO CUADRA CON TOTAL MOVIMIENTOS');</script><?php }
	  }
      ...
      if($error==0){ 
        if (($sfecha>$Fec_Fin_Ejer)or($sfecha<$Fec_Ini_Ejer)){$error=1;?><script language="JavaScript">muestra('FECHA DE MOVIMIENTO INVALIDA');</script><?php }
      } 
	  if ($error==0){ $sSQL="SELECT MULTIPLE_CON002('$codigo_mov','$referencia','$sfecha','$tipo_comp','$tipo_asiento','A','M','S','01','0','00000000','$ced_rif','','$usuario_sia','$minf_usuario','$descripcion')"; echo $sSQL;
	    $resultado=pg_exec($conn,$sSQL);  $error=pg_errormessage($conn);  $error=substr($error, 0, 80);
         if (!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php }
           ...
           if (!$resultado){?> <script language="JavaScript">  muestra('<?php  echo $error; ?>'); </script> <?php } else{?><script language="JavaScript">muestra('INCLUYO EXITOSAMENTE');</script><?php } }
      }
    } }
 } 
