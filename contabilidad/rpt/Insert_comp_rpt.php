...
if (pg_last_error($conn)){ ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
   else{ if (checkData($fecha)=='1'){$error=0; $sfecha=formato_aaaammdd($fecha);}else{$error=1; ?> <script language="JavaScript">muestra('FECHA NO ES VALIDA');</script><?php }
      if ($error==0){ $sSQL="Select referencia from con018 WHERE codigo_mov='$codigo_mov' and referencia='$referencia' and tipo_asiento='$tipo_asiento' and fecha='$sfecha'"; $resultado=pg_query($sSQL); $filas=pg_num_rows($resultado);
       if ($filas>0){$error=1; ?> <script language="JavaScript"> muestra('COMPROBANTE YA EXISTE EN LA LISTA');</script><?php } }
      if ($error==0){ $sSQL="SELECT INCLUYE_CON018('$codigo_mov','$referencia','$sfecha','00000','$tipo_asiento','N','N','','',0,0,'')";
	  $resultado=pg_exec($conn,$sSQL); echo $sSQL; $error=pg_errormessage($conn);  $error="ERROR GRABANDO: ".substr($error, 0, 61);if (!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php } else{ $error=0;}
      }
} pg_close($conn); 
