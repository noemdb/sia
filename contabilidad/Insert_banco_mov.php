<?php include ("../class/conect.php");  include ("../class/funciones.php"); $nro_linea=1; $error=0;
...
if (pg_last_error($conn)){ ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
 ...
 if($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DE BANCO NO EXISTE');</script><?php }
 else{$registro=pg_fetch_array($resultado,0);  $nombre_banco=$registro["nombre_banco"]; $codigo_cuenta=$registro["cod_contable"]; $activoA=$registro["activa"]; if($activoA=="N"){$error=1; ?> <script language="JavaScript"> muestra('BANCO NO ESTA ACTIVO');</script><?php } } 
      ...
      if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DE CUENTA NO EXISTE');</script><?php }
       else{ $registro=pg_fetch_array($resultado); $nombre_cuenta=$registro["nombre_cuenta"]; if($registro["cargable"]=="N"){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DE CUENTA NO ES CARGABLE');</script><?php } } }
      if($error==0){$sSQL="SELECT tipo FROM BAN003 WHERE tipo_movimiento='$tipo_mov'";  $resultado=pg_exec($conn,$sSQL); $filas=pg_numrows($resultado); if($filas==0){$error=1;?><script language="JavaScript">muestra('TIPO DE MOVIMIENTO NO EXISTE');</script><?php } else{$registro=pg_fetch_array($resultado,0); $debito_credito=$registro["tipo"];}}
	 ...
	 if ($error==0){  $sSQL="Select * from CON017 WHERE codigo_mov='$codigo_mov' and cod_cuenta='$cod_banco' and debito_credito='$debito_credito' and referencia='$referencia'"; $resultado=pg_exec($conn,$sSQL);  $filas=pg_numrows($resultado);
	  ...
	  $resultado=pg_exec($conn,$sSQL); echo $sSQL;
         $error=pg_errormessage($conn);  $error="ERROR GRABANDO: ".substr($error,0,91);if (!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php } else{ $error=0;}
      }
} pg_close($conn); 
