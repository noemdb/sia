...
if (pg_last_error($conn)){$error=1; ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
  ...
  if($filas>=1){$error=1; ?> <script language="JavaScript"> muestra('LINEA DE ARCHIVO YA EXISTE');</script><?php }
  if($error==0){if(strlen($linea)==8){$error=0;} else{$error=1; ?> <script language="JavaScript"> muestra('LONGITUD LINEA INVALIDA');</script><?php } } 
