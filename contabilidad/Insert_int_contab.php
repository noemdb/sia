<?php include ("../class/conect.php");  include ("../class/funciones.php"); include ("../presupuesto/Ver_dispon.php"); include ("../class/configura.inc");   error_reporting(E_ALL); $periodo="01";
...
if (checkData($fecha)=='1'){$error=0;}else{$error=1; ?> <script language="JavaScript">muestra('FECHA DE MOVIMIENTO NO ES VALIDA');</script><?php }
if (checkData($fecha_reg)=='1'){$error=0;}else{$error=1; ?> <script language="JavaScript">muestra('FECHA REGISTRO NO ES VALIDA');</script><?php }
if ($error==0){  $sfecha=formato_aaaammdd($fecha);  $rfecha=$sfecha; 
  ...
  if ($SIA_Definicion=="N"){$error=1;?><script language="JavaScript">muestra('ETAPA DE DEFINICIÒN ABIERTA');</script><?php } $periodom=$SIA_Periodo;  $nmes=substr($fecha,3, 2);  
  ...
  if($error==0){  $sSQL="Select * from CON012 where text(fecha_mov)='$sfecha' "; $resultado=pg_exec($conn,$sSQL);$filas=pg_numrows($resultado);
    if($filas>0){$error=1;?> <script language="JavaScript"> muestra('FECHA DE MOVIMIENTO YA PROCESADO');  </script>  <?php }
  }
  ...
  if($error==0){$total_mov=0; $t_debe=0; $t_haber=0; $balance=0;
	...
	if($total_mov==0){$error=1;?><script language="JavaScript">muestra('MONTO TOTAL DEL MOVIMIENTO INVALIDO');</script><?php }
	if($balance==0){$error=$error;}else{$error=1; echo $balance.' '.$t_debe.' '.$t_haber;  ?> <script language="JavaScript"> muestra('MOVIMIENTO NO CUADRA');</script><?php }
 }
  if($error==0){ $t_debe=0; $t_haber=0;$balance=0;
    ...
    while($registro=pg_fetch_array($res)){ $monto_asiento=$registro["monto_asiento"]; $dc=$registro["debito_credito"]; $codigo_cuenta=$registro["cod_cuenta"];
       ...
       else{ $reg=pg_fetch_array($resultado); if ($reg["cargable"]=="N"){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DE CUENTA NO ES CARGABLE');</script><?php } }

    } $balance=$t_debe-$t_haber; $balance=formato_monto($balance); $balance=cambia_coma_numero($balance); $dif=$total_mov-$t_debe;
    if($balance==0){$error=0;}else{$error=1; echo $balance.' '.$t_debe.' '.$t_haber; ?> <script language="JavaScript"> muestra('COMPROBANTE CONTABLE NO CUADRA');</script><?php }
    if(($t_debe==0)or(($total_mov<>$t_debe)and($dif>0.001))){$error=1;  echo $total_mov.' '.$t_debe.' '.$t_haber; ?> <script language="JavaScript"> muestra('MONTO DEL COMPROBANTE CONTABLE NO VALIDO');</script><?php }
  }
  if($error==0){ $fecha_pre=$fecha;  if($afecta_p=="SI"){$fecha_pre=$fecha_reg;}
    ...
    while(($registro=pg_fetch_array($res))and($error==0)){
      ...
      if (verifica_disponibilidad($conn,$cod_presup,$fuente_financ,$fecha_pre,$monto_c)==0){$error=0;}
         else{$error=1;?><script language="JavaScript">muestra('ERRRO EN EL CODIGO PRESUPUESTARIO:<?php  echo $cod_presup; ?> FUENTE:<?php  echo $fuente_financ; ?>');</script><?php }
      }
 }	  
 if($error==0){ $rfecha=formato_aaaammdd($fecha); 
   ...
   if (($rfecha>$Fec_Fin_Ejer)or($rfecha<$Fec_Ini_Ejer)){$error=1;?><script language="JavaScript">muestra('FECHA DE REGISTRO INVALIDA');</script><?php }
 }
 if($error==0){ $resultado=pg_exec($conn,"SELECT INCLUYE_CON012('$codigo_mov','$fecha','$fecha_reg','$ced_rif','$nusuario_sia','$minf_usuario')");$error=pg_errormessage($conn);
     if (!$resultado){ echo $error; $error=substr($error,0,91); ?> <script language="JavaScript"> muestra('<?php  echo $error; ?>'); </script> <?php }
