   ...
   $sfecha=formato_aaaammdd($fecha);  $Nom_Emp=busca_conf(); if($sfecha>$Fec_Fin_Ejer){ $fecha=formato_ddmmaaaa($Fec_Fin_Ejer); }    
	$sql="Select * from SIA005 where campo501='05'";$resultado=pg_query($sql);
     ...
     if($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO PRESUPUESTARIO NO EXISTE');</script><?php }
      ...
      if($error==0){$sSQL="SELECT referencia_pago FROM PRE008 WHERE referencia_pago='$referencia' and tipo_pago='$tipo_comp'"; $resultado=pg_query($sSQL); $filas=pg_num_rows($resultado); if($filas>0){$error=1; ?> <script language="JavaScript"> muestra('REFERENCIA DE YA EXISTE');</script><?php } }
      if(($error==0)and($monto_asiento==0)){$error=1; ?> <script language="JavaScript"> muestra('MONTO DE MOVIMIENTO INVALIDO');</script><?php }
	 ...
	 if ($error==0){  $sSQL="Select * from CON017 WHERE codigo_mov='$codigo_mov' and cod_cuenta='$cod_presup' and debito_credito='$debito_credito' and referencia='$referencia' and monto_asiento=$monto_asiento"; $resultado=pg_exec($conn,$sSQL);  $filas=pg_numrows($resultado);
	  ...
	  if ($error==0){  $sSQL="Select * from CON017 WHERE codigo_mov='$codigo_mov' and modulo='E'  and  referencia='$referencia' "; $resultado=pg_exec($conn,$sSQL);  $filas=pg_numrows($resultado);
         if ($filas>0){ $error=1;?> <script language="JavaScript">muestra('REEFERENCIA DE MOVIMIENTO EGRESO YA EXISTE');</script> <?php }
