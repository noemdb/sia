<?php include ("../class/conect.php");  include ("../class/funciones.php"); $periodo=$_POST["txtperiodo"]; $error=0;
...
if (pg_last_error($conn)) { ?> <script language="JavaScript"> muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS'); </script> <?php }
 else{ $resultado=pg_exec($conn,"UPDATE SIA000 set campo033='$periodo'"); $error=pg_errormessage($conn); $error=substr($error,0,91); if(!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php } }
