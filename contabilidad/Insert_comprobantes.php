...
if($tipo_asiento=='ANU'||$tipo_asiento=='ANC'||$tipo_asiento=='AND'){$error=1; ?> <script language="JavaScript"> muestra('TIPO DE ASIENTO NO VALIDO');</script><?php }
if ($error==0){ $conn=pg_connect("host=".$host." port=".$port." password=".$password." user=".$user." dbname=".$dbname."");
 ...
 if ($registro=pg_fetch_array($resultado,0)){$campo502=$registro["campo502"]; if ($SIA_Periodo<$registro["campo503"]){$SIA_Periodo=$registro["campo503"];}} 
  $nmes=substr($fecha,3, 2);  if($periodom<$SIA_Periodo){$periodom=$SIA_Periodo;}if($periodom>$nmes){$error=1;?><script language="JavaScript">muestra('FECHA DE MES MENOR A ULTIMO PERIODO CERRADO');</script><?php } }
 if($error==0){
   $sSQL="Select * from con009 WHERE tipo_asiento='$tipo_asiento'"; $resultado=pg_exec($conn,$sSQL); $filas=pg_numrows($resultado);
    if ($filas==0){$error=1;?>  <script language="JavaScript">  muestra('TIPO DE ASIENTO NO EXISTE');  </script>  <?php }
      ...
      if ($error==0){ $sql="SELECT * FROM CON008  where codigo_mov='$codigo_mov' order by debito_credito desc,cod_cuenta";
         ...
         if($balance>0.001){$error=1; echo $t_debe." ".$t_haber." ".formato_monto($balance); ?> <script language="JavaScript"> muestra('COMPROBANTE CONTABLE NO CUADRA');</script><?php }
	  ...
	  if($error==0){ 
        if (($sfecha>$Fec_Fin_Ejer)or($sfecha<$Fec_Ini_Ejer)){$error=1;?><script language="JavaScript">muestra('FECHA DE COMPROBANTE INVALIDA');</script><?php }
      }
