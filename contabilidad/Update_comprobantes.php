<?php include ("../class/conect.php");  include ("../class/funciones.php");$fecha=$_POST["txtFecha"];$referencia=$_POST["txtReferencia"];
...
if (pg_last_error($conn)) { ?> <script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS'); </script> <?php }
  ...
  if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('COMPROBANTE CONTABLE NO EXISTE');</script><?php }
   else{   $registro=pg_fetch_array($resultado);   $adescripcion=$registro["descripcion"];
     if ($registro["modulo"]=="C"||$registro["modulo"]=="I"||$registro["modulo"]=="O"){$error=0;}else{$error=1; ?> <script language="JavaScript"> muestra('COMPROBANTE NO PUEDE SER MODIFICADO');</script><?php }
     if ($error==0){ $sqlg="SELECT MODIFICA_CON002('$codigo_mov','$referencia','$sfecha','$tipo_asiento','$tipo_Comp','$status','$nro_comprobante','$ced_rif','$descripcion')";
      ...
      if (!$resultado){ ?> <script language="JavaScript">  muestra('<?php  echo $error; ?>'); </script> <?php }
         ...
         if (!$resultado){ ?> <script language="JavaScript">  muestra('<?php  echo $error; ?>'); </script> <?php }     ?> <script language="JavaScript">  muestra('MODIFICO EXITOSAMENTE'); </script> <?php 
