...
else{ $res=pg_exec($conn,"SELECT ELIMINA_CON010('$codigo_mov')"); $error=pg_errormessage($conn); $error=substr($error, 0, 61);if (!$res){ ?> <script language="JavaScript">  muestra('<?php  echo $error; ?>'); </script> <?php } 
...
while($registro=pg_fetch_array($res)){ $monto_asiento=$registro["monto_asiento"]; $codigo_cuenta=$registro["cod_cuenta"]; $debito_credito=$registro["debito_credito"];
  ...
  if($modulo=="C"){
   if($error==0){$sSQL="Select cargable,nombre_cuenta from con001 WHERE codigo_cuenta='$codigo_cuenta'"; $resultado=pg_query($sSQL); $filas=pg_num_rows($resultado);
      if ($filas==0){$error=1; echo $codigo_cuenta; ?> <script language="JavaScript"> muestra('CODIGO DE CUENTA NO EXISTE');</script><?php }
       else{ $registro=pg_fetch_array($resultado); $nombre_cuenta=$registro["nombre_cuenta"]; if($registro["cargable"]=="N"){$error=1; echo $codigo_cuenta; ?> <script language="JavaScript"> muestra('CODIGO DE CUENTA NO ES CARGABLE');</script><?php } } }
   ...
   if ($error==0){  $sSQL="Select * from CON010 WHERE codigo_mov='$codigo_mov' and cod_cuenta='$codigo_cuenta' and debito_credito='$debito_credito' and monto_asiento=$monto_asiento"; $resultado=pg_exec($conn,$sSQL);$filas=pg_numrows($resultado);  
      if ($filas>0){ $error=1; echo $codigo_cuenta; ?> <script language="JavaScript">muestra('CODIGO DE CUENTA YA EXISTE');</script> <?php } 
  ...
  if($modulo=="I"){ $sSQL="Select codigo_contable, nombre from ingre001 WHERE codigo_presup='$cod_presup'";  $resultado=pg_query($sSQL); $filas=pg_num_rows($resultado);
    if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DE INGRESOS NO EXISTE');</script><?php }
