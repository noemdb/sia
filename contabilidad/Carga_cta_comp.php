...
if (pg_last_error($conn)) { ?>  <script language="JavaScript">  muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');  </script>  <?php }
 else{ $res=pg_exec($conn,"SELECT ELIMINA_CON008('$codigo_mov')"); $error=pg_errormessage($conn); $error=substr($error, 0, 61);if (!$res){ ?> <script language="JavaScript">  muestra('<?php  echo $error; ?>'); </script> <?php }
  ...
  if ($filas==0){?> <script language="JavaScript">  muestra('COMPROBANTE A CARGAR NO LOCALIZADO'); </script>    <?php }
...
pg_close($conn);?>
<script language="JavaScript">document.location ='Det_inc_comprobantes.php?codigo_mov=<?php echo $codigo_mov?>';</script>
