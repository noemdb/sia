<?php include ("../class/conect.php");  include ("../class/funciones.php");include ("../presupuesto/Ver_dispon.php"); include ("../class/configura.inc");
...
if ($error==0){  $sfecha=formato_aaaammdd($fecha_pago);  $rfecha=$sfecha;
  ...
  if ($SIA_Definicion=="N"){$error=1;?><script language="JavaScript">muestra('ETAPA DE DEFINICIÒN ABIERTA');</script><?php } $periodom=$SIA_Periodo;  $nmes=substr($fecha_pago,3, 2);  
  ...
  if($periodom<$SIA_Periodo){$periodom=$SIA_Periodo;}if($periodom>$nmes){$error=1;?><script language="JavaScript">muestra('FECHA DE MES MENOR A ULTIMO PERIODO CERRADO');</script><?php }
    ...
    while(($registro=pg_fetch_array($res))and($error==0)){
      ...
      if($refierea=="NINGUNO"){ echo $monto_c.' '.$total;
        if (verifica_disponibilidad($conn,$registro["cod_presup"],$registro["fuente_financ"],$fecha_pago,$monto_c)==0){$error=0;}
         else{$error=1;?><script language="JavaScript">muestra('ERRRO EN EL CODIGO PRESUPUESTARIO:<?php  echo $registro["cod_presup"]; ?> FUENTE:<?php  echo $registro["fuente_financ"]; ?> REF.CREDITO:<?php  echo $registro["ref_imput_presu"]; ?>');</script><?php }
        if(($error==0)and($tipo_imput_presu=="C")){
          ...
          if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CÓDIGO PRESUPUESTARIO:<?php  echo $registro["cod_presup"]; ?> FUENTE:<?php  echo $registro["fuente_financ"]; ?> REF.CREDITO:<?php  echo $registro["ref_imput_presu"]; ?> \ncon NO EXISTE EN LA EJECUCIÓN DEL CREDITO ADICIONAL');</script><?php }
           else{$reg=pg_fetch_array($res);
             if($registro["disponible"]<$monto_credito) {$error=1; $dispon=$registro["disponible"]; $dispon=formato_monto($dispon); ?> <script language="JavaScript"> muestra('CÓDIGO PRESUPUESTARIO:<?php  echo $registro["cod_presup"]; ?> FUENTE:<?php  echo $registro["fuente_financ"]; ?> REF.CREDITO:<?php  echo $registro["ref_imput_presu"]; ?> \ncon Monto Mayor que Disponibilidad del Crédito Adicional, Disponibilidad: <?php  echo $dispon; ?> ');</script><?php }
          }
        }
      }
      if($refierea=="COMPROMISO"){
        ...
        if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('NO EXISTE EN EL COMPROMISO EL CODIGO PRESUPUESTARIO:<?php  echo $registro["cod_presup"]; ?> FUENTE:<?php  echo $registro["fuente_financ"]; ?> REF.CREDITO:<?php  echo $registro["ref_imput_presu"]; ?>');</script><?php }
           ...
           if($monto_c>$compromiso){$error=1; ?> <script language="JavaScript"> muestra('MONTO A PAGAR MAYOR AL MONTO DEL CÓDIGO DEL COMPROMISO POR PAGAR');</script><?php }
         }
      }
      if($refierea=="CAUSADO"){
        ...
        if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('NO EXISTE EN EL CAUSADO EL CODIGO PRESUPUESTARIO:<?php  echo $registro["cod_presup"]; ?> FUENTE:<?php  echo $registro["fuente_financ"]; ?> REF.CREDITO:<?php  echo $registro["ref_imput_presu"]; ?>');</script><?php }
           ...
           $compromiso=$registro["monto"]-$registro["pagado"]-$registro["ajustado"];
           if($monto_c>$compromiso){$error=1; ?> <script language="JavaScript"> muestra('MONTO A PAGAR MAYOR AL MONTO DEL CÓDIGO DEL CAUSADO POR PAGAR');</script><?php }
         }
      }
    }
  ...
  if(($error==0)and($genera_comprobante=='S')){
         ...
         IF ($balance==0){$error=0;}else{$error=1; ?> <script language="JavaScript"> muestra('COMPROBANTE CONTABLE NO CUADRA');</script><?php }
