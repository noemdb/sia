...
if ($error==0){ $conn=pg_connect("host=".$host." port=".$port." password=".$password." user=".$user." dbname=".$dbname."");
    ...
    while($registro=pg_fetch_array($res)){ $c=$c+1; $tipo_id=$registro["tipo_id"];   $monto_c=$registro["monto"]; $cod_bien_mue=$registro["cod_bien"]; $campo_str2=$registro["campo_str2"];
       if($error==0){ $sSQL="Select * from BIEN015 WHERE cod_bien_mue='$cod_bien_mue'";  $resultado=pg_query($sSQL);  $filas=pg_num_rows($resultado);
         if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DEL BIEN <?php  echo $cod_bien_mue; ?> NO EXISTE');</script><?php }  
		    ...
		    if($desincorporado=="S"){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DEL BIEN EMISOR <?php  echo $cod_bien_mue; ?> ESTA DESINCORPORADO');</script><?php }
		  }
	    }
		if($error==0){ $sSQL="Select * from BIEN015 WHERE cod_bien_mue='$campo_str2'";  $resultado=pg_query($sSQL);  $filas=pg_num_rows($resultado);
		  if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DEL BIEN <?php  echo $cod_bien_mue; ?> NO EXISTE');</script><?php }  
            ...
            $cod_depb=$reg["cod_dependencia"];   $cod_dirb=$reg["cod_direccion"];  $cod_departb=$reg["cod_departamento"]; 
		    if($desincorporado=="S"){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DEL BIEN RECEPTOR <?php  echo $cod_bien_mue; ?> ESTA DESINCORPORADO');</script><?php }
		  }
		}
	}	
	if(($c==0)){$error=1;?><script language="JavaScript">muestra('TRANSFERENCIA NO TIENE MOVIMIENTOS');</script><?php }
  }  
  if($error==0){ $sfecha=formato_aaaammdd($fecha_transf); $tipo_mov_r="002"; $tipo_mov_e="051";
     ...
     if (!$resultado){?><script language="JavaScript"> muestra('<?php  echo $error; ?>');</script><?php  $error=1;}
      else{ $error=0;?><script language="JavaScript">  muestra('INCLUYO EXITOSAMENTE');</script><?php     $resultado=pg_exec($conn,"SELECT ELIMINA_BIEN050('$codigo_mov')"); $resultado=pg_exec($conn,"SELECT ELIMINA_CON010('$codigo_mov')"); }  
  }
}
