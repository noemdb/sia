...
if (pg_last_error($conn)) { ?> <script language="JavaScript">   muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');  </script> <?php }
  ...
  if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DE LA DEPENDENCIA NO EXISTE');</script> <?php }
  if ($error==0){  $sSQL="SELECT * From bien005 where cod_dependencia='$cod_dependencia'";  $resultado=pg_exec($conn,$sSQL);  $filas=pg_numrows($resultado);
    if ($filas>=1){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DE LA DEPENDENCIA TIENE DIRECCIONES ASOCIADAS');</script> <?php }	  
  }	  
  if ($error==0){  $resultado=pg_exec($conn,"SELECT ACTUALIZA_BIEN001(3,'$cod_dependencia','','','','','','','','','','','','','',0,'','','','',0,0,'')"); 
     ...
     if (!$resultado){$error=1; ?> <script language="JavaScript"> muestra('<?php  echo $merror; ?>'); </script> <?php }else{?><script language="JavaScript">muestra('ELIMINO EXITOSAMENTE');</script><?php  $error=0; }
  }
}
