...
$conn=pg_connect("host=".$host." port=".$port." password=".$password." user=".$user." dbname=".$dbname.""); if (pg_last_error($conn)) { ?> <script language="JavaScript"> muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS'); </script> <?php }
  ...
  if($filas==0){$error=1; ECHO $sql; ?> <script language="JavaScript"> muestra('USUARIO NO EXISTE'); </script><?php }
  ...
  if($error==0){ if($ClaveN<>$ClaveNR) {$error=1;?> <script language="JavaScript"> muestra('CLAVES NO SON IGUALES'); </script><?php }
     else{$resultado=pg_exec($conn,"SELECT cambio_sia001('$login','$ClaveN')"); $error=pg_errormessage($conn); $error=substr($error,0,91); if(!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php } else{?><script language="JavaScript">muestra('COMBIO CLAVE EXITOSAMENTE');</script><?php }
  }}
}pg_close($conn); error_reporting(E_ALL ^ E_WARNING);if($error==0){?><script language="JavaScript">document.location='menu.php';</script><?php }else{?><script language="JavaScript">history.back();</script><?php }?>
