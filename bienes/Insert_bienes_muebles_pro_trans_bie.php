...
if (checkData($fecha_transf)=='1'){$error=0; $fecha=$fecha_transf;}else{$error=1; ?> <script language="JavaScript">muestra('FECHA NO ES VALIDA');</script><?php }
...
if ($error==0){ $conn=pg_connect("host=".$host." port=".$port." password=".$password." user=".$user." dbname=".$dbname."");
  ...
  if($error==0){     
    ...
    if ($filas>0){$error=1;?><script language="JavaScript">muestra('REFERENCIA DE TRANSFERENCIA YA EXISTE');</script><?php }
  }
  ...
  if($error==0){ $fecha_d=formato_ddmmaaaa($Fec_Ini_Ejer);   $fecha_h=formato_ddmmaaaa($Fec_Fin_Ejer);
    if (($sfecha>$Fec_Fin_Ejer)or($sfecha<$Fec_Ini_Ejer)){$error=1;?><script language="JavaScript">muestra('FECHA DE TRANSFERENCIA INVALIDA');</script><?php }
  }
  if($error==0){$nmes=substr($fecha,3, 2);
	If ($SIA_Periodo>$nmes){$error=1;?><script language="JavaScript">muestra('FECHA TRANSFERENCIA MENOR A ULTIMO PERIODO CERRADO');</script><?php }
  }  
  if($error==0){ $sql="SELECT * FROM BIEN050 where codigo_mov='$codigo_mov' order by cod_bien";  $res=pg_query($sql); $total=0; $c=0;
    while($registro=pg_fetch_array($res)){ $total=$total+$registro["monto"]; $c=$c+1; $tipo_id=$registro["tipo_id"];   $monto_c=$registro["monto"]; $cod_bien_mue=$registro["cod_bien"];
       if($error==0){ $sSQL="Select * from BIEN015 WHERE cod_bien_mue='$cod_bien_mue'";  $resultado=pg_query($sSQL);  $filas=pg_num_rows($resultado);
         if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DEL BIEN <?php  echo $cod_bien_mue; ?> NO EXISTE');</script><?php }  
		    ...
		    if($cod_depb<>$cod_dependencia_e) {$error=1; ?> <script language="JavaScript"> muestra('CODIGO DEPENDECIA DEL BIEN <?php  echo $cod_bien_mue; ?> DIFERENTE A LA DEL MOVIMIENTO');</script><?php }
			if($cod_dirb<>$cod_direccion_e) {$error=1; ?> <script language="JavaScript"> muestra('CODIGO DIRECCION DEL BIEN <?php  echo $cod_bien_mue; ?> DIFERENTE A LA DEL MOVIMIENTO');</script><?php }
			if($cod_departb<>$cod_departamento_e) {$error=1; ?> <script language="JavaScript"> muestra('CODIGO DEPARTAMENTO DEL BIEN <?php  echo $cod_bien_mue; ?> DIFERENTE A LA DEL MOVIMIENTO');</script><?php }
    ...
    /*if(($total==0)or($c==0)){$error=1;?><script language="JavaScript">muestra('MONTO TRANSFERENCIA INVALIDO');</script><?php }*/
	if(($c==0)){$error=1;?><script language="JavaScript">muestra('CANTIDAD DE BIENES EN TRANSFERENCIA INVALIDA');</script><?php }
  }  
  if($error==0){     
    if(($tipo_transferencia=="E")and($cod_dependencia_e==$cod_dependencia_r)){$error=1;?><script language="JavaScript">muestra('TIPO DE TRANSFERENCIA INVALIDA');</script><?php }
  }
  if($error==0){$sql="SELECT cod_dependencia,denominacion_dep  FROM bien001 where cod_dependencia='$cod_dependencia_r'"; $resultado=pg_query($sql);  $filas=pg_num_rows($resultado);
     if($filas==0){$error=1; echo $cod_dependencia_r; ?> <script language="JavaScript"> muestra('CODIGO DEPENDENCIA NO EXISTE');</script><?php }
   }
   if($error==0){$sql="SELECT cod_direccion,denominacion_dir FROM bien005 where cod_dependencia='$cod_dependencia_r' and cod_direccion='$cod_direccion_r'"; $resultado=pg_query($sql);  $filas=pg_num_rows($resultado);
     if($filas==0){$error=1; echo $cod_direccion_r; ?> <script language="JavaScript"> muestra('CODIGO DIRECCION NO EXISTE');</script><?php }
   }
   if($error==0){$sql="Select denominacion_dep from BIEN006 WHERE cod_dependencia='$cod_dependencia_r' and cod_direccion='$cod_direccion_r' and cod_departamento='$cod_departamento_r'"; $resultado=pg_query($sql);  $filas=pg_num_rows($resultado);
     if($filas==0){$error=1; echo $cod_departamento_r; ?> <script language="JavaScript"> muestra('CODIGO DEPARTAMENTO NO EXISTE');</script><?php }
   }
  ...
  if($error==0){ $sfecha=formato_aaaammdd($fecha_transf); $tipo_mov_r="002"; $tipo_mov_e="051";
     ...
     $error=pg_errormessage($conn);  $error=substr($error, 0, 91);   if (!$resultado){?><script language="JavaScript"> muestra('<?php  echo $error; ?>');</script><?php  $error=1;}
