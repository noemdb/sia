<?php  include ("../class/conect.php");  include ("../class/funciones.php"); include ("../class/configura.inc");error_reporting(E_ALL); $cod_modulo="13";
...
if ($error==0){ $conn=pg_connect("host=".$host." port=".$port." password=".$password." user=".$user." dbname=".$dbname."");
  ...
  if($error==0){  $sSQL="Select * from bien028 WHERE referencia_dep='$referencia'";
    ...
    if ($filas>0){$error=1;?><script language="JavaScript">muestra('REFERENCIA DE DEPRECIACION YA EXISTE');</script><?php }
  }  
  $sfecha=formato_aaaammdd($fecha);
  if($error==0){ $fecha_d=formato_ddmmaaaa($Fec_Ini_Ejer);   $fecha_h=formato_ddmmaaaa($Fec_Fin_Ejer);
    if (($sfecha>$Fec_Fin_Ejer)or($sfecha<$Fec_Ini_Ejer)){$error=1;?><script language="JavaScript">muestra('FECHA DE DEPRECIACION INVALIDA');</script><?php }
  }
  if($error==0){$nmes=substr($fecha,3, 2);
	If ($SIA_Periodo>$nmes){$error=1;?><script language="JavaScript">muestra('FECHA DEPRECIACION MENOR A ULTIMO PERIODO CERRADO');</script><?php }
  }  
  ...
  if($error==0){ $sql="SELECT * FROM BIEN050 where codigo_mov='$codigo_mov' order by cod_bien";  $res=pg_query($sql); $total=0;
    while($registro=pg_fetch_array($res)){    $total=$total+$registro["monto"];  $tipo_id=$registro["tipo_id"];   $monto_c=$registro["monto"]; $cod_bien_mue=$registro["cod_bien"];
	   ...
	   if($met_calculo<>$tipo_id){ $error=1; ?> <script language="JavaScript"> muestra('TIPO CALCULO DEPRECIACION DIFERENTE AL DEL CALCULO');</script><?php }
	   if($error==0){ $sSQL="Select * from BIEN015 WHERE cod_bien_mue='$cod_bien_mue'";  $resultado=pg_query($sSQL);  $filas=pg_num_rows($resultado);
         if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DEL BIEN <?php  echo $cod_bien_mue; ?> NO EXISTE');</script><?php }  
		  else{   $reg=pg_fetch_array($resultado); $desincorporado=$reg["desincorporado"];   
		      if(($desincorporado=="S")){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DEL BIEN <?php  echo $cod_bien_mue; ?> ESTA DESINCORPORADO');</script><?php }
		  }
	    }
	}
    if($total==0){$error=1;?><script language="JavaScript">muestra('MONTO DEL MOVIMIENTO INVALIDO');</script><?php }
  ...
  if($error==0){$sql="SELECT * FROM CON010 where codigo_mov='$codigo_mov' order by debito_credito desc,cod_cuenta";$res=pg_query($sql);
    while($registro=pg_fetch_array($res)){ $monto_asiento=$registro["monto_asiento"]; $cod_cuenta=$registro["cod_cuenta"];	    
        ...
        if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DE CUENTA <?php  echo $codigo_cuenta; ?> NO EXISTE');</script><?php }
          else{ $reg=pg_fetch_array($resultado); if ($reg["cargable"]=="N"){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DE CUENTA <?php  echo $codigo_cuenta; ?> NO ES CARGABLE');</script><?php } }
	...
	if($gen_comp=="S"){
       ...
       if ($balance>0.001){$error=1; echo ' Debe:'.$t_debe.' Haber:'.$t_haber.' Balance:'.formato_monto($balance),"<br>"; ?> <script language="JavaScript"> muestra('COMPROBANTE CONTABLE NO CUADRA');</script><?php }
       ...
       if ($balance>0.001){$error=1; echo ' Debe:'.$t_debe.' Total:'.$total.' Diferencia:'.formato_monto($balance),"<br>"; ?> <script language="JavaScript"> muestra('COMPROBANTE CONTABLE NO CUADRA CON TOTAL DEPRECIACION');</script><?php }
    ...
    while(($registro=pg_fetch_array($res))and($error==0)){
	  ...
	  if($ref_compromiso=="NO"){ 
        ...
        if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('NO EXISTE EN EL COMPROMISO EL CODIGO PRESUPUESTARIO:<?php  echo $registro["cod_presup"]; ?> FUENTE:<?php  echo $registro["fuente_financ"]; ?> REF.CREDITO:<?php  echo $registro["ref_imput_presu"]; ?>');</script><?php }
		   ...
		   if(($monto_c>$compromiso)and($diferencia>0.001)){$error=1; ?> <script language="JavaScript"> muestra('MONTO A CAUSAR MAYOR AL MONTO DEL CODIGO POR COMPROMETER');</script><?php }
         }
      }
	}
	if ($total_c>$total){$balance=$total_c-$total;}else{$balance=$total-$total_c;}
    if ($balance>0.001){$error=1; echo ' Causado:'.$total_c.' Total:'.$total.' Diferencia:'.formato_monto($balance),"<br>"; ?> <script language="JavaScript"> muestra('CAUSADO NO CUADRA CON TOTAL DEPRECIACION');</script><?php }
