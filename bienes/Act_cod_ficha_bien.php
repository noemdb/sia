<?php include ("../class/conect.php"); include ("../class/funciones.php"); include ("../class/configura.inc"); error_reporting(E_ALL); $equipo = getenv("COMPUTERNAME");  $error=0; $fecha_hoy=asigna_fecha_hoy(); 
...
if (pg_last_error($conn)) { ?> <script language="JavaScript">   muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');  </script> <?php }
  ...
  while(($registro=pg_fetch_array($res)) ){ $grupo_c=$registro["grupo_c"];  $codigo_c=$registro["codigo_c"];
    ...
    $resg=pg_exec($conn,$sqlg); $merror=pg_errormessage($conn); $merror=substr($merror,0,91); if (!$resg){?><script language="JavaScript">muestra('<?php echo $merror;?>');</script><?php }
  }
}
