...
if ($error==0){ $conn=pg_connect("host=".$host." port=".$port." password=".$password." user=".$user." dbname=".$dbname."");
  ...
  if($error==0){     
    ...
    if ($filas>0){$error=1;?><script language="JavaScript">muestra('REFERENCIA ORDEN DE SALIDA YA EXISTE');</script><?php }
  }
  ...
  if($error==0){ $fecha_d=formato_ddmmaaaa($Fec_Ini_Ejer);   $fecha_h=formato_ddmmaaaa($Fec_Fin_Ejer);
    if (($sfecha>$Fec_Fin_Ejer)or($sfecha<$Fec_Ini_Ejer)){$error=1;?><script language="JavaScript">muestra('FECHA ORDEN DE SALIDA  INVALIDA');</script><?php }
  }
  if($error==0){$nmes=substr($fecha,3, 2);
	If ($SIA_Periodo>$nmes){$error=1;?><script language="JavaScript">muestra('FECHA ORDEN DE SALIDA  MENOR A ULTIMO PERIODO CERRADO');</script><?php }
  }
  if($error==0){ $sql="SELECT * FROM BIEN050 where codigo_mov='$codigo_mov' order by cod_bien";  $res=pg_query($sql); $total=0; $c=0;
    ...
    if(($total==0)or($c==0)){$error=1;?><script language="JavaScript">muestra('MONTO ORDEN DE SALIDA  INVALIDO');</script><?php }
  }
