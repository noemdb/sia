...
if (pg_last_error($conn)){ ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
  ...
  if ($filas>0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DEL BIEN YA EXISTE EN LA TRANSFERENCIA');</script><?php }
  if($error==0){ $sSQL="Select * from BIEN015 WHERE cod_bien_mue='$cod_bien_mue'";  $resultado=pg_query($sSQL);  $filas=pg_num_rows($resultado);
      if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DEL BIEN EMISOR NO EXISTE');</script><?php }  
	    else{  $registro=pg_fetch_array($resultado); $desincorporado=$registro["desincorporado"];  if($desincorporado=="S"){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DEL BIEN EMISOR ESTA DESINCORPORADO');</script><?php } }  }		
  if($error==0){ $sSQL="Select * from BIEN015 WHERE cod_bien_mue='$cod_bien_r'";  $resultado=pg_query($sSQL);  $filas=pg_num_rows($resultado);
      if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DEL BIEN RECEPTOR NO EXISTE');</script><?php }  
	    else{  $registro=pg_fetch_array($resultado); $desincorporado=$registro["desincorporado"];  if($desincorporado=="S"){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DEL BIEN RECEPTOR ESTA DESINCORPORADO');</script><?php } }
  }
  if($error==0){ $sSQL="Select * from BIEN053 WHERE cod_bien_mue='$cod_bien_mue' and cod_componente='$cod_componente'";  $resultado=pg_query($sSQL);  $filas=pg_num_rows($resultado);
      if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO COMPONENETE NO EXISTE EN EL BIEN EMISOR ');</script><?php }   
  }  
  if($error==0){ $sSQL="Select * from BIEN053 WHERE cod_bien_mue='$cod_bien_r' and cod_componente='$cod_componente_r'";  $resultado=pg_query($sSQL);  $filas=pg_num_rows($resultado);
      if ($filas>=1){$error=1; ?> <script language="JavaScript"> muestra('CODIGO COMPONENTE YA EXISTE EN EL BIEN RECEPTOR');</script><?php }   
  }
  if($error==0){ if($cod_bien_mue==$cod_bien_r){ $error=1; ?> <script language="JavaScript"> muestra('CODIGO DEL BIEN RECEPTOR NO PUEDE SER IGUAL AL EMISOR');</script><?php } }  
