<?php include ("../class/conect.php");  include ("../class/funciones.php"); $codigo_c=$_GET["Gcodigo_c"]; $num_descrip=$_GET["num_descrip"];  $equipo=getenv("COMPUTERNAME"); $minf_usuario = $equipo." ".date("d/m/y H:i a");echo "ESPERE POR FAVOR ELIMINANDO....","<br>"; $error=0;
$conn=pg_connect("host=".$host." port=".$port." password=".$password." user=".$user." dbname=".$dbname."");if (pg_last_error($conn)) { ?> <script language="JavaScript">   muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');  </script> <?php }
  ...
  if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('EL BIEN NO EXISTE');</script> <?php }
