...
if (pg_last_error($conn)){ ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
  ...
  if($error==0){ $sSQL="Select denomina_tipo,tipo from BIEN003 WHERE codigo='$codigo'";  $resultado=pg_exec($conn,$sSQL);  $filas=pg_numrows($resultado);
     if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO TIPO DE MOVIMIENTO NO EXISTE');</script> <?php }   else{ $registro=pg_fetch_array($resultado); $tipo=$registro["tipo"];}
   }			   
