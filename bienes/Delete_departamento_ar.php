...
if (pg_last_error($conn)){$error=1; ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
  ...
  if($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DEPARTAMENTO NO EXISTE');</script><?php }
  ...
  if ($error==0){  $sSQL="SELECT * From bien015 where cod_dependencia='$cod_dependen' and cod_direccion='$cod_direcci' and cod_departamento='$cod_departamen'";  $resultado=pg_exec($conn,$sSQL);  $filas=pg_numrows($resultado);
    if ($filas>=1){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DE DEPARTAMENTO TIENE BIENES ASOCIADAS');</script> <?php }	  
  }	  
  if($error==0){$sSQL="SELECT ACTUALIZA_BIEN006(3,'$cod_dependen','$cod_direcci','$cod_departamen','','','','','$minf_usuario')";
    $resultado=pg_exec($conn,$sSQL); $merror=pg_errormessage($conn); $merror="ERROR GRABANDO: ".substr($merror, 0, 91); if (!$resultado){$error=1;?><script language="JavaScript">muestra('<?php  echo $merror; ?>');</script><?php }else{$error=0;?><script language="JavaScript">muestra('ELIMINO EXITOSAMENTE');</script><?php }
  }
