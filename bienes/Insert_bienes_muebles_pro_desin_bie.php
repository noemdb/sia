...
if (checkData($fecha_desin)=='1'){$error=0; $sfecha=formato_aaaammdd($fecha_desin);}else{$error=1; ?> <script language="JavaScript">muestra('FECHA NO ES VALIDA');</script><?php }
if ($error==0){  $conn=pg_connect("host=".$host." port=".$port." password=".$password." user=".$user." dbname=".$dbname."");
  ...
  if($error==0){  $sSQL="Select * from bien045 WHERE referencia_desin='$referencia_desin'";
    ...
    if ($filas>0){$error=1;?><script language="JavaScript">muestra('REFERENCIA DESINCORPORACION YA EXISTE');</script><?php }
  }
  ...
  if($error==0){ $sSQL="Select denomina_tipo from BIEN003 WHERE codigo='$tipo_desin'";  $resultado=pg_exec($conn,$sSQL);  $filas=pg_numrows($resultado);
     if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO TIPO DE DESINCORPORACION NO EXISTE');</script> <?php }
   }   
  if($error==0){ $fecha_d=formato_ddmmaaaa($Fec_Ini_Ejer);   $fecha_h=formato_ddmmaaaa($Fec_Fin_Ejer);
    if (($sfecha>$Fec_Fin_Ejer)or($sfecha<$Fec_Ini_Ejer)){$error=1;?><script language="JavaScript">muestra('FECHA DE DESINCORPORACION INVALIDA');</script><?php }
  }  
    ...
    while($registro=pg_fetch_array($res)){    $total=$total+$registro["monto"];     $monto_c=$registro["monto"]; $cod_bien_mue=$registro["cod_bien"];
       if($error==0){ $sSQL="Select * from BIEN015 WHERE cod_bien_mue='$cod_bien_mue'";  $resultado=pg_query($sSQL);  $filas=pg_num_rows($resultado);
         if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DEL BIEN <?php  echo $cod_bien_mue; ?> NO EXISTE');</script><?php }  
		    ...
		    if($cod_depb<>$cod_dependencia) {$error=1; ?> <script language="JavaScript"> muestra('CODIGO DEPENDECIA DEL BIEN <?php  echo $cod_bien_mue; ?> DIFERENTE A LA DEL MOVIMIENTO');</script><?php }
			if($cod_dirb<>$cod_direccion) {$error=1; ?> <script language="JavaScript"> muestra('CODIGO DIRECCION DEL BIEN <?php  echo $cod_bien_mue; ?> DIFERENTE A LA DEL MOVIMIENTO');</script><?php }
			if($cod_departb<>$cod_departamento) {$error=1; ?> <script language="JavaScript"> muestra('CODIGO DEPARTAMENTO DEL BIEN <?php  echo $cod_bien_mue; ?> DIFERENTE A LA DEL MOVIMIENTO');</script><?php }
  ...
  if($error==0){$sql="SELECT * FROM CON010 where codigo_mov='$codigo_mov' order by debito_credito desc,cod_cuenta";$res=pg_query($sql);
	...
	if($campo_str2=="S"){
	   ...
	   if($error==0){ if ($t_debe>$total){$balance=$t_debe-$total;}else{$balance=$total-$t_debe;}
         if($balance>0.001){$error=1; echo 'Comprobante:'.$t_debe.' Movimiento:'.$total.' '.formato_monto($balance),"<br>"; ?> <script language="JavaScript"> muestra('COMPROBANTE NO CUADRA CON TOTAL MOVIMIENTO');</script><?php } }
	}
  }
  if($error==0){$sfecha=formato_aaaammdd($fecha_desin); if($ced_rif==""){ $ced_rif=$Rif_Emp; }
     ...
     $resultado=pg_exec($conn,$sSQL); $error=pg_errormessage($conn); $error=substr($error, 0, 91); if (!$resultado){?><script language="JavaScript"> muestra('<?php  echo $error; ?>');</script><?php  $error=1;}
