...
if (pg_last_error($conn)){ ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
  ...
  if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DEL COMPONENTE NO EXISTE EN EL BIEN');</script><?php }
  if($error==0){ if(checkData($fecha_rep)=='1'){ $fecha_rep=formato_aaaammdd($fecha_rep); } else{$error=1; ?> <script language="JavaScript">muestra('FECHA NO ES VALIDA');</script><?php } }
  if($error==0){ if($valor_rep<=0){$error=1; ?> <script language="JavaScript">muestra('VALOR NO ES VALIDA');</script><?php } }  
  if($error==0){ $sfecha=formato_aaaammdd($fecha); $sql="SELECT ACTUALIZA_BIEN058(2,'$cod_bien_mue','$cod_bien_aso','$fecha_rep',$valor_rep,'$denominacion','$campo_str2',$monto1,$monto2)";
      $resultado=pg_exec($conn,$sql); $error=pg_errormessage($conn);  $error="ERROR GRABANDO: ".substr($error, 0, 90);     if (!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php }
      //echo $sql;
	}
  }
pg_close($conn);
