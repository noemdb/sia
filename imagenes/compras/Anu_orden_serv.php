...
if(pg_last_error($conn)) { ?> <script language="JavaScript"> muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');  </script> <?php }
  ...
  if($filas==0){$error=1;?><script language="JavaScript">muestra('NUMERO ORDEN DE SERVICIO NO EXISTE');</script><?php }
     ...
     if($error==0){$sfecha=$fecha_orden;if(checkData($fecha_anu)=='1'){$error=0;$afecha=formato_aaaammdd($fecha_anu);} else{$error=1; ?> <script language="JavaScript">muestra('FECHA ANULACION NO ES VALIDA');</script><?php } }
     ...
     if(($error==0)and(substr($tipo_compromiso,0,1)=="A")){$error=1;?><script language="JavaScript">muestra('ORDEN DE SERVICIO NO PUEDE SER ELIMINADA');</script><?php }
     ...
     if($error==0){$sql="SELECT referencia_pago,tipo_pago FROM PRE008 WHERE (referencia_comp='$nro_orden') And (tipo_compromiso='$tipo_compromiso') and (anulado='N')"; $resultado=pg_exec($conn,$sql);  $filas=pg_numrows($resultado);
       if($filas>0){$error=1;?><script language="JavaScript">muestra('Tiene Pagos que Refieren al Compromiso, No puede ser Anulado');</script><?php }
     }
     ...
     if($error==0){$sql="SELECT referencia_ajuste,tipo_ajuste FROM PRE011 WHERE (referencia_comp='$nro_orden') And (tipo_compromiso='$tipo_compromiso') and (anulado='N')";  $resultado=pg_exec($conn,$sql);  $filas=pg_numrows($resultado);
       if($filas>0){$error=1;?><script language="JavaScript">muestra('Tiene Ajustes que Refieren al Compromiso, No puede ser Anulado');</script><?php }
     }
