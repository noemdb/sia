...
?><select name="txtcod_articulo" id="txtcod_articulo" onFocus="encender(this)" onBlur="apaga_cart(this);" class="Estilo5">  <?php 
while($registro=pg_fetch_array($res)){$codigo=$registro["codigo_articulo"];  $nombre=$registro["des_articulo"];
if($codigo==$cod_art){?><option value="<?php  echo $nombre;?>" selected><?php  echo $codigo;?></option>
<?php }else{?><option value="<?php  echo $nombre;?>"><?php  echo $codigo;?></option>
