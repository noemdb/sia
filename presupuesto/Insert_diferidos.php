...
if (checkData($fecha_diferido)=='1'){$error=0;}else{$error=1; ?> <script language="JavaScript">muestra('FECHA NO ES VALIDA');</script><?php }
if ($error==0){
  ...
  if($error==0){ $sSQL="Select * from pre024 WHERE tipo_diferido='$tipo_diferido'"; $resultado=pg_exec($conn,$sSQL);    $filas=pg_numrows($resultado);
    if ($filas==0){$error=1;?><script language="JavaScript">muestra('TIPO DE DIFERIDO NO EXISTE');</script><?php }
     else{if(($tipo_diferido=="0001")or(substr($tipo_diferido,0,1)=='A')){$error=1;?><script language="JavaScript">  muestra('TIPO DE DIFERIDO NO VALIDO');</script><?php } }
  }
  if($error==0){  $sSQL="Select * from pre023 WHERE referencia_dife='$referencia_dife' and tipo_diferido='$tipo_diferido'";
    $resultado=pg_exec($conn,$sSQL);  $filas=pg_numrows($resultado);
    if ($filas>0){$error=1;?><script language="JavaScript">muestra('REFERENCIA DE MOVIMIENTO DIFERIDO YA EXISTE');</script><?php }
  }
  ...
  if($error==0){ $fecha_d=formato_ddmmaaaa($Fec_Ini_Ejer);   $fecha_h=formato_ddmmaaaa($Fec_Fin_Ejer);
    if (($sfecha>$Fec_Fin_Ejer)or($sfecha<$Fec_Ini_Ejer)){$error=1;?><script language="JavaScript">muestra('FECHA DE DIFERIDO INVALIDA');</script><?php }
  }
  if($error==0){$nmes=substr($fecha_diferido,3, 2);
    if ($SIA_Periodo>$nmes){echo "Ultimo Periodo: ".$SIA_Periodo." Periodo del Diferido: ".$nmes; $error=1;?><script language="JavaScript">muestra('FECHA DE DIFERIDO MENOR A ULTIMO PERIODO CERRADO');</script><?php }
  }
  if($error==0){ $sql="SELECT * FROM CODIGOS_PRE026 where codigo_mov='$codigo_mov' order by cod_presup";  $res=pg_query($sql);
    ...
    while($registro=pg_fetch_array($res)){ $total=$total+$registro["monto"];  $monto_c=$registro["monto"];
      if (verifica_disponibilidad($conn,$registro["cod_presup"],$registro["fuente_financ"],$fecha_diferido,$monto_c)==0){$error=0;}
       else{$error=1; echo $fecha_diferido,"<br>";; ?><script language="JavaScript">muestra('ERRRO EN EL CODIGO PRESUPUESTARIO:<?php  echo $registro["cod_presup"]; ?> FUENTE:<?php  echo $registro["fuente_financ"]; ?>');</script><?php }
    }
  ...
  if($error==0){ $sfecha=formato_aaaammdd($fecha_diferido);
     ...
     $error=pg_errormessage($conn); $error=substr($error, 0, 61);  if (!$resultado){?><script language="JavaScript"> muestra('<?php  echo $error; ?>');</script><?php  $error=1;}
      else{   $error=0;?><script language="JavaScript">  muestra('INCLUYO EXITOSAMENTE');</script><?php    $resultado=pg_exec($conn,"SELECT BORRAR_PRE026('$codigo_mov')");
...
if ($error==0){?><script language="JavaScript">document.location ='Act_diferidos.php?Gcriterio=<?php  echo $tipo_diferido.$referencia_dife; ?>';</script> <?php }
