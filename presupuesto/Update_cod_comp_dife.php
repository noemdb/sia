...
if (pg_last_error($conn)){ ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
  ...
  if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO PRESUPUESTARIO NO EXISTE EN EL COMPROMISO');</script><?php }
   else{  
    ...
    if(($error==0)and($tipo_imput_presu=="C")){$monto_credito=$monto;
      if($monto_credito>$monto){ $error=1; ?> <script language="JavaScript"> muestra('MONTO CREDITO INVALIDO');</script><?php }
    }
    ...
    if($error==0){   $sfecha=formato_aaaammdd($fecha); $sql="SELECT MODIFICA_PRE026('$codigo_mov','$cod_presup','$fuente_financ','','0000','','0000','','0000','','0000','','','','','','','$sfecha','C','$tipo_imput_presu','$ref_imput_presu','$sfecha',$monto,0,$monto_credito,0)";
      $resultado=pg_exec($conn,$sql);   $error=pg_errormessage($conn);  echo $sql,"<br>"; $error="ERROR GRABANDO: ".substr($error, 0, 61);   if (!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php }
    }
  }
}
pg_close($conn);
