<?php include ("../class/conect.php");  include ("../class/funciones.php"); ("Ver_dispon.php"); include ("../class/configura.inc");
...
if ($error==0){ $sfecha=formato_aaaammdd($fecha_ajuste);  $rfecha=$sfecha;
  ...
  if($error==0){    $sSQL="Select * from pre005 WHERE tipo_ajuste='$tipo_ajuste'";    $resultado=pg_exec($conn,$sSQL);
    ...
    if ($filas==0){$error=1;?><script language="JavaScript">muestra('DOCUMENTO DE AJUSTE NO EXISTE');</script><?php }
     else{if(($tipo_ajuste=="0000")or(substr($tipo_ajuste,0,1)=='A')){$error=1;?><script language="JavaScript">  muestra('DOCUMENTO DE AJUSTE NO VALIDO');</script><?php }
      else{ $registro=pg_fetch_array($resultado);
        if($refierea==$registro["refierea"]){$error;} else{ echo $refiere,' ',$registro["refierea"]; $error=1;?><script language="JavaScript">muestra('DOCUMENTO DE AJUSTE NO VALIDO');</script><?php }
      }}
    $sSQL="Select * from pre005 WHERE refierea='COMPROMISO'";    $resultado=pg_exec($conn,$sSQL);    $filas=pg_numrows($resultado);
    if ($filas==0){$error=1;?><script language="JavaScript">muestra('DOCUMENTO DE AJUSTE A COMPROMISO NO EXISTE');</script><?php }
    ...
    if ($filas==0){$error=1;?><script language="JavaScript">muestra('DOCUMENTO DE AJUSTE A CAUSADO NO EXISTE');</script><?php }
    ...
    if ($filas==0){$error=1;?><script language="JavaScript">muestra('DOCUMENTO DE AJUSTE A PAGO NO EXISTE');</script><?php }
  ...
  if($error==0){ $anulado="N";
    if($refierea=="COMPROMISO"){
      ...
      if ($filas==0){$error=1;?><script language="JavaScript">muestra('REFERENCIA DE COMPROMISO NO EXISTE');</script><?php }
         ...
         if($anulado=='S'){$error=1;?><script language="JavaScript">muestra('COMPROMISO ESTA ANULADO');</script><?php }
       }
    }
    if($refierea=="CAUSADO"){$sSQL="Select * from pre007 WHERE referencia_caus='$referencia_caus' and tipo_causado='$tipo_causado' and tipo_compromiso='$tipo_compromiso' and referencia_comp='$referencia_comp' and tipo_causado<>'0000'";
      ...
      if ($filas==0){$error=1;?><script language="JavaScript">muestra('REFERENCIA DE CAUSADO NO EXISTE');</script><?php }
           ...
           if($anulado=='S'){$error=1;?><script language="JavaScript">muestra('CAUSADO ESTA ANULADO');</script><?php }
         }
    }
    if($refierea=="PAGO"){ $sSQL="Select * from pre008 WHERE tipo_pago='$tipo_pago' and referencia_pago='$referencia_pago' and referencia_caus='$referencia_caus' and tipo_causado='$tipo_causado' and tipo_compromiso='$tipo_compromiso' and referencia_comp='$referencia_comp' and tipo_pago<>'0000'";
      $resultado=pg_exec($conn,$sSQL);$filas=pg_numrows($resultado);
      if ($filas==0){$error=1;?><script language="JavaScript">muestra('REFERENCIA DE PAGO NO EXISTE');</script><?php }
         else{$registro=pg_fetch_array($resultado); $rfecha=$registro["fecha_pago"]; $anulado=$registro["anulado"];
           if($anulado=='S'){$error=1;?><script language="JavaScript">muestra('PAGO ESTA ANULADO');</script><?php }
         }
    }
  }
  if($error==0){
    ...
    $resultado=pg_exec($conn,$sSQL);$filas=pg_numrows($resultado);
    if ($filas>0){$error=1;?><script language="JavaScript">muestra('REFERENCIA DE AJUSTE YA EXISTE');</script><?php }
  }
  if($error==0){
    $fecha_d=formato_ddmmaaaa($Fec_Ini_Ejer);    $fecha_h=formato_ddmmaaaa($Fec_Fin_Ejer);
    if (($sfecha>$Fec_Fin_Ejer)or($sfecha<$Fec_Ini_Ejer)or($sfecha<$rfecha)){$error=1;?><script language="JavaScript">muestra('FECHA DE AJUSTE INVALIDA');</script><?php }
  }
  if($error==0){$nmes=substr($fecha_ajuste,3, 2);
    if ($SIA_Periodo>$nmes){echo "Ultimo Periodo: ".$SIA_Periodo." Periodo del Ajuste: ".$nmes; $error=1;?><script language="JavaScript">muestra('FECHA DE AJUSTE MENOR A ULTIMO PERIODO CERRADO');</script><?php }
  }
    ...
    while(($registro=pg_fetch_array($res))and($error==0)){
      ...
      if($refierea=="COMPROMISO"){
        ...
        if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('NO EXISTE EN EL COMPROMISO EL CODIGO PRESUPUESTARIO:<?php  echo $registro["cod_presup"]; ?> FUENTE:<?php  echo $registro["fuente_financ"]; ?> REF.CREDITO:<?php  echo $registro["ref_imput_presu"]; ?>');</script><?php }
           ...
           if(($monto_c>$compromiso)and($balance>0.009)){$error=1; ?> <script language="JavaScript"> muestra('MONTO A PAGAR MAYOR AL MONTO DEL CÓDIGO DEL COMPROMISO POR AJUSTAR');</script><?php }
         }
      }
      if($refierea=="CAUSADO"){
        ...
        if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('NO EXISTE EN EL CAUSADO EL CODIGO PRESUPUESTARIO:<?php  echo $registro["cod_presup"]; ?> FUENTE:<?php  echo $registro["fuente_financ"]; ?> REF.CREDITO:<?php  echo $registro["ref_imput_presu"]; ?>');</script><?php }
           ...
           if(($monto_c>$compromiso)and($balance>0.009)){$error=1; ?> <script language="JavaScript"> muestra('MONTO A PAGAR MAYOR AL MONTO DEL CÓDIGO DEL CAUSADO POR AJUSTAR');</script><?php }
         }
      }
      if($refierea=="PAGO"){
        ...
        if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('NO EXISTE EN EL CAUSADO EL CODIGO PRESUPUESTARIO:<?php  echo $registro["cod_presup"]; ?> FUENTE:<?php  echo $registro["fuente_financ"]; ?> REF.CREDITO:<?php  echo $registro["ref_imput_presu"]; ?>');</script><?php }
           ...
           if(($monto_c>$compromiso)and($balance>0.009)){$error=1; ?> <script language="JavaScript"> muestra('MONTO A PAGAR MAYOR AL MONTO DEL CÓDIGO DEL PAGO POR AJUSTAR');</script><?php }
         }
      }
    }
    if($total==0){$error=1;?><script language="JavaScript">muestra('MONTO TOTAL DEL AJUSTE INVALIDO');</script><?php }
  }
...
if ($error==0){?><script language="JavaScript">document.location ='Act_ajustes.php?Gcriterio=<?php  echo $tipo_ajuste.$referencia_ajuste.$tipo_pago.$referencia_pago.$tipo_causado.$referencia_caus.$tipo_compromiso.$referencia_comp; ?>';</script> <?php }
