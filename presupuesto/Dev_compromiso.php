...
if (pg_last_error($conn)) { ?> <script language="JavaScript"> muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');  </script> <?php }
  ...
  if ($filas==0){$error=1;?><script language="JavaScript">muestra('REFERENCIA DE COMPROMISO NO EXISTE');</script><?php }
     ...
     if($error==0){$sfecha=$fecha_compromiso; if (checkData($fecha_anu)=='1'){$error=0;$afecha=formato_aaaammdd($fecha_anu);}
       else{$error=1; ?> <script language="JavaScript">muestra('FECHA DEVOLUCION NO ES VALIDA');</script><?php } }
     if($error==0){if ($afecha<$sfecha){$error=1; ?> <script language="JavaScript">muestra('FECHA DEVOLUCION NO PUEDE SER MENOR A FECHA DEL COMPROMISO');</script><?php } }
	 ...
	 if($error==0){$resultado=pg_exec($conn,"SELECT DEVUELVE_PRE006('$referencia_comp','$tipo_compromiso','$cod_comp','$sfecha','$afecha','$usuario_sia','$descrip_dev')");
       ...
       if (!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php }
        else{?><script language="JavaScript">muestra('DEVOLVIO EXITOSAMENTE');</script><?php 
