...
if (pg_last_error($conn)){$error=1;?> <script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script> <?php } else { $Nom_Emp=busca_conf(); if($utf_rpt=="SI"){if($php_os=="WINNT"){$php_os="LINUX";}else{$php_os="WINNT";}} }
  ...
  if(($Cod_Emp=="86")and($tipo_rep=="PDF3")){ $tipo_rep="PDF4"; $tipo_rep="PDF5";
    $res=pg_exec($conn,"SELECT ACTUALIZA_CONSOLIDADO('B','$cod_mov','O','$sfechad','$sfechah','N','$buscaf_ant','$Rif_Emp')");  $error=pg_errormessage($conn); $error=substr($error,0,91);if (!$res){ ?> <script language="JavaScript">  muestra('<?php  echo $error; ?>'); </script> <?php }
          ...
          if($imp_total_g=="S"){
		   ...
		   ?>	
			...
			<tr>
			  ...
			  <td width="80" align="left"></td>
			  <td width="400" align="left"></td>
			  <td width="200" align="right"><?php  echo "Totale General : "; ?></td>
