...
if ($error==0){
  ...
  if($error==0){$sSQL="Select * from pre009 WHERE referencia_modif='$referencia_modif' and tipo_modif='$tipo_m'";   $resultado=pg_exec($conn,$sSQL); $filas=pg_numrows($resultado);
    if ($filas==0){$error=1; echo $sSQL; ?><script language="JavaScript">muestra('REFERENCIA DE LA MODIFICACION NO EXISTE');</script><?php }
  }
  ...
  if($error==0){$fecha_d=formato_ddmmaaaa($Fec_Ini_Ejer); $fecha_h=formato_ddmmaaaa($Fec_Fin_Ejer);
    ...
    if (($mfecha>$Fec_Fin_Ejer)or($sfecha<$Fec_Ini_Ejer)){$error=1;?><script language="JavaScript">muestra('FECHA DE MODIFICACION INVALIDA');</script><?php }
  }
  if($error==0){$resultado=pg_exec($conn,"SELECT APRUEBA_PRE009('$referencia_modif','$tipo_m','$sfecha','$mfecha','$nro_documento','$dfecha','$aprobada_por','$tipo_dife')");
    $error=pg_errormessage($conn); $error=substr($error, 0, 91);   if (!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php }
     else{?><script language="JavaScript">muestra('APROBO EXITOSAMENTE');</script><?php 
...
pg_close($conn); error_reporting(E_ALL ^ E_WARNING);?> <script language="JavaScript">history.back();</script>