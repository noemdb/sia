<?php include ("../class/conect.php");  include ("../class/funciones.php"); include ("../class/configura.inc"); include ("Ver_dispon.php");
...
if (pg_last_error($conn)){ ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
  ...
  if ($filas>0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO PRESUPUESTARIO YA EXISTE EN EL COMPROMISO');</script><?php }
   else{
    ...
    if(($error==0)and($tipo_imput_presu=="C")){
      if($monto_credito>$monto_c){ $error=1; ?> <script language="JavaScript"> muestra('MONTO CREDITO INVALIDO');</script><?php }
    }
    ...
    if(($error==0)and($Cod_Emp=="70")){ if(substr($cod_presup,0,$lc)==$cod_cat){  $error=0; } else{$error=1; ?> <script language="JavaScript"> muestra('CODIGO INVALIDO NO ES LA MISMA CATEGORIA');</script><?php }
    }
