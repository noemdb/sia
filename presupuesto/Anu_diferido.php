...
if (pg_last_error($conn)) { ?> <script language="JavaScript"> muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');  </script> <?php }
  ...
  if ($filas==0){$error=1;?><script language="JavaScript">muestra('REFERENCIA DE MOVIMIENTO DIFERIDO NO EXISTE');</script><?php }
     ...
     if($anulado=='S'){$error=1;?><script language="JavaScript">muestra('DIFERIDO YA ESTA ANULADO');</script><?php }
     if($error==0){$sfecha=$fecha_diferido; if (checkData($fecha_anu)=='1'){$error=0;$afecha=formato_aaaammdd($fecha_anu);}
       else{$error=1; ?> <script language="JavaScript">muestra('FECHA ANULACION NO ES VALIDA');</script><?php }
     }
     if($error==0){if ($afecha<$sfecha){$error=1; ?> <script language="JavaScript">muestra('FECHA ANULACION NO PUEDE SER MENOR A FECHA DEL DIFERIDO');</script><?php } }
	 ...
	 if($error==0){  $sql="Select * from pre006 WHERE diferido='S' and ref_aep='$referencia_dife' and tipo_documento='$tipo_diferido'"; $resultado=pg_exec($conn,$sql);   $filas=pg_numrows($resultado);
       if ($filas>=1){ $registro=pg_fetch_array($resultado); $ref_comp=$registro["referencia_comp"]; $tipo_comp=$registro["tipo_compromiso"]; $error=1;?><script language="JavaScript">muestra('MOVIMIENTO DIFERIDO TIENE COMPROMISO, REFERENCIA COMPROMISO: <?php  echo $ref_comp.' '.$tipo_comp; ?> ');</script><?php }
     }  
     if($error==0){$resultado=pg_exec($conn,"SELECT ANULA_pre023('$referencia_dife','$tipo_diferido','$afecha','$descrip_anu','$usuario_sia','$minf_usuario')");  $merror=pg_errormessage($conn);  $merror=substr($merror,0,91);
       if (!$resultado){$error=1;  ?><script language="JavaScript">muestra('<?php  echo $merror; ?>');</script><?php }   else{?><script language="JavaScript">muestra('ANULO EXITOSAMENTE');</script><?php 
           $desc_doc="MOV.DIFERIDO: DOCUMENTO:".$tipo_diferido.", REFERENCIA:".$referencia_dife.", DESCRIPCION:".$adescripcion;
           $resultado=pg_exec($conn,"SELECT INCLUYE_SIA004('05','$usuario_sia','$usuario_sia','$equipo','Anulo','$sfecha','$desc_doc')");  $merror=pg_errormessage($conn);  $merror=substr($merror,0,91); if (!$resultado){$error=1;  ?><script language="JavaScript">muestra('<?php echo $merror;?>');</script><?php } }
     }
  }
}
pg_close($conn); error_reporting(E_ALL ^ E_WARNING);
if ($error==0){?><script language="JavaScript">cerrar_ventana();</script> <?php } else {?>  <script language="JavaScript">history.back();</script> <?php } ?>