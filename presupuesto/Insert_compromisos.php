...
/*  
  if($error==0){
    ...
    if(($tipo_compromiso==$tipo_doc_oc)or($tipo_compromiso==$tipo_doc_os)){ $error=1;?><script language="JavaScript">  muestra('DOCUMENTO DE COMPROMISO NO VALIDO');</script><?php }
  }
*/  
  if($error==0){$sSQL="Select * from pre002 WHERE tipo_compromiso='$tipo_compromiso'";  $resultado=pg_exec($conn,$sSQL);  $filas=pg_numrows($resultado);
    if ($filas==0){$error=1;?><script language="JavaScript">muestra('DOCUMENTO DE COMPROMISO NO EXISTE');</script><?php }
     else{if(($tipo_compromiso=="0000")or(substr($tipo_compromiso,0,1)=='A')){$error=1;?><script language="JavaScript">  muestra('DOCUMENTO DE COMPROMISO NO VALIDO');</script><?php } }
  }
  if(($error==0)and($nro_aut=="N")) {
    ...
    if ($filas>0){$error=1;?><script language="JavaScript">muestra('REFERENCIA DE COMPROMISO YA EXISTE');</script><?php }
	else{
      ...
      if($Cod_Emp<>"38"){	
        ...
        if ($filas>0){ $reg=pg_fetch_array($resultado); $ult_fecha=$reg["ult_fecha"]; 
	        if($sfecha<$ult_fecha){$error=1; $fecha_up=formato_ddmmaaaa($ult_fecha); echo "Ultima Fecha: ".$fecha_up; ?><script language="JavaScript">muestra('FECHA DE COMPROMISO MENOR AL ANTERIOR');</script><?php }
	    }	 
      }	  
	}  
  }
  if($error==0){ $fecha_d=formato_ddmmaaaa($Fec_Ini_Ejer); $fecha_h=formato_ddmmaaaa($Fec_Fin_Ejer);
    if (($sfecha>$Fec_Fin_Ejer)or($sfecha<$Fec_Ini_Ejer)){$error=1;?><script language="JavaScript">muestra('FECHA DE COMPROMISO INVALIDA');</script><?php }
  }  
  if($error==0){$nmes=substr($fecha_compromiso,3, 2);
    if ($SIA_Periodo>$nmes){echo "Ultimo Periodo: ".$SIA_Periodo." Periodo del Compromiso: ".$nmes; $error=1;?><script language="JavaScript">muestra('FECHA DE COMPROMISO MENOR A ULTIMO PERIODO CERRADO');</script><?php }
  }  
  if($error==0){ $sSQL="Select cod_presup_cat,cod_fuente_cat,denominacion_cat from pre019 WHERE cod_presup_cat='$unidad_sol'";
    $resultado=pg_exec($conn,$sSQL);  $filas=pg_numrows($resultado);
    if ($filas==0){$error=1;?><script language="JavaScript">muestra('CODIGO DE CATEGORIA PRESUPUESTARIA NO EXISTE');</script><?php }
  }
  if($error==0){ $sSQL="SELECT tipo_comp,cod_contable from pre016 WHERE tipo_comp='$cod_tipo_comp'"; $resultado=pg_exec($conn,$sSQL); $filas=pg_numrows($resultado);
    if ($filas==0){$error=1;?><script language="JavaScript">muestra('CODIGO TIPO DE COMPROMISO NO EXISTE');</script><?php }
  ...
  if(($error==0)and($tiene_anticipo=="S")){ $sSQL="Select * from con001 WHERE codigo_cuenta='$cod_con_anticipo'"; $resultado=pg_query($sSQL); $filas=pg_num_rows($resultado);
      if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO CONTABLE DE ANTICIPO NO EXISTE');</script><?php }
      else{ $registro=pg_fetch_array($resultado); if($registro["cargable"]=="N"){$error=1; ?> <script language="JavaScript"> muestra('CODIGO CONTABLE DE ANTICIPO NO ES CARGABLE');</script><?php }
      }
    }else{$cod_con_anticipo="";$tasa_anticipo=0;}
  if (strlen($referencia_comp)==8){$error=$error;} else{$error=1; ?> <script language="JavaScript">muestra('REFERENCIA DE COMPROMISO INVALIDA');</script><?php }
  ...
  if($error==0){ $sql="Select * from CODIGOS_PRE026 where codigo_mov='$codigo_mov' order by cod_presup"; $res=pg_query($sql);  $total=0; $sfecha=formato_aaaammdd($fecha_compromiso);
    while(($registro=pg_fetch_array($res))and($error==0)){$total=$total+$registro["monto"];$cod_presup=$registro["cod_presup"];$fuente_financ=$registro["fuente_financ"];
      ...
      if(strlen($cod_presup)==strlen($formato_presup)){
       ...
       else{$error=1;?><script language="JavaScript">muestra('ERROR EN CODIGO PRESUPUESTARIO:<?php  echo $registro["cod_presup"]; ?> FUENTE:<?php  echo $registro["fuente_financ"]; ?> REF.CREDITO:<?php  echo $registro["ref_imput_presu"]; ?>');</script><?php }
     }
     else{$error=1; ?> <script language="JavaScript"> muestra('LONGITUD DE CODIGO PRESUPUESTARIO:<?php  echo $registro["cod_presup"]; ?> FUENTE:<?php  echo $registro["fuente_financ"]; ?> INVALIDA');</script><?php }
	  ...
	  if(($error==0)and($tipo_imput_presu=="C")){
        ...
        if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO PRESUPUESTARIO:<?php  echo $registro["cod_presup"]; ?> FUENTE:<?php  echo $registro["fuente_financ"]; ?> REF.CREDITO:<?php  echo $registro["ref_imput_presu"]; ?> \ncon NO EXISTE EN LA EJECUCION DEL CREDITO ADICIONAL');</script><?php }
         else{$reg=pg_fetch_array($resusltado);
           if($reg["disponible"]<$monto_credito) {$error=1; $dispon=$registro["disponible"]; $dispon=formato_monto($dispon); ?> <script language="JavaScript"> muestra('CODIGO PRESUPUESTARIO:<?php  echo $registro["cod_presup"]; ?> FUENTE:<?php  echo $registro["fuente_financ"]; ?> REF.CREDITO:<?php  echo $registro["ref_imput_presu"]; ?> \ncon Monto Mayor que Disponibilidad del Credito Adicional, Disponibilidad: <?php  echo $dispon; ?> ');</script><?php }
        }
      }
    ...
    if($total==0){$error=1;?><script language="JavaScript">muestra('MONTO TOTAL DEL COMPROMISO INVALIDO');</script><?php }
  }
  if(($error==0)and($g_comprobante=="S")){ $monto_c=cambia_coma_numero($total); $status="D";  
    ...
    if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO CONTABLE DE GASTO NO EXISTE');</script><?php }
    else{$registro=pg_fetch_array($resultado);if ($registro["cargable"]=="N"){$error=1; ?> <script language="JavaScript"> muestra('CODIGO CONTABLE DE GASTO NO ES CARGABLE');</script><?php } }
  ...
  if($error==0){$sfecha=formato_aaaammdd($fecha_compromiso); $fechav=formato_aaaammdd($fecha_vencim);     //echo $g_comprobante;	 
     ...
     if (!$resultado){?><script language="JavaScript"> muestra('<?php  echo $error; ?>');</script><?php  $error=1;} else{  $error=0;?><script language="JavaScript">  muestra('INCLUYO EXITOSAMENTE');</script><?php 
...
/*  */
if ($error==0){?><script language="JavaScript">document.location ='Act_compromisos.php?Gcriterio=<?php  echo $tipo_compromiso.$referencia_comp.$cod_comp; ?>';</script> <?php }
