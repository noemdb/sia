...
if (pg_last_error($conn)){ ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
      ...
      $resultado=pg_query($sSQL);  $filas=pg_num_rows($resultado); if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DE CUENTA NO EXISTE');</script><?php }
...
pg_close($conn);
if ($error==0){?><script language="JavaScript">document.location ='Det_inc_comp_caus.php?codigo_mov=<?php echo $codigo_mov?>';</script> <?php } else {?>  <script language="JavaScript">history.back();</script> <?php } ?>
