...
if (pg_last_error($conn)){$error=1; ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
  ...
  if($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DE SERVICIO NO EXISTE');</script><?php }
    else{  $registro=pg_fetch_array($resultado,0);  $prev_aprobado=$registro["aprobado"];
     if($prev_aprobado=="S"){$error=1; ?> <script language="JavaScript"> muestra('SERVICIO YA APROBADO');</script><?php } 
	 else{$part="000001";
  ...
  if($error==0){if(strlen($cod_aux2)==10){$error=0;} else{$error=1; ?> <script language="JavaScript"> muestra('LONGITUD CODIGO DE SERVICIO INVALIDA');</script><?php } }
  if($error==0){if(strlen($denominacion)==0){$error=1; ?> <script language="JavaScript"> muestra('DESCRIPCIÓN INVALIDA');</script><?php } }
  if($error==0){$sSQL="Select cod_par_ramo_ser from COMP026 WHERE cod_ramo_ser='$ramo'"; $resultado=pg_query($sSQL); $filas=pg_num_rows($resultado); if($filas==0){$error=1;?><script language="JavaScript">muestra('CODIGO RAMO DE SERVICIO NO EXISTE');</script><?php }else{$registro=pg_fetch_array($resultado); $cod_par_ramo_ser=$registro["cod_par_ramo_ser"];}}
  if($error==0){if($ramo==substr($cod_aux2,0,3)){$error=0;} else{$error=1; ?> <script language="JavaScript"> muestra('CODIGO DE SERVICIO INVALIDO');</script><?php } }
  if($error==0){$l=strlen($cod_par_ramo_ser); if($cod_par_ramo_ser==substr($partida,0,$l)){$error=0;} else{$error=1; ?> <script language="JavaScript"> muestra('CODIGO DE PARTIDA INVALIDO');</script><?php } }
  if($error==0){$sSQL="Select cod_servicio,des_servicio from COMP027 WHERE des_servicio='$denominacion'"; $resultado=pg_query($sSQL); $filas=pg_num_rows($resultado); if($filas>=1){$error=1; ?> <script language="JavaScript"> muestra('SERVICIO YA EXISTE CON LA MISMA DESCRIPCION');</script><?php } }
