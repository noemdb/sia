...
if (pg_last_error($conn)) { ?> <script language="JavaScript">   muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');  </script> <?php }
  ...
  if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CÓDIGO TIPO DE COMPROMISO NO EXISTE'); </script> <?php }
   else{  $error=1;  $sSQL="SELECT ACTUALIZA_PRE016(3,'$tipo_comp','$des_tipo_comp','','$cod_part_iva','$func_inv_tpcomp','$c_imp_unico')";
     $resultado=pg_exec($conn,$sSQL);     $error=pg_errormessage($conn);  $error=substr($error, 0, 61);   if (!$resultado){ ?> <script language="JavaScript"> muestra('<?php  echo $error; ?>'); </script> <?php }      else{?><script language="JavaScript">muestra('ELIMINO EXITOSAMENTE');</script><?php  $error=0; }
  }
}
pg_close($conn);
