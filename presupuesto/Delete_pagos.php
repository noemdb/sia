...
if (pg_last_error($conn)) { ?> <script language="JavaScript"> muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');  </script> <?php }
    ...
    if ($filas==0){$error=1;?><script language="JavaScript">muestra('REFERENCIA DE PAGO NO EXISTE');</script><?php }
        ...
        if($genera_comprobante=="S"){ if ($SIA_Periodo<$SIA_Periodoc){$SIA_Periodo=$SIA_Periodoc;}
		  if(($error==0)and($valida_c=="S")){ $tipo_comp='G'.$tipo_pago;
            ...
            if ($filas>0){ $reg=pg_fetch_array($resultado); $mstatusc=$reg["status"]; if($mstatusc=="A"){$error=1;?><script language="JavaScript">muestra('Comprobante Contable del Pago esta Actualizado, debe cambiar a Diferido');</script><?php } }
	      }
		}
	  }
  }	  
  ...
  if($error==0){$nmes=substr($fecha_pago,5, 2);if($periodom<$SIA_Periodo){$periodom=$SIA_Periodo;}
   if($periodom>$nmes){echo $periodom.' '.$nmes.' '.$fecha_pago;$error=1;?><script language="JavaScript">muestra('FECHA DE MES MENOR A ULTIMO PERIODO CERRADO');</script><?php } }
