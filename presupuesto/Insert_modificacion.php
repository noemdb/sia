<?php include ("../class/conect.php");  include ("../class/funciones.php");include ("Ver_dispon.php"); include ("../class/configura.inc");error_reporting(E_ALL);
...
if (checkData($fecha_registro)=='1'){$error=0;}else{$error=1; ?> <script language="JavaScript">muestra('FECHA REGISTRO NO ES VALIDA');</script><?php }
if (checkData($fecha_modif)=='1'){$error=0;}else{$error=1; ?> <script language="JavaScript">muestra('FECHA MODIFICACION NO ES VALIDA');</script><?php }
if (checkData($fecha_documento)=='1'){$error=0;}else{$error=1; ?> <script language="JavaScript">muestra('FECHA DOCUMENTO NO ES VALIDA');</script><?php }
if ($error==0){
  ...
  if($error==0){  $sSQL="Select * from pre009 WHERE referencia_modif='$referencia_modif' and tipo_modif='$tipo_m'";
    ...
    if ($filas>0){echo $tipo_modif." ".$sSQL,"<br>"; $error=1;?><script language="JavaScript">muestra('REFERENCIA DE LA MODIFICACION YA EXISTE');</script><?php }
  }
  ...
  if($error==0){ $fecha_d=formato_ddmmaaaa($Fec_Ini_Ejer);  $fecha_h=formato_ddmmaaaa($Fec_Fin_Ejer);
    if (($sfecha>$Fec_Fin_Ejer)or($sfecha<$Fec_Ini_Ejer)){$error=1;?><script language="JavaScript">muestra('FECHA DE REGISTRO INVALIDA');</script><?php }
    if (($mfecha>$Fec_Fin_Ejer)or($sfecha<$Fec_Ini_Ejer)){ echo $_POST["txtfecha"]." ".$fecha_aut." ".$mfecha." ".$Fec_Fin_Ejer." ".$Fec_Ini_Ejer,"<br>"; $error=1;?><script language="JavaScript">muestra('FECHA DE MODIFICACION INVALIDA');</script><?php }
  }  
  if($error==0){$nmes=substr($fecha_registro,3, 2);
    if ($SIA_Periodo>$nmes){echo "Ultimo Periodo: ".$SIA_Periodo." Periodo de la Modificacion: ".$nmes; $error=1;?><script language="JavaScript">muestra('FECHA DE MODIFICACION MENOR A ULTIMO PERIODO CERRADO');</script><?php }
  }  
    ...
    while($registro=pg_fetch_array($res)){ $total=$total+$registro["monto"]; $monto_c=$registro["monto"];  $operacion=$registro["operacion"];
      if($operacion=="-"){$totalr=$totalr+$registro["monto"];
        if (verifica_disponibilidad($conn,$registro["cod_presup"],$registro["fuente_financ"],$fecha_registro,$monto_c)==0){$error=0;}
           else{$error=1;?><script language="JavaScript">muestra('ERRROR EN EL CODIGO PRESUPUESTARIO:<?php  echo $registro["cod_presup"]; ?> FUENTE:<?php  echo $registro["fuente_financ"]; ?>');</script><?php }
         ...
         if ($filas==0){$error=1;  echo "Codigo:".$mcodigo." Fuente:".$mfuente,"<br>"; ?> <script language="JavaScript">muestra('CODIGO PRESUPUESTARIO NO EXISTE');</script> <?php } }
    }
    if($total==0){$error=1;?><script language="JavaScript">muestra('MONTO DE LA MODIFICACION INVALIDO');</script><?php }
    if(($tipo_m=="5")or($tipo_m=="2")){ if($totalr>$totals){$balance=$totalr-$totals;}else{$balance=$totals-$totalr;}
	  //echo $totalr." ".$totals." ".$balance;
      if($balance>0.001){echo $totalr." ".$totals." ".$balance; $error=1;?><script language="JavaScript">muestra('MONTO DEL TRASPASO INVALIDO');</script><?php }
    }
  }
...
if ($error==0){?><script language="JavaScript">document.location ='Act_modificaciones.php?Gcriterio=<?php  echo $referencia_modif.$tipo_m; ?>';</script> <?php }
