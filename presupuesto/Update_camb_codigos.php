...
if (pg_last_error($conn)) { ?> <script language="JavaScript">   muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');  </script> <?php }
  ...
  }else{$error=1; ?> <script language="JavaScript"> muestra('LONGITUD CODIGO PRESUPUESTARIO INVALIDA');</script><?php }
  if($error==0){ $sSQL="Select * from PRE095 WHERE cod_fuente_financ='$cod_fuenten'";  $resultado=pg_query($sSQL); $filas=pg_num_rows($resultado);      if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DE FUENTE NO EXISTE');</script><?php } }
  if($error==0){ 
    ...
    If ($filas>=1){$error=1; ?> <script language="JavaScript"> muestra('CODIGO PRESUPUESTARIO NUEVO YA EXISTE');  </script> <?php }
  }
  if($error==0){ $fecha=asigna_fecha_hoy(); if($fecha==""){$sfecha="2007-01-01";}else{$sfecha=formato_aaaammdd($fecha);}
     ...
     $error=pg_errormessage($conn);$error=substr($error, 0, 91); if (!$resultado){?> <script language="JavaScript"> muestra('<?php  echo $error; ?>'); </script> <?php }
      else{ ?> <script language="JavaScript"> muestra('MODIFICO EXITOSAMENTE'); </script><?php  $error=0;
        ...
        $resultado=pg_exec($conn,"SELECT INCLUYE_SIA004('05','$usuario_sia','$usuario_sia','$equipo','Modifico','$sfecha','$desc_doc')");  $error=pg_errormessage($conn); $error=substr($error, 0, 91);  if (!$resultado){ ?> <script language="JavaScript">  muestra('<?php  echo $error; ?>'); </script> <?php } }
...
/* */
if ($error==0){?><script language="JavaScript">LlamarURL('<?php echo $url;?>'); </script> <?php } else { ?> <script language="JavaScript">history.back();</script>  <?php } 
?>
