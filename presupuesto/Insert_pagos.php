...
else{$error=1; ?> <script language="JavaScript">muestra('FECHA NO ES VALIDA');</script><?php }
if ($error==0){  $sfecha=formato_aaaammdd($fecha_pago);  $rfecha=$sfecha;
  ...
  if($error==0){$sSQL="Select * from pre004 WHERE tipo_pago='$tipo_pago'";  $resultado=pg_exec($conn,$sSQL); $filas=pg_numrows($resultado);
    if ($filas==0){$error=1;?><script language="JavaScript">muestra('DOCUMENTO DE PAGO NO EXISTE');</script><?php }
     else{if(($tipo_pago=="0000")or(substr($tipo_pago,0,1)=='A')){$error=1;?><script language="JavaScript">  muestra('DOCUMENTO DE PAGO NO VALIDO');</script><?php }
      else{ $registro=pg_fetch_array($resultado);
        if($refierea==$registro["refierea"]){$error;} else{$error=1;?><script language="JavaScript">muestra('DOCUMENTO DE PAGO NO VALIDO');</script><?php }
      }}
  }
  if($error==0){
    ...
    if($refierea=="CAUSADO"){
      ...
      $resultado=pg_exec($conn,$sSQL); $filas=pg_numrows($resultado);
      if ($filas==0){$error=1;?><script language="JavaScript">muestra('REFERENCIA DE CAUSADO NO EXISTE');</script><?php }
       else{$registro=pg_fetch_array($resultado);
  ...
  if($error==0){
    ...
    $resultado=pg_exec($conn,$sSQL);  $filas=pg_numrows($resultado);
    if ($filas>0){$error=1;?><script language="JavaScript">muestra('REFERENCIA DE PAGO YA EXISTE');</script><?php }
  }
  if($error==0){
    $fecha_d=formato_ddmmaaaa($Fec_Ini_Ejer);$fecha_h=formato_ddmmaaaa($Fec_Fin_Ejer);
    if (($sfecha>$Fec_Fin_Ejer)or($sfecha<$Fec_Ini_Ejer)or($sfecha<$rfecha)){$error=1;?><script language="JavaScript">muestra('FECHA DE PAGO INVALIDA');</script><?php }
  }
  if($error==0){
    ...
    while(($registro=pg_fetch_array($res))and($error==0)){
      ...
      if($refierea=="NINGUNO"){
        if (verifica_disponibilidad($conn,$registro["cod_presup"],$registro["fuente_financ"],$fecha_pago,$monto_c)==0){$error=0;}
         else{$error=1;?><script language="JavaScript">muestra('ERROR EN EL CODIGO PRESUPUESTARIO:<?php  echo $registro["cod_presup"]; ?> FUENTE:<?php  echo $registro["fuente_financ"]; ?> REF.CREDITO:<?php  echo $registro["ref_imput_presu"]; ?>');</script><?php }
        if(($error==0)and($tipo_imput_presu=="C")){
          ...
          if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO PRESUPUESTARIO:<?php  echo $registro["cod_presup"]; ?> FUENTE:<?php  echo $registro["fuente_financ"]; ?> REF.CREDITO:<?php  echo $registro["ref_imput_presu"]; ?> \ncon NO EXISTE EN LA EJECUCIÓN DEL CREDITO ADICIONAL');</script><?php }
           else{  $reg=pg_fetch_array($res);
             if($registro["disponible"]<$monto_credito) {$error=1; $dispon=$registro["disponible"]; $dispon=formato_monto($dispon); ?> <script language="JavaScript"> muestra('CODIGO PRESUPUESTARIO:<?php  echo $registro["cod_presup"]; ?> FUENTE:<?php  echo $registro["fuente_financ"]; ?> REF.CREDITO:<?php  echo $registro["ref_imput_presu"]; ?> \ncon Monto Mayor que Disponibilidad del Crédito Adicional, Disponibilidad: <?php  echo $dispon; ?> ');</script><?php }
          }
        }
      }
      if($refierea=="COMPROMISO"){
        ...
        if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('NO EXISTE EN EL COMPROMISO EL CODIGO PRESUPUESTARIO:<?php  echo $registro["cod_presup"]; ?> FUENTE:<?php  echo $registro["fuente_financ"]; ?> REF.CREDITO:<?php  echo $registro["ref_imput_presu"]; ?>');</script><?php }
           ...
           if(($monto_c>$compromiso)and($balance>0.001)){$error=1; echo $monto_c.' '.$compromiso.' '.formato_monto($balance),"<br>"; ?> <script language="JavaScript"> muestra('MONTO A PAGAR MAYOR AL MONTO DEL CODIGO DEL COMPROMISO POR PAGAR');</script><?php }
         }
      }
      if($refierea=="CAUSADO"){
        ...
        $resultado=pg_query($sSQL); $filas=pg_num_rows($resultado);
        if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('NO EXISTE EN EL CAUSADO EL CODIGO PRESUPUESTARIO:<?php  echo $registro["cod_presup"]; ?> FUENTE:<?php  echo $registro["fuente_financ"]; ?> REF.CREDITO:<?php  echo $registro["ref_imput_presu"]; ?>');</script><?php }
         else{ $registro=pg_fetch_array($resultado);
           $compromiso=$registro["monto"]-$registro["pagado"]-$registro["ajustado"]; $balance=$monto_c-$compromiso;
           if(($monto_c>$compromiso)and($balance>0.001)){$error=1; echo $monto_c.' '.$compromiso.' '.formato_monto($balance),"<br>"; ?> <script language="JavaScript"> muestra('MONTO A PAGAR MAYOR AL MONTO DEL CODIGO DEL CAUSADO POR PAGAR');</script><?php }
         }
      }
    }
    if($total==0){$error=1;?><script language="JavaScript">muestra('MONTO TOTAL DEL PAGO INVALIDO');</script><?php }
  }
  if(($error==0)and($genera_comprobante=='S')){ $t_debe=0; $t_haber=0;$balance=0;
         ...
         if (($t_debe==0)or($balance>0.001)){$error=1; echo $t_debe.' '.$t_haber.' '.formato_monto($balance),"<br>";  ?> <script language="JavaScript"> muestra('MONTO DEL COMPROBANTE CONTABLE NO VALIDO');</script><?php }
  ...
  if($error==0){
     ...
     if (!$resultado){?><script language="JavaScript"> muestra('<?php  echo $error; ?>');</script><?php  $error=1;}
       ...
       $error=0;?><script language="JavaScript">  muestra('INCLUYO EXITOSAMENTE');</script><?php  $resultado=pg_exec($conn,"SELECT BORRAR_PRE026('$codigo_mov')");
...
if ($error==0){?><script language="JavaScript">document.location ='Act_pagos.php?Gcriterio=<?php  echo $tipo_pago.$referencia_pago.$tipo_causado.$referencia_caus.$tipo_compromiso.$referencia_comp.$cod_banco; ?>';</script> <?php }
