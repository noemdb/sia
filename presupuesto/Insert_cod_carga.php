<?php include ("../class/conect.php");  include ("../class/funciones.php"); $formato_presup="XX-XX-XX-XXX-XX-XX-XX";
...
if (pg_last_error($conn)){ ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
  ...
  if ($error==0){    $sSQL="Select * from PRE098 WHERE cod_partida='$cod_partida'";
    ...
    if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DE PARTIDA NO EXISTE EN EL CLASIFICADOR');</script><?php }
    ...
    if($error==0){ $sSQL="Select * from pre032 WHERE cod_presup='$cod_presup' and cod_fuente='$cod_fuente'";
      $resultado=pg_exec($conn,$sSQL); $filas=pg_numrows($resultado);  if ($filas>0){$error=1;?> <script language="JavaScript">muestra('CODIGO PRESUPUESTARIO YA CARGADO');</script> <?php }
    }
	 ...
	 */ 
    }else{$error=1; ?> <script language="JavaScript"> muestra('LONGITUD DE CODIGO PRESUPUESTARIO NO VALIDA');</script><?php }
