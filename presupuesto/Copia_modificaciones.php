...
if (pg_last_error($conn)) { ?> <script language="JavaScript"> muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');  </script> <?php }
  ...
  if ($filas==0){$error=1;?><script language="JavaScript">muestra('REFERENCIA DE MODIFICACION NO EXISTE');</script><?php }
	...
	$resultado=pg_exec($conn,"SELECT COPIA_PRE009('$cod_busca','$referencia_modif','$tipo_modif','$sfecha')"); $error=pg_errormessage($conn);$error=substr($error, 0, 61);
     if (!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php }
      else{?><script language="JavaScript">/*muestra('COPIO EXITOSAMENTE');*/ alert('COPIO EXITOSAMENTE');</script><?php }
  }
}
