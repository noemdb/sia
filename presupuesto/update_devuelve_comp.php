<?php include ("../class/conect.php");  include ("../class/funciones.php"); error_reporting(E_ALL); $url="Devol_compromisos.php"; 
...
if (pg_last_error($conn)) { ?> <script language="JavaScript"> muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');  </script> <?php }
  ...
  if ($filas==0){$error=1;?><script language="JavaScript">muestra('REFERENCIA DE COMPROMISO NO EXISTE');</script><?php }
   ...
   if ($error==0){$sql="SELECT referencia_caus,tipo_causado FROM PRE007 WHERE (referencia_comp='$referencia_comp') and (tipo_compromiso='$tipo_compromiso')"; $resultado=pg_exec($conn,$sql);  $filas=pg_numrows($resultado);
       if($filas>0){$error=1;?><script language="JavaScript">muestra('Tiene Compromiso que Refieren al causados , No puede ser Eliminado');</script><?php } } 
   if($error==0){     
     ...
     if (!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php }
      else{?><script language="JavaScript">muestra('ACTUALIZO EXITOSAMENTE');</script><?php }
  } 
