<?php include ("../class/conect.php");  include ("../class/funciones.php"); $cod_presup=$_POST["txtcod_presup"];$cod_fuente=$_POST["txtcod_fuente"];$denominacion=$_POST["txtdenominacion"];$cod_contable=$_POST["txtCodigo_Cuenta"];
...
if (pg_last_error($conn)) { ?> <script language="JavaScript">   muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');  </script> <?php }
  ...
  if($error==0){ $sSQL="Select * from PRE025 WHERE cod_aplicacion='$aplicacion'"; $resultado=pg_exec($conn,$sSQL);$filas=pg_numrows($resultado);
      if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DE APLICACION NO EXISTE');</script><?php }
  }
  if(strlen($cod_presup)==strlen($formato_presup)){  $sSQL="Select * from con001 WHERE codigo_cuenta='$cod_contable'"; $resultado=pg_query($sSQL);$filas=pg_num_rows($resultado);
      ...
      else{ $registro=pg_fetch_array($resultado); if ($registro["cargable"]=="N"){$error=0; ?> <script language="JavaScript"> muestra('CODIGO  CONTABLE DE GASTO NO ES CARGABLE');</script><?php } }
  }else{$cod_contable="";}
  if($error==0){ $montoc = 0;
    ...
    if ($filas>0){$registro=pg_fetch_array($resultado); $montoc = $montoc - $registro["montoc"];}
    If ($montoc>$asignado) {$error=1; ?> <script language="JavaScript"> muestra('MONTO DE ASIGNACION ES MENOR AL COMPROMETIDO');</script><?php }
  }
