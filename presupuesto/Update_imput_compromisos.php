...
if ($error==0){$sfecha=formato_aaaammdd($fecha_compromiso);
  ...
  if($error==0){$sfecha=formato_aaaammdd($fecha_compromiso); 
    ...
    if($imput_presu_cod=="SI"){  
      ...
      if ($filas==0){$error=1;?><script language="JavaScript">muestra('REFERENCIA DE CREDITO ADICIONAL NO EXISTE');</script><?php }
      else{ $registro=pg_fetch_array($resultado);$fecha_modif=$registro["fecha_modif"]; 
	    if($sfecha<$fecha_modif){  $error=1;?><script language="JavaScript">muestra('FECHA DE CREDITO ADICIONAL MAYOR A FECHA DE COMPROMISO');</script><?php }
    ...
    if ($filas==0){$error=1;?><script language="JavaScript">muestra('REFERENCIA DE COMPROMISO NO EXISTE');</script><?php }
        ...
        while($registro=pg_fetch_array($res)){$total=$total+$registro["monto"]; $cod_presup=$registro["cod_presup"]; $fuente_financ=$registro["fuente_financ"];
		   ...
		   if($monto_credito>$monto){ $error=1; ?><script language="JavaScript">muestra('MONTO CREDITO NO PUEDE SER MAYOR AL MONTO COMPROMETIDO');</script><?php }	   
		   if(($error==0)and($tipo_imp=="C")){ //$monto_credito=$registro["monto"];
			  ...
			  if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO PRESUPUESTARIO:<?php  echo $registro["cod_presup"]; ?> FUENTE:<?php  echo $registro["fuente_financ"]; ?> REF.CREDITO:<?php  echo $ref_imput_presu; ?> \ncon NO EXISTE EN LA EJECUCION DEL CREDITO ADICIONAL');</script><?php }
				 else{$reg=pg_fetch_array($res);
				   if($reg["disponible"]<$monto_credito) {$error=1; $dispon=$reg["disponible"]; $dispon=formato_monto($dispon); ?> <script language="JavaScript"> muestra('CODIGO PRESUPUESTARIO:<?php  echo $registro["cod_presup"]; ?> FUENTE:<?php  echo $registro["fuente_financ"]; ?> REF.CREDITO:<?php  echo $ref_imput_presu; ?> \ncon Monto Mayor que Disponibilidad del Credito Adicional, Disponibilidad: <?php  echo $dispon; ?> ');</script><?php }
			  }
	    ...
	    if($error==0){
		   ...
		   if (!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php }
			else{?><script language="JavaScript">muestra('MODIFICO IMPUTACION PRESUPUESTARIA EXITOSAMENTE');</script><?php 
