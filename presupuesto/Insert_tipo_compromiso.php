...
if (pg_last_error($conn)) { ?> <script language="JavaScript">   muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');  </script> <?php }
  ...
  if($error==0){ $sSQL="Select * from pre016 WHERE tipo_comp='$tipo_comp'";  $resultado=pg_exec($conn,$sSQL); $filas=pg_numrows($resultado);
    if ($filas>0){$error=1; ?> <script language="JavaScript"> muestra('CÓDIGO TIPO DE COMPROMISO YA EXISTE'); </script> <?php }
...
pg_close($conn);if ($error==0){?><script language="JavaScript">document.location ='Act_tipo_compromiso.php';</script> <?php }else {?>  <script language="JavaScript">history.back();</script> <?php }?>