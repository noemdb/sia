<?php include ("../class/conect.php");  include ("../class/funciones.php"); header ('Content-type: text/html; charset=utf-8');
...
if (pg_last_error($conn)) { ?> <script language="JavaScript">   muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');  </script> <?php }
  ...
  if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CEDULA/RIF DE BENEFICIARIO NO EXISTE'); </script> <?php }
     ...
     $resultado=pg_exec($conn,$sSQL);$merror=pg_errormessage($conn);$merror=substr($merror, 0, 91);  if (!$resultado){$error=1; ?> <script language="JavaScript"> muestra('<?php  echo $merror; ?>'); </script> <?php }
      else{?><script language="JavaScript">muestra('MODIFICO EXITOSAMENTE');</script><?php  $error=0;  $desc_doc="CEDULA/RIF:".$ced_rif.",  NOMBRE BENEFICIARIO:".$nom_ant;  $resultado=pg_exec($conn,"SELECT INCLUYE_SIA004('05','$usuario_sia','$usuario_sia','$equipo','Modifico','$sfecha','$desc_doc')");
...
pg_close($conn);
if ($error==0){?><script language="JavaScript">document.location ='Mod_beneficiario.php?Gced_rif=<?php echo $ced_rif?>';</script> <?php } else {?>  <script language="JavaScript">history.back();</script> <?php }?>