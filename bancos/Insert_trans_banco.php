...
if($error==0){$sfecha=formato_aaaammdd($fecha); if($sfecha>=$Fec_Ini_Ejer){echo $sfecha; $error=1;?><script language="JavaScript">muestra('FECHA DE MOVIMIENTO INVALIDA');</script><?php } }
...
if($error==0){$sSQL="SELECT cod_banco FROM BAN008 WHERE cod_banco='$cod_banco' and referencia='$referencia' and tipo_trans_banco='$tipo_mov'"; $resultado=pg_query($sSQL); $filas=pg_num_rows($resultado); if($filas>0){$error=1; ?> <script language="JavaScript"> muestra('MOVIMIENTO EN TRANSITO YA EXISTE');</script><?php } }
