<?php include ("../class/conect.php");  include ("../class/funciones.php");  include ("../class/configura.inc");   error_reporting(E_ALL);  $error=0; 
...
if (pg_last_error($conn)){ ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
  ...
  if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('TIPO DE PLANILLA NO EXISTE');</script><?php }
   else{
    ...
    if($filas>0){$error=1; ?> <script language="JavaScript"> muestra('NUMERO PLANILLA DE RETENCION YA EXISTE');</script><?php }
	if($error==0){if (checkData($fecha_e)=='1'){$fecha_emision=formato_aaaammdd($fecha_e);}else{$error=1; ?> <script language="JavaScript">muestra('FECHA EMISION NO ES VALIDA');</script><?php } }
    if($error==0){      
	...
	if($monto_retencion==0){$error=1; ?> <script language="JavaScript">muestra('MONTO DE PLANILLA INVALIDA');</script><?php }
	if (strlen($nro_planilla)==8){$error=$error;} else{$error=1; ?> <script language="JavaScript">muestra('NUMERO DE PLANILLA INVALIDA');</script><?php }
	if($error==0){ $nmes=substr($fecha_e,3,2);
      if ($SIA_Periodo>$nmes){$error=1;?><script language="JavaScript">muestra('FECHA DE EMISION MENOR A ULTIMO PERIODO CERRADO');</script><?php }
    }
    ...
    if($error==0){$sfechaf=$fecha_factura;  
      ...
      echo $sSQL; $resultado=pg_exec($conn,$sSQL); $error=pg_errormessage($conn); $error="ERROR GRABANDO: ".substr($error, 0, 61);  if (!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php } else{ ?><script language="JavaScript"> muestra('INCLUYO EXITOSAMENTE');</script><?php }
    }
  }
