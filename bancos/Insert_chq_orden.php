...
if($SIA_Definicion=="N"){$error=1;?><script language="JavaScript">muestra('ETAPA DE DEFINICION ABIERTA');</script><?php }
...
if(checkData($fecha)=='1'){$error=$error;} else{$error=1; ?> <script language="JavaScript">muestra('FECHA NO ES VALIDA');</script><?php }
if(($error==0)and($monto==0)){  $error=1; ?> <script language="JavaScript"> muestra('MONTO DE CHEQUE INVALIDO');</script><?php }
if($error==0){$sfecha=formato_aaaammdd($fecha); if(($sfecha>$Fec_Fin_Ejer)or($sfecha<$Fec_Ini_Ejer)){echo $sfecha; $error=1;?><script language="JavaScript">muestra('FECHA DE CHEQUE INVALIDA');</script><?php } }
if($error==0){$nmes=substr($fecha,3, 2);  if($periodom<$SIA_Periodo){$periodom=$SIA_Periodo;} if($periodom>$nmes){$error=1;?><script language="JavaScript">muestra('FECHA DE MES MENOR A ULTIMO PERIODO CERRADO');</script><?php } }
...
if($error==0){$sSQL="SELECT cod_banco FROM BAN006 WHERE cod_banco='$cod_banco' and num_cheque='$num_cheque'"; $resultado=pg_query($sSQL); $filas=pg_num_rows($resultado);if($filas>0){$error=1; ?> <script language="JavaScript"> muestra('NUMERO DE CHEQUE YA EXISTE');</script><?php } }
if($error==0){$tipo_comp="B".$cod_banco; $sSql="Select * from con002 where text(fecha)='$sfecha' and referencia='$num_cheque' and tipo_comp='$tipo_comp'"; $resultado=pg_query($sSql); $filas=pg_num_rows($resultado);if($filas>0){$error=1; ?> <script language="JavaScript"> muestra('COMPROBANTE CONTABLE YA EXISTE');</script><?php } }
if($error==0){$nmes=substr($fecha,3, 2); $codc_banco=""; $tasa_idb=0; $cod_cont_idb="";
 ...
 if($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DE BANCO EXISTE');</script><?php }
 else {$registro=pg_fetch_array($resultado,0); $codc_banco=$registro["cod_contable"];  $activo=$registro["activa"]; $saldo_ant_libro=$registro["s_inic_libro"]; $tasa_idb=$registro["campo_num1"]; $cod_cont_idb=$registro["campo_str1"];  $formato=$registro["formato_cheque"];  if($formato<>""){$formato_cheque=$formato;}else{ $formato_cheque="Rpt_formato_chq.php";} if($activo=="N"){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DE BANCO NO ESTA ACTIVO');</script><?php }
 if($error==0){$disponible=$saldo_ant_libro; for ($i=1;$i<=$nmes;$i++){$spos=$i; If($i<=9){$spos="0".$spos;} $disponible=$disponible+$registro["deb_libro".$spos] - $registro["cre_libro".$spos]; } $balance=$monto-$disponible;
  if(($disponible<$monto)and($balance>0.001)){if($sobreg_saldo=="N"){$error=1;} echo "Disponible: ".$disponible.'  Requerido: '.$monto,"<br>"; ?> <script language="JavaScript"> muestra('SOBREGIRA SALDO DEL BANCO');</script><?php }
...
if(($error==0)and($multiple=="S")){ $sql="SELECT * FROM pag027 where codigo_mov='$codigo_mov' and seleccionada='S' order by ced_rif"; $res=pg_query($sql); while($registro=pg_fetch_array($res)){if($ced_rif==""){$ced_rif=$registro["ced_rif"];} else { if($ced_rif<>$registro["ced_rif"]){$error=1; ?> <script language="JavaScript"> muestra('SELECCION DE DIFERENTE BENEFICIARIO');</script><?php } }} }
if($error==0){ 
 if($multiple=="S"){ $resultado=pg_exec($conn,"SELECT ELIMINA_CON010('$codigo_mov')");  $mvalor=pg_errormessage($conn); $mvalor=substr($mvalor,0,90); if(!$resultado){ ?> <script language="JavaScript">  muestra('<?php  echo $mvalor; ?>'); </script> <?php }  $res=pg_exec($conn,"SELECT BORRAR_PRE026('$codigo_mov')");  $merror=pg_errormessage($conn);  $merror=substr($merror,0,90);if(!$res){ ?> <script language="JavaScript">  muestra('<?php  echo $merror; ?>'); </script> <?php  $error=1; }
     ...
     if(((($status=="N") and (($orden_ret=="S") Or ($orden_ret=="N"))) Or  (($status_r=="N") and ($orden_ret=="R"))) and ($anulado=="N")){ $tipo_chq="O"; if($orden_ret=="R"){$tipo_chq="R";}  $fecha_orden=$pag001["fecha"]; $tipo_comp="O".$tipo_causado; $descripcion=$des_chq.$pag001["concepto"]; if(($doc_concepto=="S")and($nro_documento!=="")){$descripcion=$descripcion.", ".$tipo_documento.": ".$nro_documento;} $F=0; $cod_contable_o=$pag001["cod_contable_o"]; $afecta_presu=$pag001["afecta_presu"];  $ced_rif=$pag001["ced_rif"]; $beneficiario=$pag001["nombre"]; if($pag001["pago_ces"]=="S"){$ced_rif=$pag001["ced_rif_ces"];$beneficiario=$pag001["nombre_ces"];} $descripcion_a=$beneficiario." Orden:".$prev_ord_can;
       if((($g_comp_ret=="S") Or ($retencion=="N"))and($genera_comp=="N")) { $sql="SELECT * FROM pag020  where nro_orden='$nro_orden' order by cod_cuenta"; $res=pg_query($sql);
        while($registro=pg_fetch_array($res)){ $monto_asiento=$registro["monto_pasivo"]; $cod_cuenta=$registro["cod_cuenta"];  $tipo_dc=$registro["debito_credito"]; $F=$F+1; $t_debe=$t_debe+$monto_asiento;
          ...
          if($filas>0){ $reg=pg_fetch_array($resultado);   $monto_c=$monto_asiento+$reg["monto_asiento"]; $monto_c=cambia_coma_numero($monto_c);  if($monto_c>0){$resultado=pg_exec($conn,"SELECT MODIFICA_CUENTA_CON010('$codigo_mov','D','$cod_cuenta',$monto_c,'$descripcion_a')"); $error=pg_errormessage($conn);  $error="ERROR GRABANDO: ".substr($error,0,90); if(!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php  $error=1;} } }
           else{$resultado=pg_exec($conn,"SELECT INCLUYE_CON010('$codigo_mov','$nro_orden','D','$cod_cuenta','00000','',$monto_asiento,'D','B','N','02','0','$descripcion_a')"); $error=pg_errormessage($conn);  $error="ERROR GRABANDO: ".substr($error,0,90); if(!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php  $error=1; }}
        }
       ...
       if((($g_comp_ret=="S") Or ($retencion=="N"))and($genera_comp=="S")) { $sql="SELECT * FROM con003  where referencia='$nro_orden' and fecha='$fecha_orden' and tipo_comp='$tipo_comp' and debito_credito='C' order by cod_cuenta"; $res=pg_query($sql); $continua=1;
         ...
         if($continua==1){
           ...
           if($continua==1){$monto_asiento=$registro["monto_asiento"]; $t_debe=$t_debe+$monto_asiento; $sSQL="Select * from CON010 WHERE codigo_mov='$codigo_mov' and cod_cuenta='$cod_cuenta' and debito_credito='D'"; $resultado=pg_exec($conn,$sSQL); $filas=pg_numrows($resultado);
             if($filas>0){  $reg=pg_fetch_array($resultado);   $monto_c=$monto_asiento+$reg["monto_asiento"]; $monto_c=cambia_coma_numero($monto_c); if($monto_c>0){$resultado=pg_exec($conn,"SELECT MODIFICA_CUENTA_CON010('$codigo_mov','D','$cod_cuenta',$monto_c,'$descripcion_a')"); $F=$F+1; $error=pg_errormessage($conn);  $error="ERROR GRABANDO: ".substr($error,0,90); if(!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php  $error=1;} } }
              else{$resultado=pg_exec($conn,"SELECT INCLUYE_CON010('$codigo_mov','$nro_orden','D','$cod_cuenta','00000','',$monto_asiento,'D','B','N','02','0','$descripcion_a')"); $error=pg_errormessage($conn); $F=$F+1; $error="ERROR GRABANDO: ".substr($error,0,90); if(!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php  $error=1;}}
             } }
        }
        if(($monto_pasivo>0) and ($status_2<>"C")){ $sql="SELECT * FROM pag020 where nro_orden='$nro_orden' and debito_credito='D' order by cod_cuenta"; $res=pg_query($sql);
         while($registro=pg_fetch_array($res)){ $monto_asiento=$registro["monto_pasivo"]; $cod_cuenta=$registro["cod_cuenta"];  $tipo_dc="D"; $F=$F+1;
          $sSQL="Select * from CON010 WHERE codigo_mov='$codigo_mov' and cod_cuenta='$cod_cuenta' and debito_credito='D'"; $resultado=pg_exec($conn,$sSQL); $filas=pg_numrows($resultado);
          if($filas>0){ $reg=pg_fetch_array($resultado);   $monto_c=$monto_asiento+$reg["monto_asiento"];   $monto_c=cambia_coma_numero($monto_c);  if($monto_c>0){$resultado=pg_exec($conn,"SELECT MODIFICA_CUENTA_CON010('$codigo_mov','D','$cod_cuenta',$monto_c,'$descripcion_a')"); $error=pg_errormessage($conn);  $error="ERROR GRABANDO: ".substr($error,0,90); if(!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php  $error=1;} } }
           else{$resultado=pg_exec($conn,"SELECT INCLUYE_CON010('$codigo_mov','$nro_orden','D','$cod_cuenta','00000','',$monto_asiento,'D','B','N','02','0','$descripcion_a')"); $error=pg_errormessage($conn);  $error="ERROR GRABANDO: ".substr($error,0,90); if(!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php  $error=1; }}  $t_debe=$t_debe+$monto_asiento;
         }
        }		
		if(($monto_pasivo==0) and ($status_2=="C")){ $sql="SELECT * FROM pag020 where nro_orden='$nro_orden' and debito_credito='C' order by cod_cuenta"; $res=pg_query($sql);
          while($registro=pg_fetch_array($res)){ $monto_asiento=$registro["monto_pasivo"];  $cod_cuenta=$registro["cod_cuenta"]; 
            ...
            if($filas>0){ $sSQL="SELECT ELIMINA_CUENTA_CON010('$codigo_mov','D','$cod_cuenta')";    $resultado=pg_exec($conn,$sSQL); $error=pg_errormessage($conn);  $error="ERROR ELIMINANDO: ".substr($error,0,90); 
			   if(!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php  $error=1; }   else{   $t_debe=$t_debe-$monto_asiento; } }
		  }
        }		
       ...
       if(((($genera_comp=="N") and ($status_1=="S")) Or (($genera_comp=="S") and ($status_2=="S"))) and ($genera_orden_r=="N") and ($ret_presup=="S")) {
         ...
         while($reg=pg_fetch_array($res)){ $monto_c=$reg["monto_retencion"]; $cod_presup=$reg["cod_presup_ret"]; $fuente_financ=$reg["fuente_fin_ret"]; $nro_orden_r=$reg["nro_orden_ret"]; $tipo_causado_r=$reg["tipo_caus_ret"]; $referencia_comp=$reg["ref_comp_ret"]; $tipo_compromiso=$reg["tipo_comp_ret"]; $tipo_imput_presu=$reg["tipo_imput_presu"];  $ref_imput_presu=$reg["ref_imput_presu"]; $monto_credito=$reg["monto_credito"]; $func_inv=$reg["func_inv"];  $fecha_aep=$reg["fecha_aep"]; $ref_aep=$reg["ref_aep"]; $num_proyecto=$reg["num_proyecto"];  $operacion="N";  if($reg["ref_compromiso"]="SI"){$operacion="C";}
          ...
          if($filas==0){ $resultado=pg_exec($conn,"SELECT INCLUYE_PRE026('$codigo_mov','$cod_presup','$fuente_financ','$referencia_comp','$tipo_compromiso','$nro_orden_r','$tipo_causado_r','','0000','','0000','$operacion','','','','$ref_aep','$num_proyecto','$fecha_aep','$func_inv','$tipo_imput_presu','$ref_imput_presu','$fecha_orden',$monto_c,0,$monto_credito,0)");  $merror=pg_errormessage($conn);   $merror="ERROR GRABANDO: ".substr($merror,0,90);  if(!$resultado){?><script language="JavaScript">muestra('<?php  echo $merror; ?>');</script><?php  $error=1; } }
			...
			$resultado=pg_exec($conn,$sqlg);  $merror=pg_errormessage($conn);   $merror="ERROR GRABANDO: ".substr($merror,0,90);  if(!$resultado){?><script language="JavaScript">muestra('<?php  echo $merror; ?>');</script><?php  $error=1; } }
         }
       }
	   if(($total_retencion>0) and ($retencion=="N") and (($g_comp_ret=="N")Or($genera_orden_r=="N"))) { $prev_orden = ""; $sql="select nro_orden_ret,aux_orden,tipo_caus_ret,ref_comp_ret,tipo_comp_ret,cod_presup_ret,fuente_fin_ret,monto_retencion from pag004 where (nro_orden_ret='$nro_orden')"; $res=pg_query($sql);
         while($reg=pg_fetch_array($res)){ $aux_orden=$reg["aux_orden"]; $monto_r=$reg["monto_retencion"]; $monto_c=$reg["monto_retencion"]; $cod_presup=$reg["cod_presup_ret"]; $fuente_financ=$reg["fuente_fin_ret"]; $nro_orden_r=$reg["nro_orden_ret"]; $tipo_causado_r=$reg["tipo_caus_ret"]; $referencia_comp=$reg["ref_comp_ret"]; $tipo_compromiso=$reg["tipo_comp_ret"];
           if(($genera_orden_r=="N") and ($ret_presup=="S") and ($afecta_presu=="S")) {
			...
			//$sqlg="SELECT MOD_MONTO_PRE026('$codigo_mov','$cod_presup','$fuente_financ','$referencia_comp','$tipo_compromiso','$ref_imput_presu',$monto_c,$monto_credito)";
			//echo $sqlg,"<br>";
			$resultado=pg_exec($conn,$sqlg);  $error=pg_errormessage($conn);   $error="ERROR GRABANDO: ".substr($error,0,90);  if(!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php  $error=1;} }
           }
           if(($prev_orden!==$aux_orden) and ($g_comp_ret=="N")) { $prev_orden=$aux_orden; $sSQL="select referencia,cod_cuenta,monto_asiento,debito_credito from pag005 Where (referencia='$nro_orden') and (fecha='$fecha_orden') and (debito_credito='c')"; $res=pg_query($sSQL);
            while($registro=pg_fetch_array($res)){ $monto_asiento=$registro["monto_asiento"]; $cod_cuenta=$registro["cod_cuenta"];  $tipo_dc=$registro["debito_credito"];
              ...
              if($filas>0){ $reg=pg_fetch_array($resultado);   $monto_c=$monto_asiento+$reg["monto_asiento"];  $monto_c=cambia_coma_numero($monto_c); if($monto_c>0){$resultado=pg_exec($conn,"SELECT MODIFICA_CUENTA_CON010('$codigo_mov','C','$cod_cuenta',$monto_c,'$descripcion_a')"); $F=$F+1; $error=pg_errormessage($conn);  $error="ERROR GRABANDO: ".substr($error,0,90); if(!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php  $error=1;} } }
               else{$resultado=pg_exec($conn,"SELECT INCLUYE_CON010('$codigo_mov','$nro_orden','C','$cod_cuenta','00000','',$monto_asiento,'D','B','N','02','0','$descripcion_a')"); $error=pg_errormessage($conn); $F=$F+1; $error="ERROR GRABANDO: ".substr($error,0,90); if(!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php  $error=1; }}  $t_haber=$t_haber+$monto_asiento;
            }
           }
         }
       }
       if($F==0){$error=1; ?> <script language="JavaScript"> muestra('LOCALIZANDO COMPROBANTES MULTIPLE DE ORDEN :'+<?php  echo $nro_orden; ?>);</script><?php }
	   if($error==0){  $fecha2=formato_ddmmaaaa($fecha_e); if(FDate($fecha1)<FDate($fecha2)){ echo "Orden .".$nro_orden." Fecha: ".$fecha2.", Fecha Cheque: ".$fecha1,"<br>"; $error=1; ?> <script language="JavaScript">  muestra('Fecha Cheque menor a Fecha Orden'); </script> <?php } }
       if($error==0){ $resultado=pg_exec($conn,"SELECT SELECCIONA_PAG027('$codigo_mov','$nro_orden','$tipo_causado','I')"); $merror=pg_errormessage($conn);  $merror=substr($merror,0,90); if(!$resultado){ ?> <script language="JavaScript">  muestra('<?php  echo $merror; ?>'); </script> <?php   $error=1;}}
     }
    }else{$error=1; ?> <script language="JavaScript"> muestra('ERROR LOCALIZANDO ORDEN :'+<?php  echo $nro_orden; ?>);</script><?php }
  }
  if(($total_Cheques>0) and ($error==0)){  $descripcion_a=$beneficiario." Orden:".$prev_ord_can; $total_Cheques=cambia_coma_numero($total_Cheques);
     $resultado=pg_exec($conn,"SELECT INCLUYE_CON010('$codigo_mov','$num_cheque','C','$codc_banco','00000','',$total_Cheques,'D','B','N','02','0','$descripcion_a')"); $error=pg_errormessage($conn); $F=$F+1; $error="ERROR GRABANDO: ".substr($error,0,90); if(!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php  $error=1; }  $t_haber=$t_haber+$total_Cheques;
     if($t_debe>$t_haber){$balance=$t_debe-$t_haber;}else{$balance=$t_haber-$t_debe;}
     if($balance>0.001){$error=1; echo $t_debe.' '.$t_haber.' '.formato_monto($balance),"<br>"; ?> <script language="JavaScript"> muestra('COMPROBANTE CONTABLE NO CUADRA');</script><?php }
     if(($t_debe==0)or($t_haber==0)){$error=1; echo $t_debe.' '.$t_haber.' '.formato_monto($balance),"<br>"; ?> <script language="JavaScript"> muestra('MONTO DEL COMPROBANTE CONTABLE NO VALIDO');</script><?php }
     if($error==0){ $resultado=pg_exec($conn,"SELECT MODIFICA_DES_CON010('$codigo_mov','$descripcion_a')"); $merror=pg_errormessage($conn);  $merror=substr($merror,0,90); if(!$resultado){ ?> <script language="JavaScript">  muestra('<?php  echo $merror; ?>'); </script> <?php   $error=1;}}
     if($error==0){ if($tasa_idb>0)  {$mstatus="I";$monto_idb=$total_Cheques*($tasa_idb/100); $monto_idb=Round($monto_idb, 2);} else{$cod_cont_idb=""; $mstatus="N"; $monto_idb=0;}    $sfecha=formato_aaaammdd($fecha_cheque);    $descripcion=$concepto;
        ...
        else{ $error=0;?><script language="JavaScript">  muestra('INCLUYO EXITOSAMENTE CHEQUE: '+<?php  echo $num_cheque; ?>);</script><?php  $cant=$cant+1;  $num_cheque_h=$num_cheque; $resultado=pg_exec($conn,"SELECT ELIMINA_CON010('$codigo_mov')");  $mvalor=pg_errormessage($conn); $mvalor=substr($mvalor,0,90); if(!$resultado){ ?> <script language="JavaScript">  muestra('<?php  echo $mvalor; ?>'); </script> <?php }  $res=pg_exec($conn,"SELECT BORRAR_PRE026('$codigo_mov')");  $merror=pg_errormessage($conn);  $merror=substr($merror,0,90);if(!$res){ ?> <script language="JavaScript">  muestra('<?php  echo $error; ?>'); </script> <?php  $error=1; } }
       /* */
	 }
  } 
 }
    ...
    if($filas==1){$pag001=pg_fetch_array($resultado,0); $fecha_e=$pag001["fecha"]; $total_causado=$pag001["total_causado"]; $total_retencion=$pag001["total_retencion"]; $monto_pasivo=$pag001["total_pasivos"]; $monto_am_ant=$pag001["monto_am_ant"]; $genera_comp=$pag001["genera_comp"]; $status=$pag001["status"];  $status_r=$pag001["status_r"];  $anulado=$pag001["anulado"];  $nro_documento=$pag001["nro_documento"]; $tipo_documento=$pag001["tipo_documento"]; $amort_ant=$pag001["status_1"]; $status_1=$pag001["status_1"]; $status_2=$pag001["status_2"]; $cta_anticipo=$pag001["campo_str1"]; $g_comp_ret=$pag001["genera_comp_ret"]; $retencion=$pag001["retencion"]; $genera_orden_r=$pag001["genera_orden_r"];
     if(((($status=="N") and (($orden_ret=="S") Or ($orden_ret=="N"))) Or  (($status_r=="N") and ($orden_ret=="R"))) and ($anulado=="N")){ $tipo_chq="O"; if($orden_ret=="R"){$tipo_chq="R";}  $fecha_orden=$pag001["fecha"]; $tipo_comp="O".$tipo_causado; 
	  ...
	  // echo $total_retencion." ".$retencion." ".$g_comp_ret." ".$genera_orden_r,"<br>";       
       ...
       if($F==0){$error=1; ?> <script language="JavaScript"> muestra('LOCALIZANDO COMPROBANTE DE ORDEN :'+<?php  echo $nro_orden; ?>);</script><?php }
	   if($error==0){  $fecha2=formato_ddmmaaaa($fecha_e); if(FDate($fecha1)<FDate($fecha2)){ echo "Orden .".$nro_orden." Fecha: ".$fecha2.", Fecha Cheque: ".$fecha1,"<br>"; $error=1; ?> <script language="JavaScript">  muestra('Fecha Cheque menor a Fecha Orden'); </script> <?php } }
       if($error==0){ $resultado=pg_exec($conn,"SELECT INCLUYE_CON010('$codigo_mov','$num_cheque','C','$codc_banco','00000','',$total_Cheques,'D','B','N','02','0','$descripcion_a')"); $error=pg_errormessage($conn); $F=$F+1; $error="ERROR GRABANDO: ".substr($error,0,90); if(!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php  $error=1; }  $t_haber=$t_haber+$total_Cheques; }
       if($t_debe>$t_haber){$balance=$t_debe-$t_haber;}else{$balance=$t_haber-$t_debe;}
       if($balance>0.001){$error=1; echo ' Debe: '.$t_debe.' Haber: '.$t_haber.' Dif: '.formato_monto($balance).' Monto chq: '.formato_monto($total_Cheques),"<br>"; ?> <script language="JavaScript"> muestra('COMPROBANTE CONTABLE NO CUADRA');</script><?php }
       if(($t_debe==0)or($t_haber==0)){$error=1; echo $t_debe.' '.$t_haber.' '.formato_monto($balance),"<br>"; ?> <script language="JavaScript"> muestra('MONTO DEL COMPROBANTE CONTABLE NO VALIDO');</script><?php }
       if($error==0){ $resultado=pg_exec($conn,"SELECT SELECCIONA_PAG027('$codigo_mov','$nro_orden','$tipo_causado','I')"); $merror=pg_errormessage($conn);  $merror=substr($merror,0,90); if(!$resultado){ ?> <script language="JavaScript">  muestra('<?php  echo $merror; ?>'); </script> <?php   $error=1;}}
...
/**/	
         ...
         $resultado=pg_exec($conn,$sSQL);  $merror=pg_errormessage($conn);  $merror=substr($merror,0,90); if(!$resultado){?><script language="JavaScript"> muestra('<?php  echo $merror; ?>');</script><?php  $error=1;}
         else{ $error=0;?><script language="JavaScript">  muestra('INCLUYO EXITOSAMENTE CHEQUE: '+'<?php  echo $num_cheque; ?>'+' DE ORDEN :'+'<?php  echo $nro_orden; ?>');</script><?php   $cant=$cant+1;  $num_cheque_h=$num_cheque;  
		 $resultado=pg_exec($conn,"SELECT ELIMINA_CON010('$codigo_mov')");  $mvalor=pg_errormessage($conn); $mvalor=substr($mvalor,0,90); if(!$resultado){ ?> <script language="JavaScript">  muestra('<?php  echo $mvalor; ?>'); </script> <?php }
		 $res=pg_exec($conn,"SELECT BORRAR_PRE026('$codigo_mov')");$merror=pg_errormessage($conn);  $merror=substr($merror,0,90);if(!$res){ ?> <script language="JavaScript">  muestra('<?php  echo $merror; ?>'); </script> <?php  $error=1; } 
		 }
...
<table width="957">
  ...
  <tr><td><table width="923">
	  ...
	  <td width="120">
<?php  $num=0; $temp_chq=$num_cheque_d;
?> <select name="txtnum_chq_imp" id="txtnum_chq_imp" onFocus="encender(this)" onBlur="apagar(this)" ><?php 
while($num<$cant){ $ult_ref=$temp_chq+$num; $len=strlen($ult_ref); $ult_ref=substr("00000000",0,8-$len).$ult_ref;    $num=$num+1;
?><option value="<?php  echo $ult_ref;?>"><?php  echo $ult_ref;?></option>
<?php }?></select> </td>
      </td>
	  <td width="200"></td>
	  <td width="250"></td>
...
<?php  if($error==0){	
	...
	<td width="200"><input name="btEmitir" type="button" id="btEmitir" value="Imprimir Cheque" title="Impresion Cheque Emitido" onclick="javascript:Ventana_chq('<?php  echo $furl; ?>'+document.form1.txtnum_chq_imp.value+'&tipo_rpt='+document.form1.txttipo_rpt.value);"></td>
<?php }else{?>   
    <td width="160"></td>
    <td width="130"><input name="txttipo_rpt" type="hidden" id="txttipo_rpt" value='TXT'></td>
	<td width="200"><input name="txtnum_cheque" type="hidden" id="txtnum_cheque" value="<?php echo $num_cheque?>"></td> 
<?php } ?>
        <td width="250"><input name="btEmitir" type="button" id="btEmitir" value="Volver a Emision de Cheque" title="Retornar Emision de " onclick="javascript:document.location ='<?php  echo $url; ?>';"></td>
