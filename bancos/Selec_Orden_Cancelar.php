<?php include ("../class/conect.php");  include ("../class/funciones.php"); $tfechac=asigna_fecha_hoy(); $tfechac=formato_aaaammdd($tfechac);
...
if($filas==0){$error=1; ?> <script language="JavaScript">  muestra('Error Localizando Orden'); </script> <?php }
else{$registro=pg_fetch_array($resultado); $c_rif=$registro["ced_rif"];  $fecha_e=$registro["fecha"];}
if($multiple=="S"){if($c_rif!=$ced_rif){$error=1; ?> <script language="JavaScript">  muestra('Benficiario debe ser el Mismo'); </script> <?php } }
if($error==0){ if($fechac==""){$fechac=$tfechac;} $fecha1=formato_ddmmaaaa($fechac); $fecha2=formato_ddmmaaaa($fecha_e);
  ...
  if($num1<$num2){ echo "Fecha Orden: ".$fecha2.", Fecha Cheque: ".$fecha1." ".$num1." ".$num2,"<br>"; $error=1; ?> <script language="JavaScript">  muestra('Fecha Cheque menor a Fecha Orden'); </script> <?php }
}
if($error==0){
...
pg_close($conn); 
error_reporting(E_ALL ^ E_WARNING);?> <script language="JavaScript">  document.location ='<?php  echo $url; ?>' </script>