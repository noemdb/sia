...
if($c_rif!=$ced_rif){$error=1; ?> <script language="JavaScript">  muestra('Benficiario debe ser el Mismo'); </script> <?php } }

if($error==0){$resultado=pg_exec($conn,"SELECT SELECCIONA_PAG027('$codigo_mov','$nro_orden','$tipo_causado','$selec')"); $error=pg_errormessage($conn); $error=substr($error, 0, 61); if (!$resultado){ ?> <script language="JavaScript">  muestra('<?php  echo $error; ?>'); </script> <?php } }
