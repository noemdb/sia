<?php include ("../class/conect.php");  include ("../class/funciones.php"); error_reporting(E_ALL);  $cod_tipo_en=$_GET["txtcod_tipo_en"]; $tipo_en="";
  ...
  if ($filas==0){ $error=1; ?>  <script language="JavaScript">  muestra('TIPO DE ENRIQUECIMIENTO  NO EXISTE');  </script> <?php }
     ...
     if($error==0){  $sSQL="SELECT ACTUALIZA_ban020(3,'$cod_tipo_en','$tipo_en')";  $resultado=pg_exec($conn,$sSQL); $error=pg_errormessage($conn);$error=substr($error,0,91);
       if (!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php }else{$error=0;?><script language="JavaScript">muestra('ELIMINO EXITOSAMENTE'); </script><?php }
         $desc_doc="TIPO DE ENRIQUECIMIENTO:".$cod_tipo_en.", DESCRIPCION:".$adescripcion_tipo; $resultado=pg_exec($conn,"SELECT INCLUYE_SIA004('02','$usuario_sia','$usuario_sia','$equipo','Elimino','$sfecha','$desc_doc')"); $error=pg_errormessage($conn);$error=substr($error,0,91);if (!$resultado){?><script language="JavaScript">muestra('<?php echo $error;?>');</script><?php } }
  }
}
