...
if (pg_last_error($conn)) { ?>  <script language="JavaScript"> muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS'); </script> <?php }
 else{ if (checkData($fecha_inicio)=='1'){$error=0;} else{$error=1; ?> <script language="JavaScript">muestra('FECHA DE INICIO NO ES VALIDA');</script><?php } }
if($error==0){if (checkData($fecha_vencimiento)=='1'){$error=0;} else{$error=1; ?> <script language="JavaScript">muestra('FECHA DE VENCIMIENTO  NO ES VALIDA');</script><?php } }
if($error==0){if(strlen($referencia)==8){$error=0;} else {$error=1; ?> <script language="JavaScript"> muestra('LONGITUD DE REFERENCIA INVALIDA');</script><?php } }
...
if($error==0){ $sfechai=formato_aaaammdd($fecha_inicio);  $sfechav=formato_aaaammdd($fecha_vencimiento);
  ...
  if ($filas>0){ $error=1; ?>  <script language="JavaScript">  muestra('REFERENCIA DE INVERSION YA EXISTE');  </script> <?php }
...
pg_close($conn); error_reporting(E_ALL ^ E_WARNING); if ($error==0){?><script language="JavaScript">document.location='Act_Colocaciones.php?Greferencia=U';</script> <?php } else {?>  <script language="JavaScript">history.back();</script> <?php } ?>
