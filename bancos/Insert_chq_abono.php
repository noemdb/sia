...
if($error==0){$sql="Select * from ORD_PAGO where nro_orden='$nro_orden' and anulado='N'"; $res=pg_query($sql); $filas=pg_num_rows($res); 
	...
	else{ if(($monto_bruto<>$monto_abono)and($balance>0.001)){$error=1; echo $monto_bruto.' '.$monto_abono.' '.formato_monto($balance),"<br>"; ?> <script language="JavaScript"> muestra('MONTO DEL PAGADO PRESUPUESTARIO NO VALIDO');</script><?php } } }
}
