<?php include ("../class/conect.php");  include ("../class/funciones.php"); error_reporting(E_ALL);  $tipo_movimiento=$_GET["txttipo_movimiento"];
...
if (pg_last_error($conn)) { ?>  <script language="JavaScript"> muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS'); </script> <?php }
  ...
  if ($filas==0){ $error=1; ?>  <script language="JavaScript">  muestra('TIPO DE MOVIMIENTO NO EXISTE');  </script> <?php }
    ...
    if($error==0){ $sSQL="Select * from ban004 WHERE tipo_mov_libro='$tipo_movimiento'"; $resultado=pg_exec($conn,$sSQL);$filas=pg_numrows($resultado);
       if ($filas>0){$error=1; ?>  <script language="JavaScript">  muestra('TIPO DE MOVIMIENTO TIENE MOV. LIBROS');  </script> <?php }
    }
    if($error==0){ $sSQL="Select * from ban005 WHERE tipo_mov_banco='$tipo_movimiento'"; $resultado=pg_exec($conn,$sSQL);$filas=pg_numrows($resultado);
       if ($filas>0){$error=1; ?>  <script language="JavaScript">  muestra('TIPO DE MOVIMIENTO TIENE MOV. BANCOS');  </script> <?php }
    }
    if($error==0){ $sSQL="Select * from ban007 WHERE tipo_trans_libro='$tipo_movimiento'"; $resultado=pg_exec($conn,$sSQL);$filas=pg_numrows($resultado);
       if ($filas>0){$error=1; ?>  <script language="JavaScript">  muestra('TIPO DE MOVIMIENTO TIENE MOV. TRANS. LIBROS');  </script> <?php }
    }
    if($error==0){ $sSQL="Select * from ban008 WHERE tipo_trans_banco='$tipo_movimiento'"; $resultado=pg_exec($conn,$sSQL);$filas=pg_numrows($resultado);
      if ($filas>0){$error=1; ?>  <script language="JavaScript">  muestra('TIPO DE MOVIMIENTO TIENE MOV. TRANS. BANCOS');  </script> <?php }
    }
    if($error==0){ $sSQL="SELECT ACTUALIZA_BAN003(3,'$tipo_movimiento','$adescripcion_tipo','D','I')"; $resultado=pg_exec($conn,$sSQL); $error=pg_errormessage($conn);$error=substr($error,0,91);
       if (!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php }else{$error=0;?><script language="JavaScript">muestra('ELIMINO EXITOSAMENTE'); </script><?php }
         $desc_doc="TIPO DE MOVIMIENTO:".$tipo_movimiento.", DESCRIPCION:".$adescripcion_tipo; $resultado=pg_exec($conn,"SELECT INCLUYE_SIA004('02','$usuario_sia','$usuario_sia','$equipo','Modifico','$sfecha','$desc_doc')"); $error=pg_errormessage($conn);$error=substr($error,0,91);if (!$resultado){?><script language="JavaScript">muestra('<?php echo $error;?>');</script><?php } }
  }
}
