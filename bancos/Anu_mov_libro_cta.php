...
if(($tipo_mov=="NDB")or($tipo_mov=="CHQ")){$error=1; ?><script language="JavaScript">  muestra('MOVIMIENTO NO VALIDO');  </script><?php } $DOperacion="1";
...
if($error==0){ $fecha_d=formato_ddmmaaaa($Fec_Ini_Ejer);  $fecha_h=formato_ddmmaaaa($Fec_Fin_Ejer); $afecha=formato_aaaammdd($fecha_anu);
  if (($afecha>$Fec_Fin_Ejer)or($afecha<$Fec_Ini_Ejer)){$error=1;?><script language="JavaScript">muestra('FECHA DE ANULACION INVALIDA');</script><?php }
}
...
if($error==0){
if($cod_contab_ban==$cod_contab_rev){$error=1; ?> <script language="JavaScript"> muestra('CUENTA CONTABLE INVALIDA');</script><?php }
}
if ($error==0){ $sSQL="SELECT cod_banco,nombre_banco,nro_cuenta,cod_contable FROM ban002 WHERE cod_contable='$cod_contab_rev'";  $resultado=pg_query($sSQL);  $filas=pg_num_rows($resultado);
  if ($filas>0){ $error=1;?> <script language="JavaScript">muestra('CODIGO DE CUENTA ANULACION ES DE A UN BANCO');</script> <?php } }
if($error==0){ $sfecha=$fecha; $sfechaa=$fecha_anu; $proceso=0;  $proceso=0;
  ...
  if (!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php }else{?><script language="JavaScript">  muestra('ANULO EXITOSAMENTE'); </script><?php 
    ...
    $error=pg_errormessage($conn);   if (!$resultado){echo $error; $error=substr($error, 0, 61);?><script language="JavaScript">muestra('<?php echo $error;?>');</script><?php } }
}
