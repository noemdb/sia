...
if(($error==0)and($tasa_cambio==0)){$error=1; ?> <script language="JavaScript"> muestra('TASA DE CAMBIO INVALIDO');</script><?php }
if(($error==0)and($monto_bs==0)){$error=1; ?> <script language="JavaScript"> muestra('MONTO EN BOLIVARES INVALIDO');</script><?php }
...
*/
if($error==0){if(strlen($referencia)==8){$error=0;} else{$error=1; ?> <script language="JavaScript"> muestra('LONGITUD REFERENCIA MOVIMIENTO INVALIDO');</script><?php } }
if($error==0){$sSQL="SELECT cod_banco FROM BAN044 WHERE cod_banco='$cod_banco' and referencia='$referencia' and tipo_mov_libro='$tipo_mov'"; $resultado=pg_query($sSQL); $filas=pg_num_rows($resultado); if($filas>0){$error=1; ?> <script language="JavaScript"> muestra('MOVIMIENTO EN DOLARES YA EXISTE');</script><?php } }
...
if($error==0){$sSQL="Select * from BAN043 WHERE codigo_cuenta='$cod_cta_flujo'";  $resultado=pg_exec($conn,$sSQL); $filas=pg_numrows($resultado); if($filas==0){$error=1;?><script language="JavaScript">muestra('CODIGO CUENTA FLUJO NO EXISTE');</script><?php } }
