...
if (pg_last_error($conn)) { ?> <script language="JavaScript"> muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');  </script> <?php }
  ...
  if ($filas==0){ $error=1; ?>  <script language="JavaScript">  muestra('GASTO DE FLUJO NO EXISTE');  </script> <?php }
   else{
     if($error==0){ $ssqlg="SELECT ACTUALIZA_BAN021(4,'$periodo','','','','','$sfecha',0,'','','','',0,'','',0,0,'')";  $resultado=pg_exec($conn,$ssqlg); $error=pg_errormessage($conn);  $error=substr($error, 0, 100);  
	   if (!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php }else{?><script language="JavaScript">  muestra('ELIMINO EXITOSAMENTE'); </script><?php 
         $desc_doc="GASTO DE FLUJO, MES:".$periodo; $resultado=pg_exec($conn,"SELECT INCLUYE_SIA004('02','$usuario_sia','$usuario_sia','$equipo','Elimino','$sfecha','$desc_doc')"); $error=pg_errormessage($conn); $error=substr($error,0,91);  if (!$resultado){?><script language="JavaScript">muestra('<?php echo $error;?>');</script><?php } } }
  }
