<?php include ("../class/conect.php");  include ("../class/funciones.php");  $cod_banco=$_GET["cod_banco"];$codigo_mov=$_GET["codigo_mov"];$monto=$_GET["monto"];  $monto=formato_numero($monto); if(is_numeric($monto)){$monto=$monto;} else{$monto=0;}
...
$resultado=pg_exec($conn,"SELECT ACT_DEB_CON010('$codigo_mov','00000000','C','$codc_banco','00000','',$monto,'D','B','N','01','0','')");
 ?><iframe src="Det_inc_comp_libro.php?codigo_mov=<?php echo $codigo_mov?>" width="940" height="350" scrolling="auto" frameborder="1"></iframe>