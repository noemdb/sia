<?php include ("../class/seguridad.inc");  include ("../class/conect.php");  include ("../class/funciones.php"); $fecha_hoy=asigna_fecha_hoy();
...
function Nivel_Cta_Valido($Clave, $Formato){  $mval=0;  $e=0;
  ...
  if ($mval==1){?><script language="JavaScript">muestra('ERROR niveles de la Cuenta Invalida');</script><?php }
  return $mval;
}
?>
<?php
...
If (pg_last_error($conn)){ ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
  ...
  if (checkData($fecha)=='1'){$sfecha=formato_aaaammdd($fecha);} else{$error=1; ?> <script language="JavaScript"> muestra('FECHA NO ES VALIDA');</script><?php }
  if ($error==0){
    ...
    if ($l>0){$temp_cuenta=substr($codigo_cuenta,0,$l);
       ...
       if ($filas==0){ $error=1; ?> <script language="JavaScript"> muestra('CODIGO DE CUENTA NIVEL ANTERIOR NO EXISTE');</script> <?php }	   
    ...
    if ($error==0){  $sSQL="Select * from BAN043 WHERE codigo_cuenta='$codigo_cuenta'";       $resultado=pg_exec($conn,$sSQL); $filas=pg_numrows($resultado);
      if ($filas>0){?> <script language="JavaScript">muestra('CODIGO DE CUENTA YA EXISTE');</script> <?php }
       else{
         ...
         $error=pg_errormessage($conn);$error="ERROR GRABANDO: ".substr($error, 0, 61);if (!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php } else{?><script language="JavaScript">muestra('INCLUYO EXITOSAMENTE');</script><?php }
      }
    }
  }
}
