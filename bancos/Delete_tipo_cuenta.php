<?php include ("../class/conect.php");  include ("../class/funciones.php");  $tipo_cuenta=$_GET["txttipo_cuenta"];$equipo = getenv("COMPUTERNAME"); $minf_usuario = $usuario_sia." ".$equipo." ".date("d/m/y H:i a");
...
if (pg_last_error($conn)) { ?> <script language="JavaScript"> muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');  </script> <?php }
  ...
  if ($filas==0){ $error=1; ?>  <script language="JavaScript">  muestra('TIPO DE CUENTA NO EXISTE');  </script> <?php }
     ...
     if($error==0){ $sSQL="Select * from ban002 WHERE tipo_cuenta='$tipo_cuenta'"; $resultado=pg_exec($conn,$sSQL);$filas=pg_numrows($resultado);
       if ($filas>0){$error=1; ?>  <script language="JavaScript">  muestra('TIPO DE CUENTA TIENE BANCO DEFINIDO');  </script> <?php }
     }
