...
if (pg_last_error($conn)) { ?> <script language="JavaScript"> muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');  </script> <?php }
  ...
  if ($filas==0){ $error=1; ?>  <script language="JavaScript">  muestra('CODIGO DE BANCO NO EXISTE');  </script> <?php }
     ...
     if(($saldo_ant_libro>0)||($saldo_ant_banco>0)){$error=1; ?>  <script language="JavaScript">  muestra('BANCO TIENE SALDOS INICIALES');  </script> <?php }
     if (checkData($fecha_anu)=='1'){$error=0;$afecha=formato_aaaammdd($fecha_anu);} else{$error=1; ?> <script language="JavaScript">muestra('FECHA ANULACION NO ES VALIDA');</script><?php }
     if($error==0){ $sSQL="Select * from ban004 WHERE cod_banco='$cod_banco'"; $resultado=pg_exec($conn,$sSQL);$filas=pg_numrows($resultado);
       if ($filas>0){$error=1; ?>  <script language="JavaScript">  muestra('CODIGO DE BANCO TIENE MOV. LIBROS');  </script> <?php }
     }
     if($error==0){ $sSQL="Select * from ban005 WHERE cod_banco='$cod_banco'"; $resultado=pg_exec($conn,$sSQL);$filas=pg_numrows($resultado);
       if ($filas>0){$error=1; ?>  <script language="JavaScript">  muestra('CODIGO DE BANCO TIENE MOV. BANCOS');  </script> <?php }
     }
     if($error==0){ $sSQL="Select * from ban007 WHERE cod_banco='$cod_banco'"; $resultado=pg_exec($conn,$sSQL);$filas=pg_numrows($resultado);
       if ($filas>0){$error=1; ?>  <script language="JavaScript">  muestra('CODIGO BANCO TIENE MOV. TRANS. LIBROS');  </script> <?php }
     }
     if($error==0){ $sSQL="Select * from ban008 WHERE cod_banco='$cod_banco'"; $resultado=pg_exec($conn,$sSQL);$filas=pg_numrows($resultado);
       if ($filas>0){$error=1; ?>  <script language="JavaScript">  muestra('CODIGO BANCO TIENE MOV. TRANS. BANCOS');  </script> <?php }
     }
     if($error==0){ $sSQL="Select * from pag008 WHERE cod_banco_t='$cod_banco'"; $resultado=pg_exec($conn,$sSQL);$filas=pg_numrows($resultado);
       if ($filas>0){$error=1; ?>  <script language="JavaScript">  muestra('CODIGO BANCO TIENE TIPO ORDENES ASOCIADA');  </script> <?php }
     }
     if($error==0){ $sfecha=date("y/m/d");$sSQL="SELECT ACTUALIZA_ban002(3,'$cod_banco','','','','','','','00000000','00000000','N','','N','','$sfecha','$afecha',0,0,'','',0,0,'$minf_usuario')"; $resultado=pg_exec($conn,$sSQL); $error=pg_errormessage($conn); $error=substr($error, 0, 61);
       if (!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php }else{?><script language="JavaScript">  muestra('DESACTIVADO EXITOSAMENTE'); </script><?php 
