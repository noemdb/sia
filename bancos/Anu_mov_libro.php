...
if(($tipo_mov=="NDB")or($tipo_mov=="CHQ")){
  ...
  if ($error==0){$sql="SELECT nro_planilla FROM BAN012 Where (Referencia='$referencia') And (Tipo_Mov='$tipo_mov') And (anulada='N')"; $resultado=pg_exec($conn,$sql); $filas=pg_numrows($resultado); if($filas>0){$error=1;?><script language="JavaScript">muestra('Existe Comprobante de Retención Asociado al movimiento');</script><?php } }
} $DOperacion="1";
...
if(($error==0)and($tipo_mov=="CHQ")){$sSQL="SELECT num_cheque,chq_o_f_c,nro_orden_pago,tipo_pago From BAN006 Where (cod_banco='$cod_banco') And (num_cheque='$referencia')"; $resultado=pg_exec($conn,$sSQL);$filas=pg_numrows($resultado);
  if($filas>=1){$reg=pg_fetch_array($resultado); $morden=$reg["nro_orden_pago"]; $tipo_pago=$reg["tipo_pago"]; $chq_o_f_c=$reg["chq_o_f_c"]; if(($chq_o_f_c=="A")or($chq_o_f_c=="O")or($chq_o_f_c=="F")or($chq_o_f_c=="P")or($chq_o_f_c=="B")){$tipo_chq=$chq_o_f_c;}} else{$error=1; ?><script language="JavaScript">  muestra('NÚMERO DE CHEQUE NO EXISTE EN ESTADO DE CHEQUES');  </script><?php }
}
...
if($error==0){?><script language="JavaScript">window.close(); window.opener.location.reload();</script> <?php } else {?>  <script language="JavaScript">history.back();</script> <?php } 
?>
