...
if (pg_last_error($conn)) { ?> <script language="JavaScript">   muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');  </script> <?php }
 else{ if (checkData($fecha_ent)=='1'){$error=0;}else{$error=1; ?> <script language="JavaScript">muestra('FECHA ENTERAR NO ES VALIDA');</script><?php }
  if($error==0){ $sfechaf=formato_aaaammdd($fecha_ent); 
    ...
    if($filas==0){$error=1; ?> <script language="JavaScript"> muestra('NUMERO PLANILLA DE RETENCION NO ENTERADA');</script><?php }
