...
if($error==0){$sfecha=formato_aaaammdd($fecha); if(($sfecha>$Fec_Fin_Ejer)or($sfecha<$Fec_Ini_Ejer)){echo $sfecha; $error=1;?><script language="JavaScript">muestra('FECHA DE MOVIMIENTO INVALIDA, FUERA DEL PERIODO FISCAL');</script><?php } }
if($error==0){$sfechac=formato_aaaammdd($fechac); if($sfecha>$sfechac){echo $fechac; $error=1;?><script language="JavaScript">muestra('FECHA DE MOVIMIENTO MAYOR A FECHA DE CONCILIACION');</script><?php } }
if($error==0){$sfechad=formato_aaaammdd($fecha_desde); if($sfecha<$sfechad){echo $fechac; $error=1;?><script language="JavaScript">muestra('FECHA DE MOVIMIENTO MENOR A FECHA DE CONCILIACION');</script><?php } }
...
if($error==0){$sSQL="SELECT cod_banco FROM BAN005 WHERE cod_banco='$cod_banco' and referencia='$referencia' and tipo_mov_banco='$tipo_mov'"; $resultado=pg_query($sSQL); $filas=pg_num_rows($resultado); if($filas>0){$error=1; ?> <script language="JavaScript"> muestra('MOVIMIENTO EN BANCO YA EXISTE');</script><?php } }
...
if($error==0){if(($tipo_mov=="REV") Or ($tipo_mov=="ANU") Or ($tipo_mov=="ANC") Or ($tipo_mov=="AND")) {$error=1; ?> <script language="JavaScript"> muestra('TIPO DE MOVIMIENTO INVALIDO');</script><?php } }
