<?php include ("../class/conect.php");  include ("../class/funciones.php");  include ("../class/configura.inc"); include ("../presupuesto/Ver_dispon.php");   error_reporting(E_ALL);
...
if($error==0){$sfecha=formato_aaaammdd($fecha);
  if(($sfecha>$Fec_Fin_Ejer)or($sfecha<$Fec_Ini_Ejer)){echo $sfecha; $error=1;?><script language="JavaScript">muestra('FECHA DE CHEQUE INVALIDA');</script><?php }
}
if($error==0){$nmes=substr($fecha,3, 2);  if($periodom<$SIA_Periodo){$periodom=$SIA_Periodo;}
  if($periodom>$nmes){$error=1;?><script language="JavaScript">muestra('FECHA DE MES MENOR A ULTIMO PERIODO CERRADO');</script><?php }
}
...
if($error==0){$sSQL="SELECT ced_rif FROM pre099 WHERE ced_rif='$ced_rif'";  $resultado=pg_exec($conn,$sSQL); $filas=pg_numrows($resultado);
  if($filas==0){$error=1;?><script language="JavaScript">muestra('CÉDULA/RIF BENEFICIARIO NO EXISTE');</script><?php }
}
...
if($error==0){ $nmes=substr($fecha,3, 2); $codc_banco=""; $tasa_idb=0; $cod_cont_idb="";   $total=$monto;
 ...
 if($error==0){$disponible=$saldo_ant_libro; for ($i=1;$i<=$nmes;$i++){$spos=$i; If($i<=9){$spos="0".$spos;} $disponible=$disponible+$registro["deb_libro".$spos] - $registro["cre_libro".$spos]; }
  if($disponible<$monto){if($sobreg_saldo=="N"){$error=1;} echo "Disponible: ".formato_monto($disponible).'  Requerido: '.formato_monto($monto),"<br>"; ?> <script language="JavaScript"> muestra('SOBREGIRA SALDO DEL BANCO');</script><?php }
 }}
}
...
if($error==0){ $unidad_sol=substr($cod_presup,0, $l_cat);
   ...
   if($error==0){$sSQL="Select * from PRE095 WHERE cod_fuente_financ='$fuente_financ'";$resultado=pg_query($sSQL);  $filas=pg_num_rows($resultado);  if($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CÓDIGO DE FUENTE NO EXISTE');</script><?php } }
   if(($error==0)and($monto==0)){$error=1; ?> <script language="JavaScript"> muestra('MONTO CODIGO INVALIDO');</script><?php }
   if(($error==0)and($tipo_imput_presu=="C")){ if($monto_credito>$monto){ $error=1; ?> <script language="JavaScript"> muestra('MONTO CREDITO INVALIDO');</script><?php } }
   ...
   if(($error==0)and($tipo_imput_presu=="C")){  $sSQL="Select * from PRE010 WHERE (referencia_adicion='$ref_imput_presu') and (cod_presup='$cod_presup') and (fuente_financ='$fuente_financ')"; $resultado=pg_query($sSQL); $filas=pg_num_rows($resultado);
     if($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CÓDIGO NO EXISTE EN LA EJECUCIÓN DEL CREDITO ADICIONAL');</script><?php }
      else{ $registro=pg_fetch_array($resultado);if($registro["disponible"]<$monto_credito) {$error=1; $dispon=$registro["disponible"]; $dispon=formato_monto($dispon); ?> <script language="JavaScript"> muestra('Monto Mayor que Disponibilidad del Crédito Adicional, Disponibilidad: <?php  echo $dispon; ?> ');</script><?php } }}
}
...
if($error==0){ $sSQL="Select denominacion,cod_contable from pre001 WHERE cod_presup='$cod_presup' and cod_fuente='$fuente_financ'"; ;$resultado=pg_query($sSQL); $filas=pg_num_rows($resultado);  if($filas>0){$registro=pg_fetch_array($resultado,0); $cod_cuentap=$registro["cod_contable"]; } else{$error=1; ?> <script language="JavaScript"> muestra('LOCALIZANDO CÓDIGO PRESUPUESTARIO');</script><?php }
$resultado=pg_exec($conn,"SELECT INCLUYE_CON010('$codigo_mov','$num_nota','D','$cod_cuentap','00000','',$monto,'D','B','N','03','0','$descripcion')"); $error=pg_errormessage($conn); $error="ERROR GRABANDO: ".substr($error,0,91); if(!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php  $error=1; }
$resultado=pg_exec($conn,"SELECT INCLUYE_CON010('$codigo_mov','$num_nota','C','$codc_banco','00000','',$monto,'D','B','N','03','0','$descripcion')"); $error=pg_errormessage($conn);  $error="ERROR GRABANDO: ".substr($error,0,91); if(!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php  $error=1; }
}
