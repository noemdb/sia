...
if (pg_last_error($conn)) { ?> <script language="JavaScript"> muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');  </script> <?php }
  ...
  if ($filas==0){$error=1; ?><script language="JavaScript">  muestra('MOVIMIENTO EN LIBRO NO EXISTE');  </script><?php }
  ...
  if($error==0){$sSQL="Select descrip_tipo_mov,tipo from ban003 WHERE tipo_movimiento='$tipo_mov'"; $resultado=pg_exec($conn,$sSQL);$filas=pg_numrows($resultado); if($filas==0){$error=1;?><script language="JavaScript">muestra('TIPO DE MOVIMIENTO NO EXISTE');</script><?php } else{$reg=pg_fetch_array($resultado); $tipodc=$reg["tipo"];} }
  if($error==0){if(($tipo_mov=="TRD")Or($tipo_mov=="ANU")Or($tipo_mov=="TRD")Or($tipo_mov=="ANC")Or($tipo_mov=="AND")Or($tipo_mov=="IDB")) {$error=1; ?> <script language="JavaScript"> muestra('TIPO DE MOVIMIENTO NO PUEDE SER ELIMINADO');</script><?php } }
  if($error==0){$nmes=substr($fecha_mov,5, 2);if($periodom<$SIA_Periodo){$periodom=$SIA_Periodo;}
  if($periodom>$nmes){echo $periodom.' '.$nmes.' '.$fecha_mov;$error=1;?><script language="JavaScript">muestra('FECHA DE MES MENOR A ULTIMO PERIODO CERRADO');</script><?php } }
  if(($tipo_mov=="NDB")or($tipo_mov=="CHQ")){
    if ($error==0){$sql="SELECT Ano_Fiscal,Mes_Fiscal,nro_comprobante FROM BAN027 Where (Referencia='$referencia') And (Tipo_Mov='$tipo_mov')"; $resultado=pg_exec($conn,$sql); $filas=pg_numrows($resultado); if($filas>0){$error=1;?><script language="JavaScript">muestra('Existe Comprobante de Retencion IVA Asociado al movimiento');</script><?php } }
    if ($error==0){$sql="SELECT nro_planilla FROM BAN012 Where (Referencia='$referencia') And (Tipo_Mov='$tipo_mov')"; $resultado=pg_exec($conn,$sql); $filas=pg_numrows($resultado); if($filas>0){$error=1;?><script language="JavaScript">muestra('Existe Comprobante de Retencion Asociado al movimiento');</script><?php } }
  }
  if($error==0){ $sSQL="select * from ban010 where cod_banco='$cod_banco' and referencia='$referencia' and tipo_mov_trans='$tipo_mov' and tipo_registro='1'"; $resultado=pg_exec($conn,$sSQL);$filas=pg_numrows($resultado);
     if ($filas>0){$error=1; ?><script language="JavaScript">  muestra('MOVIMIENTO EXISTE EN CONCILIACION COMO TRANSITO');  </script><?php } }
  ...
  if(($error==0)and($valida_c=="S")){ $tipo_comp='B'.$cod_banco;
       ...
       if ($filas>0){ $reg=pg_fetch_array($resultado); $mstatusc=$reg["status"]; if($mstatusc=="A"){$error=1;?><script language="JavaScript">muestra('Comprobante Contable del Movimiento esta Actualizado, debe cambiar a Diferido');</script><?php } }
  }	 
  if($error==0){$DOperacion="1";
      ...
      if(($error==0)and($tipo_mov=="CHQ")){$sSQL="SELECT num_cheque,chq_o_f_c,nro_orden_pago,tipo_pago From BAN006 Where (cod_banco='$cod_banco') And (num_cheque='$referencia')"; $resultado=pg_exec($conn,$sSQL);$filas=pg_numrows($resultado);
        if($filas>=1){$reg=pg_fetch_array($resultado); $morden=$reg["nro_orden_pago"]; $tipo_pago=$reg["tipo_pago"]; $chq_o_f_c=$reg["chq_o_f_c"]; if(($chq_o_f_c=="A")or($chq_o_f_c=="O")or($chq_o_f_c=="P")or($chq_o_f_c=="F")or($chq_o_f_c=="B")){$tipo_chq=$chq_o_f_c;}} else{$error=1; ?><script language="JavaScript">  muestra('NUMERO DE CHEQUE NO EXISTE EN ESTADO DE CHEQUES');  </script><?php }
      }
      ...
      if (!$resultado){$error=1; echo $merror,"<br>"; ?><script language="JavaScript">muestra('<?php  echo $merror; ?>');</script><?php }else{?><script language="JavaScript">  muestra('ELIMINO EXITOSAMENTE'); </script><?php 
