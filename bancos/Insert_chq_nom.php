...
if($error==0){if(strlen($num_cheque)==8){$error=0;} else {$error=1; ?> <script language="JavaScript"> muestra('LONGITUD NUMERO DE CHEQUE INVALIDA');</script><?php } }
...
if($error==0){ $nmes=substr($fecha,3, 2); $codc_banco=""; $tasa_idb=0; $cod_cont_idb="";   $total=$monto; $monto_cheque=$monto;  $monto_bruto=$monto;
 ...
 if($error==0){$disponible=$saldo_ant_libro; for ($i=1;$i<=$nmes;$i++){$spos=$i; If($i<=9){$spos="0".$spos;} $disponible=$disponible+$registro["deb_libro".$spos] - $registro["cre_libro".$spos]; } $balance=$monto-$disponible;
  if(($disponible<$monto)and($balance>0.001)){if($sobreg_saldo=="N"){$error=1;} echo "Disponible: ".formato_monto($disponible).'  Requerido: '.formato_monto($monto),"<br>"; ?> <script language="JavaScript"> muestra('SOBREGIRA SALDO DEL BANCO');</script><?php }
...
if($error==0){$sql="Select * from ORD_PAGO where nro_orden='$nro_orden' and anulado='N'"; $res=pg_query($sql); $filas=pg_num_rows($res); 
	if($filas>=1){ $registro=pg_fetch_array($res,0);   $tipo_caus=$registro["tipo_causado"];  $fecha_ord=$registro["fecha"];
	  ...
	  if($error==0){ $monto_asiento=0; $resultado=pg_exec($conn,"SELECT ELIMINA_CON010('$codigo_mov')"); $error=pg_errormessage($conn); $error=substr($error,0,91); 
		...
		$resultado=pg_exec($conn,"SELECT INCLUYE_CON010('$codigo_mov','$nro_orden','D','$cod_contable_o','00000','',$monto_cheque,'D','B','N','02','0','$descripcion_a')"); $error=pg_errormessage($conn);  $error="ERROR GRABANDO: ".substr($error,0,91); if(!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php  $error=1; }
		$resultado=pg_exec($conn,"SELECT INCLUYE_CON010('$codigo_mov','$nro_orden','C','$codc_banco','00000','',$monto_cheque,'D','B','N','02','0','$descripcion_a')"); $error=pg_errormessage($conn);  $error="ERROR GRABANDO: ".substr($error,0,91); if(!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php  $error=1; }
	  }	  
...
if($error==0){$sql="SELECT * FROM CON010  where codigo_mov='$codigo_mov' order by debito_credito desc,cod_cuenta";  $res=pg_query($sql);  $t_debe=0; $t_haber=0; $balance=0;
  while($registro=pg_fetch_array($res)){if($registro["debito_credito"]=="D"){$t_debe=$t_debe+$registro["monto_asiento"];}else{$t_haber=$t_haber+$registro["monto_asiento"];}}
    ...
    if(($t_debe==0)or($t_haber==0)or($total<>$t_debe)){$error=1; ?> <script language="JavaScript"> muestra('MONTO DEL COMPROBANTE CONTABLE NO VALIDO');</script><?php }
}
...
<?php  if($error==0){?><td width="250"><input name="btEmitir" type="button" id="btEmitir" value="Imprimir Cheque" title="Impresion Cheque Emitido" onclick="javascript:Ventana_chq('<?php  echo $furl; ?>');"></td>
