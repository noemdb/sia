...
if (pg_last_error($conn)){$error=1; ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
 ...
 if($error==0){$sSQL="Select * from NOM015 WHERE cod_tipo_personal='$cod_tipo_personal'"; $resultado=pg_query($sSQL); $filas=pg_num_rows($resultado); if($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO TIPO DE PERSONAL NO EXISTE');</script><?php } }
 if($error==0){$sSQL="Select * from NOM005 WHERE codigo_departamento='$cod_departamento'"; $resultado=pg_query($sSQL); $filas=pg_num_rows($resultado);if($filas==0){$error=1; ECHO $sSQL; ?> <script language="JavaScript"> muestra('CODIGO DE DEPARTAMENTO NO EXISTE');</script><?php }else{$registro=pg_fetch_array($resultado); $des_departamento=$registro["descripcion_dep"];} }
 /*
 if($error==0){if($sueldo==0){$error=1; ?> <script language="JavaScript">muestra('MONTO SUELDO NO ES VALID0');</script><?php } }  */
 ...
 if($error==0){ $sSQL="Select * from NOM068 WHERE codigo_mov='$codigo_mov' and cod_empleado='$cod_empleado' and fecha_asigna='$sfecha'";  $resultado=pg_query($sSQL);  $filas=pg_num_rows($resultado);
  if ($filas>=1){$error=1;  ?> <script language="JavaScript"> muestra('ASIGNACION DE CARGO YA EXISTE');</script><?php }
      ...
      $resultado=pg_exec($conn,$sSQL); $error=pg_errormessage($conn); $error="ERROR GRABANDO: ".substr($error, 0, 61); if (!$resultado){echo $sSQL; ?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php }
  }
}
