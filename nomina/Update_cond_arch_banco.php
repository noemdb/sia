...
if (pg_last_error($conn)){$error=1; ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
  ...
  if($filas==0){$error=1; ?> <script language="JavaScript"> muestra('LINEA DE ARCHIVO NO EXISTE');</script><?php }  else{ $registro=pg_fetch_array($resultado);   $tipo_campo=$registro["tipo_campo"]; }  
  ...
  if($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CONDICION DEL CAMPO DE ARCHIVO NO EXISTE');</script><?php } 
