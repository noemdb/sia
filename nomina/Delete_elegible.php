<?php include ("../class/conect.php");  include ("../class/funciones.php");$cedula=$_GET["Gcedula"];   $fecha_hoy=asigna_fecha_hoy(); $sfechan=formato_aaaammdd($fecha_hoy);
...
if (pg_last_error($conn)){$error=1; ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
  ...
  if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('INFORMACION DE ELEGIBLE NO EXISTE');</script><?php }
