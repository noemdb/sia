<?php include ("../class/conect.php");  include ("../class/funciones.php"); $fecha_hoy=asigna_fecha_hoy();   $error=0; $referencia_comp="00000000"; $tipo_compromiso="0000";  $ref_imput_presu="00000000";  $tipo_imput_presu="P";
...
if (pg_last_error($conn)){$error=1; ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
  ...
  if ($filas==0){ $error=1; ?>  <script language="JavaScript">muestra('CODIGO DE ESTRUCTURA NO EXISTE'); </script><?php }
  ...
  if($error==0){$sql="SELECT * FROM CODIGOS_PRE026 where codigo_mov='$codigo_mov' order by cod_presup"; $res=pg_query($sql);$total=0;
    while($registro=pg_fetch_array($res)){ $monto=$registro["monto"]; $total=$total+$registro["monto"]; $disponible=$registro["disponible"];  $monto_c=$monto; 
      if($registro["tipo_compromiso"]=="0000"){if($registro["monto"]>$registro["disponible"]){$error=0; ?> <script language="JavaScript"> muestra('CODIGO PRESUPUESTARIO : <?php  echo $registro["cod_presup"]; ?> NO TIENE DISPONIBILIDAD');</script><?php } } 
		  ...
		  if ($filasc==0){$error=1; $error=0; ?> <script language="JavaScript"> muestra('NO EXISTE EN EL COMPROMISO EL CODIGO PRESUPUESTARIO:<?php  echo $registro["cod_presup"]; ?> FUENTE:<?php  echo $registro["fuente_financ"]; ?> REF.CREDITO:<?php  echo $registro["ref_imput_presu"]; ?>');</script><?php }
				...
				if(($monto_c>$compromiso)and($diferencia>0.001)){$error=1; $error=0; ?> <script language="JavaScript"> muestra('MONTO A CAUSAR MAYOR AL MONTO DEL CODIGO POR COMPROMETER CODIGO PRESUPUESTARIO:<?php  echo $registro["cod_presup"]; ?> FUENTE:<?php  echo $registro["fuente_financ"]; ?>');</script><?php }
			}
		}
	}
  }
  if($error==0){ $sql="SELECT * FROM COD_RET where codigo_mov='$codigo_mov' order by tipo_retencion,cod_presup_ret"; $res=pg_query($sql);
    while($registro=pg_fetch_array($res)){$monto=$registro["monto_retencion"];  $tipo_retencion=$registro["tipo_retencion"];  $cod_presup=$registro["cod_presup_ret"];  $cod_fuente=$registro["fuente_fin_ret"];   $referencia_comp=$registro["ref_comp_ret"]; $tipo_compromiso=$registro["tipo_comp_ret"];
      ...
      if($filas==0){$error=1;?> <script language="JavaScript">muestra('CODIGO PRESUPUESTARIO <?php  echo $cod_presup; ?> \n ASOCIADO A LA RETENCION <?php  echo $tipo_retencion; ?>, NO TIENE IMPUTACIÓN ');</script><?php } }
  }
...
if($error==0){?><script language="JavaScript"> Ventana_chq('<?php  echo $furl; ?>'); document.location='<?php  echo $url; ?>';</script><?php }else{?><script language="JavaScript">history.back();</script><?php }
?>
