<?php include ("../class/conect.php");  include ("../class/funciones.php");$codigo_departamento=$_GET["txtcodigo"]; $fecha_hoy=asigna_fecha_hoy(); $sfechan=formato_aaaammdd($fecha_hoy);
...
if (pg_last_error($conn)){$error=1; ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php } else{$error=0; }
 if($error==0){$sSQL="Select cod_cargo from NOM006 WHERE cod_departam='$codigo_departamento'"; $resultado=pg_exec($conn,$sSQL);$filas=pg_numrows($resultado); if($filas>0){$error=1;?><script language="JavaScript">muestra('DEPARTAMENTO TIENE TRABAJADORES ASIGNADO');</script><?php } }
  ...
  if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DE DEPARTAMENTO NO EXISTE');</script><?php }
