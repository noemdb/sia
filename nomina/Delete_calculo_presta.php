<?php include ("../class/conect.php");  include ("../class/funciones.php"); $fecha_hoy=asigna_fecha_hoy(); $fecha_tope="30/04/2012"; $fechah=$_GET["fechah"]; $m1=FDate($fechah); $m2=FDate($fecha_tope);  $fechah=formato_aaaammdd($fechah);
...
if($m1>$m2) { $error=0; ?> <script language="JavaScript">muestra('FECHA INVALIDA, FECHA DEBE SER MENOR A: '+'<?php  echo $fecha_tope; ?>');   </script> <?php }
...
while($reg=pg_fetch_array($res)){ $cod_empleado=$reg["cod_empleado"];   $fecha_c=$reg["fecha_calculo"]; $tipo_nomina=$reg["tipo_nomina"]; $tipo_calculo=$reg["tipo_calculo"];  $num_calculo=$reg["num_calculo"];
  ...
  if(($cod_empleado>=$cod_empleado_d)and($cod_empleado<=$cod_empleado_h)and($fecha_c>=$fechah)and($tipo_nomina>=$tipo_nomina_d)and($tipo_nomina<=$tipo_nomina_h)){ $cant_trab=$cant_trab+1;
   ...
   $resultado=pg_exec($conn,$sSQL); $error=pg_errormessage($conn); $error="ERROR ELIMINANDO: ".substr($error, 0, 61); if (!$resultado){$mgraba=1;?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php } }
   //echo $sSQL,"<br>";
}}
pg_close($conn);
if($error==0){?><script language="JavaScript">muestra('FINALIZO ELIMINAR CALCULO, CANTIDAD MOVIMIENTOS: '+'<?php  echo $cant_trab; ?>');  document.location ='<?php  echo $url; ?>'; </script> <?php } else{ ?><script language="JavaScript">document.location ='<?php  echo $url; ?>'; </script> <?php } ?>
