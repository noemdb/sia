...
if (pg_last_error($conn)){$error=1; ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
 ...
 if($error==0){$sSQL="Select tipo_nomina from NOM002 WHERE tipo_nomina='$tipo_nomina'"; $resultado=pg_exec($conn,$sSQL);$filas=pg_numrows($resultado); if($filas>0){$error=1;?><script language="JavaScript">muestra('TIPO DE NOMINA TIENE CONCEPTOS');</script><?php } }
 if($error==0){$sSQL="Select tipo_nomina from NOM003 WHERE tipo_nomina='$tipo_nomina'"; $resultado=pg_exec($conn,$sSQL);$filas=pg_numrows($resultado); if($filas>0){$error=1;?><script language="JavaScript">muestra('TIPO DE NOMINA TIENE FORMULA CONCEPTOS');</script><?php } }
 if($error==0){$sSQL="Select tipo_nomina from NOM011 WHERE tipo_nomina='$tipo_nomina'"; $resultado=pg_exec($conn,$sSQL);$filas=pg_numrows($resultado); if($filas>0){$error=1;?><script language="JavaScript">muestra('TIPO DE NOMINA TIENE ASIGNACION DE CONCEPTOS');</script><?php } }
 if($error==0){$sSQL="Select tipo_nomina from NOM006 WHERE tipo_nomina='$tipo_nomina'"; $resultado=pg_exec($conn,$sSQL);$filas=pg_numrows($resultado); if($filas>0){$error=1;?><script language="JavaScript">muestra('TIPO DE NOMINA TIENE TRABAJADORES RELACIONADO');</script><?php } }
 if($error==0){$sSQL="Select * from NOM001 WHERE tipo_nomina='$tipo_nomina'"; $resultado=pg_query($sSQL);  $filas=pg_num_rows($resultado);
  if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('TIPO DE NOMINA NO EXISTE');</script><?php }
      ...
      $resultado=pg_exec($conn,$sSQL); $error=pg_errormessage($conn); $error="ERROR ELIMINANDO: ".substr($error, 0, 61); if (!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php }else{?><script language="JavaScript">  muestra('ELIMINO EXITOSAMENTE'); </script><?php 
