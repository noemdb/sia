<?php include ("../class/seguridad.inc"); include ("../class/conects.php");  include ("../class/funciones.php");$codigo_cargo=$_GET["txtcodigo_cargo"]; $fecha_hoy=asigna_fecha_hoy(); $sfechan=formato_aaaammdd($fecha_hoy);
...
if (pg_last_error($conn)){$error=1; ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
 ...
 if($error==0){$sSQL="Select cod_empleado,cod_cargo from NOM006 WHERE cod_cargo='$codigo_cargo'"; $res=pg_exec($conn,$sSQL);$filas=pg_numrows($res); 
  if($filas>0){$registro=pg_fetch_array($res,0);  echo "Codigo Tabajador: ".$registro["cod_empleado"];$error=1;?><script language="JavaScript">muestra('CARGO TIENE TRABAJADORES ASIGNADO');</script><?php } }
  if($error==0){$sSQL="Select * from NOM004 WHERE codigo_cargo='$codigo_cargo'"; $resultado=pg_query($sSQL);  $filas=pg_num_rows($resultado);
  if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DE CARGO NO EXISTE');</script><?php }
