...
if (pg_last_error($conn)){$error=1; ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
  ...
  if($error==0){$sSQL="Select cod_empleado from NOM028 WHERE cod_empleado='$cod_empleado' and fecha_sueldo='$fechap'"; $resultado=pg_query($sSQL);  $filas=pg_num_rows($resultado);if($filas>=1){$error=1; ?> <script language="JavaScript"> muestra('SUELDO DE PRESTACIONES YA EXISTE');</script><?php } }
  ...
  if($error==0){ IF($monto_sueldo_adic==0){$error=1; ?> <script language="JavaScript">muestra('MONTO DE SUELDO PRESTACIONES NO VALIDO');</script><?php } }
