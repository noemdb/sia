<?php include ("../class/conect.php");  include ("../class/funciones.php"); $fecha_hoy=asigna_fecha_hoy(); $url="Act_tip_perso_ar.php";  $formato_tipo="XX-XX";
...
if (pg_last_error($conn)){$error=1; ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
  ...
  if($filas>=1){$error=1; ?> <script language="JavaScript"> muestra('TIPO DE PERSONAL YA EXISTE');</script><?php }
  if($error==0){if(strlen($cod_tipo_personal)==strlen($formato_tipo)){$error=0;} else{$error=1; ?> <script language="JavaScript"> muestra('LONGITUD TIPO DE PERSONAL INVALIDA');</script><?php } }
