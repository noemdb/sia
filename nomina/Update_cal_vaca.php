...
if (pg_last_error($conn)){$error=1; ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
  ...
  if($error==0){$sSQL="Select tipo_nomina,descripcion,con_sue_bas,con_compen,g_orden_pago,con_cal_vac,con_bon_vac from NOM001 WHERE tipo_nomina='$tipo_nomina'"; $resultado=pg_query($sSQL); $filas=pg_num_rows($resultado); if($filas==0){$error=1; ?> <script language="JavaScript"> muestra('TIPO DE NOMINA NO EXISTE');</script><?php } else{$registro=pg_fetch_array($resultado); $con_cal_vac=$registro["con_cal_vac"]; $cod_bon_vac=$registro["con_bon_vac"]; $g_orden_pago=$registro["g_orden_pago"]; $cod_conc_s=$registro["con_sue_bas"]; $cod_conc_c=$registro["con_compen"];}}
  ...
  if($error==0){ $afechac=$fechacd;
     ...
     $resultado=pg_exec($conn,$sSQL); $error=pg_errormessage($conn); $merror="ERROR ELIMINANDO: ".substr($error, 0, 61); if (!$resultado){$error=1;?><script language="JavaScript">muestra('<?php  echo $merror; ?>');</script><?php }else{$error=0;}
  } 
