...
if (pg_last_error($conn)){$error=1; ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
 else{$error=0; $sSQL="Select * from NOM021 WHERE numero='$numero'"; $resultado=pg_query($sSQL);  $filas=pg_num_rows($resultado); if($filas==0){$error=1; ?> <script language="JavaScript"> muestra('NÚMERO DE GACETA NO EXISTE');</script><?php }
      ...
      $sSQL="SELECT ACTUALIZA_NOM021(3,'$numero','$adesde','$ahasta',0)";  $resultado=pg_exec($conn,$sSQL); $merror=pg_errormessage($conn); $merror="ERROR ELIMINANDO: ".substr($merror, 0, 91); if (!$resultado){?><script language="JavaScript">muestra('<?php  echo $merror; ?>');</script><?php }else{?><script language="JavaScript">  muestra('ELIMINO EXITOSAMENTE'); </script><?php 
      ...
      $merror=pg_errormessage($conn); $merror=substr($merror, 0, 91);  if (!$resultado){?><script language="JavaScript">muestra('<?php echo $merror;?>');</script><?php } }
  }
}
