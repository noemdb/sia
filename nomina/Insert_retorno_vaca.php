...
if (pg_last_error($conn)){$error=1; ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
  ...
  if($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DE TRABAJADOR NO TIENE SALIDA DE VACACIONES');</script><?php }
  ...
  if(($error==0)and($calcula_nomina=="SI")){ $fecha_desde=formato_ddmmaaaa($ultima_fecha); $fecha_desde=nextDate($fecha_desde,1);
     ...
     if($m2>$m1){ $error=1; echo $fecha_hasta." ".$fecha_reincorp." ".$m1." ".$m2,"<br>"; ?> <script language="JavaScript">muestra('Fecha Actual Calculo de nomina menor a Fecha Reincorporarse');</script><?php }
  }
