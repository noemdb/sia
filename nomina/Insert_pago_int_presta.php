<?php include ("../class/conect.php");  include ("../class/funciones.php"); $fecha_hoy=asigna_fecha_hoy(); $fecha_ley="19/06/2007";   $total_interes=0;  $fecha_calculo="";
...
if (pg_last_error($conn)){$error=1; ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
 else{ if(checkData($fecha_pago)=='1'){$error=0; $fechap=formato_aaaammdd($fecha_pago);} else{$error=1; ?> <script language="JavaScript">muestra('FECHA DE PAGO NO ES VALIDA');</script><?php }
  if($error==0){$sSQL="Select cod_empleado from NOM032 WHERE cod_empleado='$cod_empleado' and fecha_pago='$fechap'"; $resultado=pg_query($sSQL);  $filas=pg_num_rows($resultado);if($filas>=1){$error=1; ?> <script language="JavaScript"> muestra('PAGO DE INTERES YA EXISTE');</script><?php } }
  ...
  $resultado=pg_query($StrSQL);$filas=pg_num_rows($resultado);if($filas>0){$registro=pg_fetch_array($resultado);$total_interes=$registro["total_interes"]; $fecha_calculo=$registro["fecha_calculo"];}else{$error=1;?> <script language="JavaScript"> muestra('CODIGO DE TRABAJADOR NO TIENE CALCULO DE PRESTACIONES');</script><?php } }
  ...
  if($error==0){ $fechap=formato_aaaammdd($fecha_pago); if($fechap<$fecha_calculo){$error=1; ?> <script language="JavaScript">muestra('FECHA DE PAGO NO PUEDE SER MENOR ULTIMA FECHA CALCULO');</script><?php } }
  if($error==0){ if($monto_pago==0){$error=1; ?> <script language="JavaScript">muestra('MONTO DE PAGO NO VALIDO');</script><?php } }
  if($error==0){ IF($monto_pago>$total_interes){$error=1; ?> <script language="JavaScript">muestra('MONTO DE PAGO NO PUEDE SER MAYOR A SALDO DE INTERESES');</script><?php } }
