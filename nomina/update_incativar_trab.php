<?php include ("../class/conect.php"); include ("../class/funciones.php"); $tipo_nomina=$_GET["Gtipo_nomina"]; $fecha_hoy=asigna_fecha_hoy(); $sfecha=formato_aaaammdd($fecha_hoy);
...
if (pg_last_error($conn)){$error=1; ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
	...
	if (!$resultado){echo $resultado; ?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php }else{?><script language="JavaScript">  muestra('ACTUALIZO EXITOSAMENTE'); </script><?php  
