...
if (pg_last_error($conn)){$error=1; ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
 else{ if(checkData($fecha_cal_fin)=='1'){$error=0; $fechap=formato_aaaammdd($fecha_cal_fin);} else{$error=1; ?> <script language="JavaScript">muestra('FECHA DE PAGO NO ES VALIDA');</script><?php   }
  if($error==0){$sSQL="Select cod_empleado from NOM077 WHERE cod_empleado='$cod_empleado' "; $resultado=pg_query($sSQL);  $filas=pg_num_rows($resultado);if($filas>=1){$error=1; ?> <script language="JavaScript"> muestra('CALCULO YA EXISTE PARA ESTE TRABAJADOR');</script><?php } }
  if($error==0){$sql="SELECT NOM006.cod_empleado,NOM006.tipo_nomina,NOM006.fecha_ingreso,NOM006.cedula,NOM006.nombre FROM NOM006 Where cod_empleado='$cod_empleado'"; $resultado=pg_query($sql);  $filas=pg_num_rows($resultado); 
    if($filas==0){$error=1;?> <script language="JavaScript"> muestra('CODIGO DE TRABAJADOR NO EXISTE');</script><?php } 
  ...
  if($error==0){ $fechap=formato_aaaammdd($fecha_cal_fin); if($fechap<$fecha_ing){$error=1; ?> <script language="JavaScript">muestra('FECHA DE CALCULO NO PUEDE SER MENOR FECHA INGRESO');</script><?php } }
  ...
  $resultado=pg_query($StrSQL);$filas=pg_num_rows($resultado);if($filas>0){$registro=pg_fetch_array($resultado); $fecha_cal_garantia=$registro["fecha_calculo"];}else{$error=1;?> <script language="JavaScript"> muestra('CODIGO DE TRABAJADOR NO TIENE CALCULO DE PRESTACIONES');</script><?php } }
  if($error==0){ $fechap=formato_aaaammdd($fecha_cal_fin); if($fechap<$fecha_cal_garantia){$error=1; ?> <script language="JavaScript">muestra('FECHA DE CALCULO NO PUEDE SER MENOR ULTIMA FECHA CALCULO PRESTACIONES');</script><?php } }
  if($error==0){ if($monto_art142==0){$error=1; ?> <script language="JavaScript">muestra('MONTO DE CALCULO NO VALIDO');</script><?php } }
