<?php include ("../class/conect.php");  include ("../class/funciones.php"); $fecha_hoy=asigna_fecha_hoy(); $url="Cierre_nomina.php"; $tipo_nomina=$_GET["tipo_nomina"]; $tp_calculo=$_GET["tp_calculo"]; $num_periodos=$_GET["num_periodos"]; $carga_bono_vac=$_GET["carga_bono_vac"]; $php_os=PHP_OS; $cod_modulo="04";
...
if($filas>0){$registro=pg_fetch_array($resultado); $fechah=$registro["fecha_p_hasta"]; } else{$error=1; ?><script language="JavaScript">muestra('NOMINA NO HA SIDO CALCULADO');</script><?php }
...
$res=pg_exec($conn,$sql); $terror=pg_errormessage($conn); $merror=substr($terror, 0, 91); if (!$res){$error=1;  echo $terror,"<br>"; ?><script language="JavaScript">muestra('ERROR CERRANDO NOMINA');   muestra('<?php echo $merror;?>');</script><?php }else{$error=0;}
...
pg_close($conn); ?><script language="JavaScript"> muestra('<?php  echo $mensaje; ?>'); window.close(); </script>