...
if (pg_last_error($conn)){$error=1; ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
 ...
 if($error==0){ $sSQL="Select * from NOM040 WHERE cod_tipo_personal='$cod_tipo_personal' and grado='$grado' and paso='$paso'";  $resultado=pg_query($sSQL);  $filas=pg_num_rows($resultado);
  if ($filas==0){$error=1;  ?> <script language="JavaScript"> muestra('PASO Y GRADO NO EXISTE EN LA TABLA');</script><?php }
   else{$sfecha=formato_aaaammdd($fecha);
      $sSQL="SELECT ACTUALIZA_NOM040(2,'$cod_tipo_personal','$sfecha','$grado','$paso',$sueldo)"; //echo $sSQL;
      $resultado=pg_exec($conn,$sSQL); $error=pg_errormessage($conn); $error="ERROR GRABANDO: ".substr($error, 0, 91); if (!$resultado){echo $sSQL; ?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php }
  }
}
pg_close($conn);
