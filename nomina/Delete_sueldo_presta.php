...
if (pg_last_error($conn)){$error=1; ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
 else{$error=0; $sSQL="Select cod_empleado,monto_sueldo,monto_sueldo_adic from NOM028 WHERE cod_empleado='$cod_empleado' and fecha_sueldo='$fechap'"; $resultado=pg_query($sSQL);  $filas=pg_num_rows($resultado);if($filas==0){$error=1; ?> <script language="JavaScript"> muestra('SUELDO DE PRESTACIONES NO EXISTE');</script><?php }
