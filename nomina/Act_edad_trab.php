...
if (pg_last_error($conn)){$error=1; ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
      ...
      $error=pg_errormessage($conn); $error="ACTUALIZANDO: ".substr($error, 0, 91); if (!$resultado){echo $resultado; ?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php }else{?><script language="JavaScript">  muestra('ACTUALIZO EXITOSAMENTE'); </script><?php } }
