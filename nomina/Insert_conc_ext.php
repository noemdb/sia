...
if (pg_last_error($conn)){$error=1; ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
 ...
 if($error==0){ $sSQL="Select * from NOM066 WHERE codigo_mov='$codigo_mov' and cod_concepto='$cod_concepto'";
  ...
  if ($filas>=1){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DE CONCEPTO YA EXISTE EN EL CALCULO');</script><?php }
...
}  echo $sSQL;pg_close($conn); if($error==0){?><script language="JavaScript">document.location ='<?php  echo $url; ?>';</script><?php }else{?><script language="JavaScript">history.back();</script><?php }?>
