<?php include ("../class/conect.php");  include ("../class/funciones.php");     $fecha_hoy=asigna_fecha_hoy(); $sfechan=formato_aaaammdd($fecha_hoy);
...
if (pg_last_error($conn)){$error=1; ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
 else{$error=0; $sSQL="Select cod_empleado,monto_adelanto from NOM031 WHERE cod_empleado='$cod_empleado' and fecha_adelanto='$fechap'"; $resultado=pg_query($sSQL);  $filas=pg_num_rows($resultado);if($filas==0){$error=1; ?> <script language="JavaScript"> muestra('ADELANTO DE PRESTACIONES NO EXISTE');</script><?php }     else{$registro=pg_fetch_array($resultado); $monto_adelanto=$registro["monto_adelanto"];} }
