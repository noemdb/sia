...
if (pg_last_error($conn)){$error=1; ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
  ...
  if($filas==0){$error=1; ?> <script language="JavaScript"> muestra('TIPO DE PERSONAL NO EXISTE');</script><?php } else{$registro=pg_fetch_array($resultado);$adescrip=$registro["des_tipo_personal"];}
