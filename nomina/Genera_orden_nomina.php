<?php include ("../class/conect.php");  include ("../class/funciones.php"); $fecha_hoy=asigna_fecha_hoy(); $error=0; $sfecha=formato_aaaammdd($fecha_hoy);   $codigo_mov=$_POST["txtcodigo_mov"];
...
//echo $sql." ".$tp_calculo." ".$filas;
  if($filas>0){$regis=pg_fetch_array($resultado); $fecha_d=$regis["fecha_p_desde"]; $fecha_h=$regis["fecha_p_hasta"]; } else{$error=1;?> <script language="JavaScript">muestra('NO EXISTEN NOMINAS CALCULADAS');</script><?php }
...
$resultado=pg_exec($conn,"SELECT MOD_MONTO_PRE026('$codigo_mov','$cod_presup','$cod_fuente','$referencia_comp','$tipo_compromiso','$ref_imput_presu',$monto,0)"); $error=pg_errormessage($conn); $error="ERROR GRABANDO: ".substr($error, 0, 61); if (!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php }
}else{$error=1;?> <script language="JavaScript">muestra('CODIGO PRESUPUESTARIO <?php  echo $cod_presup." ".$cod_fuente; ?> \n ASOCIADO AL CONCEPTO DE RETENCION, NO TIENE IMPUTACION ');</script><?php } }
...
$resultado=pg_exec($conn,"SELECT ACTUALIZA_PAG030 ('1','$codigo_mov','$codigo_cuenta','$debito_credito','$monto_c')"); $error=pg_errormessage($conn); $error="ERROR GRABANDO: ".substr($error, 0, 61); if (!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php }
}
...
while($reg=pg_fetch_array($res)){ if($i==0){$mcod_empleado=$reg["cod_empleado"]; $mcedula=$reg["cedula"]; $mnombre=$reg["nombre"];}
 if($mcod_empleado<>$reg["cod_empleado"]){    $resultado=pg_exec($conn,"SELECT ACTUALIZA_PAG043(1,'$codigo_mov','$cod_estructura','$mcod_empleado','$mcedula','$mnombre',$mmonto_benef)"); $error=pg_errormessage($conn); $error="ERROR GRABANDO: ".substr($error, 0, 61); if (!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php }
 ...
 $resultado=pg_exec($conn,"SELECT ACTUALIZA_PAG044(1,'$codigo_mov','$cod_estructura','$cod_empleado','$cedula','$cod_presup','$cod_fuente','$referencia_comp','$tipo_compromiso',$monto_c)"); $error=pg_errormessage($conn); $error="ERROR GRABANDO: ".substr($error, 0, 61); if (!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php }
 $i=$i+1;  $mmonto_benef=$mmonto_benef+$monto_c;
}  
if( $mmonto_benef<>0){$resultado=pg_exec($conn,"SELECT ACTUALIZA_PAG043(1,'$codigo_mov','$cod_estructura','$mcod_empleado','$mcedula','$mnombre',$mmonto_benef)"); $error=pg_errormessage($conn); $error="ERROR GRABANDO: ".substr($error, 0, 61); if (!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php } }
}
$mcod_empleado=""; $mcedula=""; $mnombre=""; $mmonto_benef=0; $i=0;
...
if($error==0){?><script language="JavaScript">muestra('INFORMACION ORDEN DE PAGO GENERADA')</script><?php }
}pg_close($conn);
/* */
