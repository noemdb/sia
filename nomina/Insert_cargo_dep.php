...
if (pg_last_error($conn)){$error=1; ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
  ...
  if($filas>=1){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DE CARGO YA EXISTE EN EL DEPARTAMENTO');</script><?php }
  ...
  if($error==0){if(strlen($codigo_cargo)==strlen($formato_cargo)){$error=0;} else{$error=1; ?> <script language="JavaScript"> muestra('LONGITUD CODIGO DE CARGO INVALIDA');</script><?php } }
  if($error==0){$sSQL="Select * from NOM004 WHERE codigo_cargo='$codigo_cargo'"; $resultado=pg_query($sSQL); $filas=pg_num_rows($resultado); if($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DE CARGO NO EXISTE');</script><?php } }
