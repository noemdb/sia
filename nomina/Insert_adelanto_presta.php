<?php include ("../class/conect.php");  include ("../class/funciones.php"); $fecha_hoy=asigna_fecha_hoy(); $fecha_ley="19/06/2007";   $saldo_prestaciones=0;  $fecha_calculo="";
...
if (pg_last_error($conn)){$error=1; ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
 else{ if(checkData($fecha_adelanto)=='1'){$error=0;$fechap=formato_aaaammdd($fecha_adelanto);  } else{$error=1; ?> <script language="JavaScript">muestra('FECHA DE PAGO NO ES VALIDA');</script><?php }
  if($error==0){$sSQL="Select cod_empleado from NOM031 WHERE cod_empleado='$cod_empleado' and fecha_adelanto='$fechap'"; $resultado=pg_query($sSQL);  $filas=pg_num_rows($resultado);if($filas>=1){$error=1; ?> <script language="JavaScript"> muestra('PAGO DE INTERES YA EXISTE');</script><?php } }
  ...
  if($error==0){ $fechap=formato_aaaammdd($fecha_adelanto); if($fechap<$fecha_calculo){$error=1; ?> <script language="JavaScript">muestra('FECHA DE ADELANTO NO PUEDE SER MENOR ULTIMA FECHA CALCULO');</script><?php } }
  if($error==0){ if($monto_adelanto==0){$error=1; ?> <script language="JavaScript">muestra('MONTO DE ADELANTO NO VALIDO');</script><?php } }

  if($error==0){ if($monto_adelanto>$saldo_prestaciones){$error=1; ?> <script language="JavaScript">muestra('MONTO DE ADELANTO NO PUEDE SER MAYOR A SALDO DE PRESTACIONES');</script><?php } }
