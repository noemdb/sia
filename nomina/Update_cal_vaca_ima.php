...
if (pg_last_error($conn)){$error=1; ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
  ...
  if($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DE TRABAJADOR NO TIENE CALCULO DE VACACIONES');</script><?php }
  if($error==0){if(checkData($fecha_causa_desde)=='1'){$error=0; $fechacd=formato_aaaammdd($fecha_causa_desde);} else{$error=1; ?> <script language="JavaScript">muestra('FECHA DE CAUSADO NO ES VALIDA');</script><?php } }
  if($error==0){if(checkData($fecha_causa_hasta)=='1'){$error=0; $fechach=formato_aaaammdd($fecha_causa_hasta);} else{$error=1; ?> <script language="JavaScript">muestra('FECHA DE CAUSADO NO ES VALIDA');</script><?php } }
  ...
  if($error==0){if(checkData($fecha_calculo_d)=='1'){$error=0; $fechaad=formato_aaaammdd($fecha_calculo_d);} else{$error=1; ?> <script language="JavaScript">muestra('FECHA DE CALCULO NO ES VALIDA');</script><?php } }
  ...
  if($error==0){$sql="SELECT * FROM TRABAJADORES Where cod_empleado='$cod_empleado'"; $resultado=pg_query($sql);  $filas=pg_num_rows($resultado); if($filas==0){$error=1;?> <script language="JavaScript"> muestra('CODIGO DE TRABAJADOR NO EXISTE');</script><?php }
	...
	if($error==0){$sSQL="Select denominacion from NOM004 WHERE codigo_cargo='$cod_cargo'"; $resultado=pg_query($sSQL); $filas=pg_num_rows($resultado); if($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DE CARGO NO EXISTE');</script><?php } else{$registro=pg_fetch_array($resultado); $des_cargo=$registro["denominacion"];}}
    if($error==0){$sSQL="Select descripcion_dep from NOM005 WHERE codigo_departamento='$cod_departam'"; $resultado=pg_query($sSQL); $filas=pg_num_rows($resultado);if($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DE DEPARTAMENTO NO EXISTE');</script><?php }else{$registro=pg_fetch_array($resultado); $des_departamento=$registro["descripcion_dep"];}}
  
  }
  if($error==0){$sSQL="Select tipo_nomina,descripcion,con_sue_bas,con_compen,g_orden_pago,con_cal_vac,con_bon_vac,con_liqui2 from NOM001 WHERE tipo_nomina='$tipo_nomina'"; $resultado=pg_query($sSQL); $filas=pg_num_rows($resultado); if($filas==0){$error=1; ?> <script language="JavaScript"> muestra('TIPO DE NOMINA NO EXISTE');</script><?php } else{$registro=pg_fetch_array($resultado); $con_cal_vac=$registro["con_cal_vac"]; $cod_bon_vac=$registro["con_bon_vac"];  $cod_bon_vac_a=$registro["con_liqui2"];  $g_orden_pago=$registro["g_orden_pago"]; $cod_conc_s=$registro["con_sue_bas"]; $cod_conc_c=$registro["con_compen"];}}
  if($error==0){$sSQL="Select cod_concepto from NOM002 WHERE tipo_nomina='$tipo_nomina' and cod_concepto='$cod_bon_vac'"; $resultado=pg_query($sSQL);  $filas=pg_num_rows($resultado); if($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DE CONCEPTO BONO VACACIONAL NO EXISTE');</script><?php } }
  if(($error==0)and($calcula_nomina=="NO")){   
	...
	if($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DE CONCEPTO BONO VACACIONAL NO ASIGNADO AL TRABAJADOR');</script><?php } 
	  ...
	  $sSQL="SELECT ACTUALIZA_NOM076(1,'$codigo_mov','$cod_concepto','$den_concepto','$calculable','$asignacion','$acumula','$oculto','$tipo_a','$asig_ded_apo','$frec_valida','$prestamo','$concepto_vac','$int_cal_vac',$acumulado,$saldo,$valore,$valorq,$valoru,$valorv,$valorw,$valorx,$valory,$valorz,$cantidad,$valor,$monto_orig,'$cod_presup','$cod_contable','$afecta_presup','$cod_retencion','$fecha_ini','$fecha_exp','$fechadh',$frecuenciaa,'$cod_orden')"; $resultado=pg_exec($conn,$sSQL);$merror=pg_errormessage($conn); $merror=substr($merror,0,70);if(!$resultado){?><script language="JavaScript">muestra('<?php  echo $merror; ?>');</script><?php  $error=11;}
    }
	...
	if($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DE CONCEPTO BONO VACACIONAL ADICIONAL NO ASIGNADO AL TRABAJADOR');</script><?php } 
	 else{$rega=pg_fetch_array($resa); $cod_concepto=$rega["cod_concepto"]; $den_concepto=$rega["denominacion"]; $cod_orden=$rega["cod_orden"]; $fecha_exp=$rega["fecha_exp"]; $fecha_ini=$rega["fecha_ini"]; $frecuenciaa=$rega["frecuenciaa"]; $frecuencia=$rega["frecuencia"]; $frec_valida="S";  $calculable=$rega["calculable"]; $statusa=$rega["status"]; $concepto_vac="S";
	  $calculable="NO"; $asignacion=$rega["asignacion"]; $oculto=$rega["oculto"]; $acumula=$rega["acumula"]; $tipo_a=$rega["tipo_asigna"]; $asig_ded_apo=$rega["asig_ded_apo"]; $prestamo=$rega["prestamo"]; $int_cal_vac=substr($statusa,0,1); $cantidad=$rega["cantidad"]; $monto_orig=$rega["monto"]; $acumulado=$rega["acumulado"]; $saldo=$rega["saldo"]; $cod_contable=$rega["cod_contable"];$cod_presup=$rega["cod_presup"]; $afecta_presup=$rega["afecta_presup"]; $cod_retencion=$rega["cod_retencion"];
  ...
  if($error==0){ $afechac=$fechacd;
     ...
     $resultado=pg_exec($conn,$sSQL); $merror=pg_errormessage($conn); $merror="ERROR ELIMINANDO: ".substr($merror, 0, 91); if (!$resultado){$error=1;?><script language="JavaScript">muestra('<?php  echo $merror; ?>');</script><?php }else{$error=0;}
  } 
  if($error==0){    $tipo_pago=substr($tipo_pago,0,3);    
     ...
     $resultado=pg_exec($conn,$sSQL); $merror=pg_errormessage($conn); $merror="ERROR GRABANDO: ".substr($merror, 0, 91); if (!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php }else{$error=0;?><script language="JavaScript">muestra('INCLUYO EXITOSAMENTE');</script><?php   }  
    echo $sSQL;  
  } 
}
pg_close($conn);
