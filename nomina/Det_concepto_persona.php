...
<body>
 <table width="845" border="0" cellspacing="0" cellpadding="0">
   ...
   <tr>
     <td>
<?php $sql="SELECT NOM006.cod_empleado,NOM006.cedula,NOM006.nombre,NOM006.tipo_nomina,NOM006.status,NOM006.calculo_grupos,NOM006.fecha_ingreso,NOM006.cod_categoria FROM NOM006 Where (NOM006.status='ACTIVO' or NOM006.status='VACACIONES' or NOM006.status='REPOSO') And (NOM006.tipo_nomina='$tipo_nomina') And (NOM006.cod_empleado not in (select NOM011.cod_empleado from NOM011 Where NOM011.tipo_nomina='$tipo_nomina' and NOM011.cod_concepto='$cod_concepto')) ORDER BY NOM006.cod_empleado";   $res=pg_query($sql);
?>
...
<?php  while($registro=pg_fetch_array($res)){ $sfecha=$registro["fecha_ingreso"];  $fecha = substr($sfecha,8,2)."/".substr($sfecha,5,2)."/".substr($sfecha,0,4); ?>
         <tr bgcolor='#FFFFFF' bordercolor='#000000' height="20" class="Estilo5" onMouseOver="this.style.backgroundColor='#CCCCCC';this.style.cursor='hand';" onMouseOut="this.style.backgroundColor='#FFFFFF'"o"];" onDblClick="javascript:Llama_Modificar('<?php  echo $codigo; ?>','<?php  echo $registro["cod_empleado"]; ?>');">
           <td width="100" align="left"><?php  echo $registro["cod_empleado"]; ?></td>
           <td width="90" align="left"><?php  echo $registro["cedula"]; ?></td>
           <td width="440" align="left"><?php  echo $registro["nombre"]; ?></td>
           <td width="90" align="left"><?php  echo $fecha; ?></td>
           <td width="90" align="left"><?php  echo $registro["cod_categoria"]; ?></td>
           </tr>
         <?php }?>
       </table></td>
   </tr>
