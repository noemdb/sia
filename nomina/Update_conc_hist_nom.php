...
if (pg_last_error($conn)){$error=1; ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
 ...
 if($filas==0){$error=1; ?> <script language="JavaScript"> muestra('HISTORICO DE CONCEPTO DE NOMINA CALULADA NO EXISTE');</script><?php } else{$registro=pg_fetch_array($resultado); $acantidad=$registro["cantidad"]; $amonto=$registro["monto"]; $valorz=$registro["valorz"];  $asig_ded_apo=$registro["asig_ded_apo"]; }
 if(($error==0)and($monto<>$amonto)and($amonto<>0)){ $sfecha=formato_aaaammdd($fecha_hoy); $sfechan=formato_ddmmaaaa($fecha_p_hasta);	 
	...
	$resultado=pg_exec($conn,$sqlg); $error=pg_errormessage($conn); $error="ERROR GRABANDO: ".substr($error, 0, 61); if (!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php }else{$error=0;?><script language="JavaScript">muestra('MODIFICO EXITOSAMENTE');</script><?php 
