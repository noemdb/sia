...
if (pg_last_error($conn)){ ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
  ...
  if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('INFORMACION CURRICULAR NO EXISTE');</script><?php }
      ...
      $resultado=pg_exec($conn,$sSQL); $error=pg_errormessage($conn); $error="ERROR MODIFICANDO: ".substr($error, 0, 61); if (!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php }
  }
}
