...
if (pg_last_error($conn)){$error=1; ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
    ...
    while($reg=pg_fetch_array($res)){ $cod_empleado=$reg["cod_empleado"]; $fechap=$reg["fecha_sueldo"];  $asueldo=$reg["monto_sueldo"]; $asueldoa=$reg["monto_sueldo_adic"]; $fecha_sueldo=formato_ddmmaaaa($fechap);	
      ...
      $resultado=pg_exec($conn,$sSQL); $merror=pg_errormessage($conn); $merror="ERROR GRABANDO: ".substr($merror, 0,91); if (!$resultado){$error=1;?><script language="JavaScript">muestra('<?php  echo $merror; ?>');</script><?php }else{$error=0;
 ...
 }pg_close($conn);  $url="Act_sueldo_prestaciones_yac.php?Gcriterio=C".$fechab.$cod_empleado;
