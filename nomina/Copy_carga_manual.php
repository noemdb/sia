<?php include ("../class/conect.php");  include ("../class/funciones.php");$tipo_nomina=$_POST["txttipo_nomina"]; $cod_concepto=$_POST["txtcod_concepto"]; $criterio=$_POST["txtcriterio"];
...
if (pg_last_error($conn)){$error=1; ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
   ...
   while(($registro=pg_fetch_array($res))and($error==0)){ $opcion=0; $valido=0; $cod_empleado=$registro["cod_empleado"]; $frec=$registro["frecuenciaa"]; $cantidad=$registro["cantidad"]; $monto=$registro["monto"];
     ...
     if($valido==1){ $sSQL="SELECT ACT_MOVIMIENTO_NOM011($opcion,'$tipo_nomina','$cod_empleado','$con_des',$cantidad,$monto,'SI','SI','$frec')";
      $resultado=pg_exec($conn,$sSQL); $error=pg_errormessage($conn); $error="ERROR GRABANDO: ".substr($error, 0, 61); if (!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php }else{$error=0; $cant_mod=$cant_mod+1;} }
   }
}pg_close($conn); if($error==0){?><script language="JavaScript">muestra('COPIO EXITOSAMENTE, CATIDAD: '+'<?php  echo $cant_mod; ?>'); document.location='<?php  echo $url; ?>';</script><?php }else{?><script language="JavaScript">history.back();</script><?php }?>
