<?php include ("../class/conect.php");  include ("../class/funciones.php"); $equipo = getenv("COMPUTERNAME"); $minf_usuario=$usuario_sia." ".$equipo." ".date("d/m/y H:i a");   $fecha_hoy=asigna_fecha_hoy();
...
if (pg_last_error($conn)){$error=1; ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
    ...
    $sSQL="Select cod_concepto from NOM002 WHERE tipo_nomina='$tipo_desde' and cod_concepto='$cod_sueldo'"; $resultado=pg_query($sSQL);  $filas=pg_num_rows($resultado); if($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DE CONCEPTO SUELDO NO EXISTE');</script><?php }
	...
	/*
	if($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DE CONCEPTO RETROACTIVO NO EXISTE');</script><?php }
	*/
	if($filas>=1){ $existe_ret=1; }
	if(($usar=="SUELDO CARGO")or($usar=="TABLA GRADOS Y PASOS")) { $error=$error; } else{
    if($sueldo_actual==0){$error=1; ?> <script language="JavaScript"> muestra('MONTO DE SUELDO ACTUAL INVALIDO');</script><?php }
	if($sueldo_nuevo==0){$error=1; ?> <script language="JavaScript"> muestra('MONTO DE SUELDO NUEVO INVALIDO');</script><?php } }
	if($error==0){if(checkData($fecha_asigna)=='1'){$error=0;} else{$error=1; ?> <script language="JavaScript">muestra('FECHA ASIGNACION NO ES VALIDA');</script><?php } }
    if($error==0){ $diferencia=$sueldo_nuevo-$sueldo_actual;  $sfechaa=formato_aaaammdd($fecha_asigna); 
	  ...
	  while($reg=pg_fetch_array($res)){ 
		 ...
		 $sSQL="Select denominacion,sueldo_cargo from NOM004 WHERE codigo_cargo='$cod_cargo'"; $resultado=pg_query($sSQL); $filas=pg_num_rows($resultado); if($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DE CARGO NO EXISTE');</script><?php } else{$registro=pg_fetch_array($resultado); $des_cargo=$registro["denominacion"]; $sueldo_cargo=$registro["sueldo_cargo"]; }
         $sSQL="Select descripcion_dep from NOM005 WHERE codigo_departamento='$cod_departam'"; $resultado=pg_query($sSQL); $filas=pg_num_rows($resultado);if($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DE DEPARTAMENTO NO EXISTE');</script><?php }else{$registro=pg_fetch_array($resultado); $des_departamento=$registro["descripcion_dep"];}
		 ...
		 if(($diferencia>0)and($sueldo_nuevo>0)){
		   ...
		   if(!$resultado){ echo "Actualizando Trabajador ".$cod_empleado." ".$errore,"<br>"; ?><script language="JavaScript">muestra('<?php  echo substr($errore,0,70); ?>');</script><?php   }
		   else{
		    if($existe_ret==1){
              ...
              $resultado=pg_exec($conn,$sSQL); $errore=pg_errormessage($conn); 
              if(!$resultado){ echo $sSQL,"<br>"; echo "Actualizando Retroactivo Trabajador ".$cod_empleado,"<br>"; ?><script language="JavaScript">muestra('<?php  echo substr($errore,0,90); ?>');</script><?php   }		   
            }
		   }	
         }
	  }	
...
pg_close($conn); 
if($error==0){?><script language="JavaScript"> muestra('Proceso Finalizado'); window.close(); window.opener.location.reload(); </script> <?php }else{?><script language="JavaScript">history.back();</script><?php }
?> 
