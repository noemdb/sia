...
if (pg_last_error($conn)){ ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
  ...
  if ($filas>0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO CONCEPTO YA EXISTE EN LA LIQUIDACION');</script><?php }
  if($error==0){if(checkData($fecha_ini)=='1'){$error=0;} else{$error=1; ?> <script language="JavaScript">muestra('FECHA INCIO NO ES VALIDA');</script><?php } }
  if($error==0){if(checkData($fecha_hasta)=='1'){$error=0;} else{$error=1; ?> <script language="JavaScript">muestra('FECHA HASTA NO ES VALIDA');</script><?php } }   
  if($error==0){  $sfechai=formato_aaaammdd($fecha_ini); $sfechah=formato_aaaammdd($fecha_hasta); $cod_presup=""; $cod_contable=""; $afecta_presup=""; $cod_retencion=""; if($asig_ded_apo=="ASIGNACION"){ $asignacion="SI"; $asig_ded_apo="A"; } else { $asignacion="NO"; $asig_ded_apo="D";} 
	 ...
	 $error="ERROR GRABANDO: ".substr($error, 0, 61); if (!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php  $error=1;} echo $sSQL;
   }
}pg_close($conn);  
