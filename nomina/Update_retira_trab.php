...
if (pg_last_error($conn)){$error=1; ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
  ...
  if($error==0){if(checkData($fecha_eg)=='1'){$error=0;} else{$error=1; ?> <script language="JavaScript">muestra('FECHA DE EGRESO NO ES VALIDA');</script><?php } }
  if($error==0){$sSQL="Select tipo_nomina from NOM017 WHERE tipo_nomina='$tipo_nomina' and cod_empleado='$cod_empleado'"; $resultado=pg_query($sSQL); $filas=pg_num_rows($resultado); if($filas>=1){$error=1; ?> <script language="JavaScript"> muestra('TRABAJADOR TIENE NOMINA CALCULADA');</script><?php } }
  if($error==0){$sfechae=formato_aaaammdd($fecha_eg); $sSQL="SELECT RETIRA_TRABAJADOR($opcion,'$cod_empleado','$cedula','$fecha_asig','$nombre','$nacionalidad','$sfechae','$motivo','$minf_usuario')"; echo $sSQL; $resultado=pg_exec($conn,$sSQL); $error=pg_errormessage($conn);
    $error="ACTUALIZANDO: ".substr($error, 0, 61); if (!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php }else{?><script language="JavaScript">  muestra('RETIRO DE TRABAJADOR, SE REGISTRO EXITOSAMENTE'); </script><?php   $sfecha=formato_aaaammdd($fecha_hoy);
...
pg_close($conn);if($error==0){?><script language="JavaScript">window.close(); window.opener.location.reload(); </script> <?php }else{?><script language="JavaScript">history.back();</script><?php }?>
