...
<?php  $dia=substr($fecha_hoy,0,2); $error=0; $fecha_hasta=$fecha_hoy;
...
$sSQL="Select cod_empleado,fecha_reincorp from NOM024 Where (fecha_reincorp<='$fechah')";$resultado=pg_query($sSQL);$filas=pg_num_rows($resultado); if($filas>=1){$error=1;?><script language="JavaScript">muestra('Existen Trabajadores en Vacaciones, los Cuales deben ser Retornados');</script><?php 
...
$sSQL="Select cod_empleado,fecha_desde FROM NOM022 Where (fecha_desde<='$fechah') ";$resultado=pg_query($sSQL);$filas=pg_num_rows($resultado);if($filas>=1){$error=1;?><script language="JavaScript">muestra('Existen Trabajadores con Calculo Vacaciones, los Cuales deben tener Salida de Vacaciones');</script><?php 
  echo "","<br>";
...
$error=0;
/*
if($error==1){?><script language="JavaScript">history.back();</script><?php }
*/
pg_close($conn);?>
<body>
