...
if (pg_last_error($conn)){$error=1; ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
 else{ if(checkData($fechad)=='1'){$error=0;} else{$error=1; ?> <script language="JavaScript">muestra('FECHA DESDE NO ES VALIDA');</script><?php } }
 if($error==0){if(checkData($fechah)=='1'){$error=0;} else{$error=1; ?> <script language="JavaScript">muestra('FECHA HASTA NO ES VALIDA');</script><?php } }
 if($error==0){
  ...
  if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('EXPERIENCIA LABORAL NO EXISTE');</script><?php }
      ...
      $resultado=pg_exec($conn,$sSQL); $error=pg_errormessage($conn); $error="ERROR ACTUALIZANDO: ".substr($error, 0, 61); if (!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php }
  }
}
