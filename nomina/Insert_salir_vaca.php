<?php include ("../class/conect.php");  include ("../class/funciones.php"); $fecha_hoy=asigna_fecha_hoy(); $fecha_ley="19/06/2007"; $equipo=getenv("COMPUTERNAME"); $minf_usuario=$usuario_sia." ".$equipo." ".date("d/m/y H:i a");echo "ESPERE POR FAVOR INCLUYENDO....","<br>";
...
if (pg_last_error($conn)){$error=1; ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
  ...
  if(($error==0)and($calcula_nomina=="SI")){ $frec=$registro["frecuencia"]; $fecha_desde=formato_ddmmaaaa($ultima_fecha); $fecha_hasta=formato_ddmmaaaa($ultima_fecha); $fecha_desde=nextDate($fecha_desde,1);
     ...
     if($m1>=$m2){ $error=0;} else{ $error=1;  ?> <script language="JavaScript">muestra('Fecha Actual Calculo de nomina menor a Salida de Vacaciones');</script><?php }
  }
  if(($error==0)and($calcula_nomina=="SI")){ $sSQL="Select tipo_nomina from nom019 where tp_calculo='E' and concepto_vac='S' and tipo_nomina='$tipo_nomina' and cod_empleado='$cod_empleado' and fecha_p_hasta>='$fechaad'"; $resultado=pg_query($sSQL);  $filas=pg_num_rows($resultado);  //echo $sSQL." ".$filas;  
	  if($filas==0){$error=1;?> <script language="JavaScript"> muestra('CODIGO DE TRABAJADOR NO TINE NOMINA DE VACACIONES CERRADA');</script><?php }
  }
