...
if (pg_last_error($conn)){$error=1; ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
  ...
  if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('TABLA DE INDEMNIZACION PARA ESTA NOMINA NO EXISTE');</script><?php }
   if($error==0){$sSQL="Select tipo_nomina from NOM001 WHERE tipo_nomina='$tipo_new'"; $resultado=pg_query($sSQL); $filas=pg_num_rows($resultado); if($filas==0){$error=1;?><script language="JavaScript">muestra('TIPO DE NOMINA A COPIAR NO EXISTE');</script><?php } }
   if($error==0){ if ($gnomina=="00"){$error=0;} else {  if($tipo_new<>$gnomina) {$error=1; ?> <script language="JavaScript"> muestra('TIPO DE NOMINA NO ACTIVA PARA EL USUARIO');</script><?php } } } 
   if($error==0){$sSQL="Select consecutivo from NOM020 WHERE tipo_nomina='$tipo_new'"; $resultado=pg_query($sSQL);  $filas=pg_num_rows($resultado); if ($filas>=1){$error=1; ?> <script language="JavaScript"> muestra('TABLA DE INDEMNIZACION PARA ESTA NOMINA YA EXISTE');</script><?php } }
    ...
    $resultado=pg_exec($conn,$sSQL); $error=pg_errormessage($conn); $error="ERROR GRABANDO: ".substr($error, 0, 61); if (!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php }else{$error=0;?><script language="JavaScript">muestra('COPIO EXITOSAMENTE');</script><?php }
  }
}
