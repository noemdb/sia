...
if (($nro_comprobante=="")or(strlen($nro_comprobante)<8)){$error=1; ?> <script language="JavaScript">muestra('NÚMERO DE COMPROBANTE NO VALIDO');</script><?php }
...
if (pg_last_error($conn)){ ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
    ...
    if ($error==0){$nmes=$mes_fiscal; if($SIA_Periodo>$nmes){$error=1;?><script language="JavaScript">muestra('FECHA DE COMPROBANTE MENOR A ULTIMO PERIODO CERRADO');</script><?php } }
    if($error==0){
      ...
      if ($filas==0){$error=1; echo $sql; ?> <script language="JavaScript"> muestra('NÚMERO DE COMPROBANTE NO EXISTE');</script><?php }
      else{$sSQL="SELECT ELIMINA_BAN027('$ano_fiscal','$mes_fiscal','$nro_comprobante')"; $resultado=pg_exec($conn,$sSQL); $error=pg_errormessage($conn); $error="ERROR ELIMINANDO: ".substr($error, 0, 61);  if (!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php } }
    }
