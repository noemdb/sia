...
if (pg_last_error($conn)) { ?> <script language="JavaScript"> muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');  </script> <?php }
  ...
  if ($filas==0){ $error=1; ?>  <script language="JavaScript">  muestra('TIPO DE OREDN NO EXISTE');  </script> <?php }
     ...
     if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CÓDIGO CONTABLE NO EXISTE');</script><?php }
      else{ $registro=pg_fetch_array($resultado);
        if ($registro["cargable"]=="N"){$error=1; ?> <script language="JavaScript"> muestra('CÓDIGO CONTABLE NO ES CARGABLE');</script><?php }
     }
     if($error==0){$fecha=asigna_fecha_hoy(); $sfecha=formato_aaaammdd($fecha);        $sSQL="SELECT ACTUALIZA_PAG008(2,'$tipo_orden','$des_tipo_orden','$cod_contable','$cod_banco','$gen_tributo','','','$minf_usuario')";
       $resultado=pg_exec($conn,$sSQL);$error=pg_errormessage($conn);$error=substr($error,0,91); if (!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php }
        else{?><script language="JavaScript">  muestra('MODIFICO EXITOSAMENTE'); </script><?php } $desc_doc="TIPO DE ORDEN:".$tipo_orden.", DESCRIPCION:".$ades_tipo_orden.", CODIGO CUENTA:".$acod_contable;
