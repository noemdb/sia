...
if (pg_last_error($conn)){ ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
    ...
    if ($balance>0.001){$error=1; echo $tmonto.' '.$monto_c.' '.formato_monto($balance),"<br>"; ?> <script language="JavaScript"> muestra('TOTAL CUENTAS NO CUADRA');</script><?php }
    if(($error==0)and($codigo1=="")){ $error=1; ?><script language="JavaScript">muestra('CODIGO DE CUENTA 1 NO VALIDO');</script><?php }
    if($error==0){$sSQL="Select * from con001 WHERE codigo_cuenta='$codigo1'";$resultado=pg_query($sSQL);$filas=pg_num_rows($resultado);
      if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DE CUENTA  1 NO EXISTE');</script><?php }
      else{$registro=pg_fetch_array($resultado);
        if ($registro["cargable"]=="N"){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DE CUENTA 1 NO ES CARGABLE');</script><?php }
      }
    }
	if(($error==0)and($codigo2=="")){ $error=1; ?><script language="JavaScript">muestra('CODIGO DE CUENTA 2 NO VALIDO');</script><?php }
    if($error==0){$sSQL="Select * from con001 WHERE codigo_cuenta='$codigo2'";$resultado=pg_query($sSQL);$filas=pg_num_rows($resultado);
      if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DE CUENTA  2 NO EXISTE');</script><?php }
      else{$registro=pg_fetch_array($resultado);
        if ($registro["cargable"]=="N"){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DE CUENTA 2 NO ES CARGABLE');</script><?php }
      }
    }
	if(($error==0)and($codigo3<>"")){$sSQL="Select * from con001 WHERE codigo_cuenta='$codigo3'";$resultado=pg_query($sSQL);$filas=pg_num_rows($resultado);
      if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DE CUENTA 3 NO EXISTE');</script><?php }
      else{$registro=pg_fetch_array($resultado);
        if ($registro["cargable"]=="N"){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DE CUENTA 3 NO ES CARGABLE');</script><?php }
      }
    }
	if($error==0){ $sql="SELECT INCLUYE_CON022('$codigo_mov','$cod_presup','$fuente_financ','$referencia_comp','$tipo_compromiso','','0000','$tipo_imput_presu','$ref_imput_presu','$cod_contable',$monto,'$codigo1',$monto1,'$codigo2',$monto2,'$codigo3',$monto3,'$codigo4',$monto4,'$codigo5',$monto5)";
      $resultado=pg_exec($conn,$sql);      $error=pg_errormessage($conn);  $error=substr($error, 0, 61); if (!$resultado){ ?> <script language="JavaScript">  muestra('<?php  echo $error; ?>'); </script> <?php }
      echo $sql;
    }
}
pg_close($conn);
