...
if(pg_last_error($conn)) { ?> <script language="JavaScript"> muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');  </script> <?php }
  ...
  if($filas==0){$error=1;?><script language="JavaScript">muestra('NUMERO DE ORDEN NO EXISTE');</script><?php }
     ...
     if($anulado=='S'){$error=1;?><script language="JavaScript">muestra('ORDEN YA ESTA ANULADA');</script><?php }
     if($error==0){$sfecha=$fecha_causado;if(checkData($fecha_anu)=='1'){$error=0;$afecha=formato_aaaammdd($fecha_anu);} else{$error=1; ?> <script language="JavaScript">muestra('FECHA ANULACION NO ES VALIDA');</script><?php } }
     if($error==0){if($afecha<$sfecha){$error=1; ?> <script language="JavaScript">muestra('FECHA ANULACION NO PUEDE SER MENOR A FECHA DE LA ORDEN');</script><?php } }
	 ...
	 if($status=="I"){$error=1;?><script language="JavaScript">muestra('ORDEN DE PAGO ESTA CANCELADA');</script><?php }
     if($status=="L"){$error=1;?><script language="JavaScript">muestra('ORDEN DE PAGO ESTA LIBERADA');</script><?php }
     if($retencion=="S"){$error=1;?><script language="JavaScript">muestra('ORDEN DE PAGO ES UNA RETENCION');</script><?php }
     if($total_ajuste>0){$error=1;?><script language="JavaScript">muestra('ORDEN DE PAGO TIENE AJUSTE');</script><?php }
     if(($error==0)and(substr($tipo_causado,0,1)=="A")){$error=1;?><script language="JavaScript">muestra('ORDEN DE PAGO NO PUEDE SER ANULADA');</script><?php }
     ...
     if($error==0){
       ...
       if (($afecha>$Fec_Fin_Ejer)or($afecha<$Fec_Ini_Ejer)){$error=1;?><script language="JavaScript">muestra('FECHA ANULACION INVALIDA');</script><?php }
     }
	 if($error==0){$sql="SELECT referencia_pago,tipo_pago FROM PRE008 WHERE (referencia_caus='$nro_orden') And (tipo_causado='$tipo_causado') and (anulado='N')"; $resultado=pg_exec($conn,$sql);  $filas=pg_numrows($resultado);
       if($filas>0){$error=1;?><script language="JavaScript">muestra('Tiene Pagos que Refieren al Causado, No puede ser Anulado');</script><?php }
     }
     if($error==0){$sql="SELECT cod_banco,nro_cheque FROM PAG007 WHERE (Nro_Orden='$nro_orden') And (tipo_causado='$tipo_causado') and (anulado='N')"; $resultado=pg_exec($conn,$sql);  $filas=pg_numrows($resultado);
       if($filas>0) {$error=1;?><script language="JavaScript">muestra('Cheque que Refieren a la Orden, No puede ser Anulado');</script><?php }
     }
     if($error==0){$sql="SELECT referencia_ajuste,tipo_ajuste FROM PRE011 WHERE (referencia_caus='$nro_orden') And (tipo_causado='$tipo_causado') and (anulado='N')";  $resultado=pg_exec($conn,$sql);  $filas=pg_numrows($resultado);
       if($filas>0){$error=1;?><script language="JavaScript">muestra('Tiene Ajustes que Refieren al Causado, No puede ser Anulado');</script><?php }
     }
     ...
     if($error==0){$sql="SELECT ano_fiscal,mes_fiscal,nro_comprobante FROM BAN027 Where (Referencia='$nro_orden') And (Tipo_Mov='O/P') And (Tipo_Operacion<>'A')"; $resultado=pg_exec($conn,$sql);  $filas=pg_numrows($resultado);
       if($filas>0){$error=1;?><script language="JavaScript">muestra('Existe Comprobante de Retencion IVA Asociado a la Orden');</script><?php }
     }
     if($error==0){$sql="SELECT nro_planilla FROM BAN012 Where (Referencia='$nro_orden') And (Tipo_Mov='O/P') And (anulada='N')"; $resultado=pg_exec($conn,$sql);  $filas=pg_numrows($resultado);
       if($filas>0){$reg=pg_fetch_array($resultado);  $nro_planilla=$reg["nro_planilla"]; $error=1;?><script language="JavaScript">muestra('Existe Comprobante de Retencion Asociado a la Orden');</script><?php }
     }
	 if ($error==0){$sql="SELECT nro_orden_ret,nro_cheque_r,tipo_pago_r from PAG004 Where (status_r='I') and (nro_orden_ret='$nro_orden')";  $resultado=pg_exec($conn,$sql);  $filas=pg_numrows($resultado);
       if ($filas>0){ $reg=pg_fetch_array($resultado); echo "Documento que Cancela: ".$reg["tipo_pago_r"]." Referencia:".$reg["nro_cheque_r"],"<br>"; 
	         $error=1;?><script language="JavaScript">muestra('Orden Tiene Retenciones Cancelada');</script><?php }
	 }
	 ...
	 if(($error==0)and($valida_c=="S")){ $tipo_comp='O'.$tipo_causado;
       ...
       if ($filas>0){ $reg=pg_fetch_array($resultado); $mstatusc=$reg["status"]; if($mstatusc=="A"){$error=1;?><script language="JavaScript">muestra('Comprobante Contable de la Orden esta Actualizado, debe cambiar a Diferido');</script><?php } }
	 }
