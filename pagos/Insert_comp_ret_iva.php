...
if (checkData($fecha_emision)=='1'){$error=0;}else{$error=1; ?> <script language="JavaScript">muestra('FECHA DE EMISION NO ES VALIDA');</script><?php }
if (($nro_comprobante=="")or(strlen($nro_comprobante)<8)){$error=1; ?> <script language="JavaScript">muestra('NUMERO DE COMPROBANTE NO VALIDO');</script><?php }
if (($ano_fiscal=="")or(strlen($ano_fiscal)<4)){$error=1; ?> <script language="JavaScript">muestra('AÑO FISCAL NO VALIDO');</script><?php }
if (($mes_fiscal=="")or(strlen($mes_fiscal)<2)){$error=1; ?> <script language="JavaScript">muestra('MES FISCAL NO VALIDO');</script><?php }
$mes_f=substr($fecha_emision,3,2); $ano_f=substr($fecha_emision,6,4);
if ($mes_fiscal<>$mes_f){$error=1; ?> <script language="JavaScript">muestra('MES FECHA NO ES IGUAL AL MES FISCAL');</script><?php }
if ($ano_fiscal<>$ano_f){$error=1; ?> <script language="JavaScript">muestra('MES FECHA NO ES IGUAL AL MES FISCAL');</script><?php }
...
if (pg_last_error($conn)){ ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
	...
	if($error==0){$sql="Select nro_comprobante from BAN027 where ano_fiscal='$ano_fiscal' and  mes_fiscal='$mes_fiscal' and nro_comprobante='$nro_comprobante'"; $resultado=pg_query($sql);  $filas=pg_num_rows($resultado);
      if ($filas>0){$error=1; echo $sql; ?> <script language="JavaScript"> muestra('NUMERO DE COMPROBANTE YA EXISTE');</script><?php }
    }	
	if($error==0){$nmes=substr($fecha_emision,3, 2);
       if ($SIA_Periodo>$nmes){$error=1;?><script language="JavaScript">muestra('FECHA DE COMPROBANTE MENOR A ULTIMO PERIODO CERRADO');</script><?php }
    }
    ...
    if($error==0){$fecha_emision=formato_aaaammdd($fecha_emision);
      ...
      while($registro=pg_fetch_array($res)){ $tipo_retencion=$registro["tipo_retencion"]; $nro_op=$tipo_retencion*1; $tipo_operacion=$registro["tipo_operacion"]; $tipo_documento=$registro["tipo_documento"]; $sfechaf=$registro["fecha_factura"]; $nro_orden=$registro["nro_orden"]; 
          ...
          $resultado=pg_exec($conn,$sSQL); $error=pg_errormessage($conn); $error="ERROR GRABANDO: ".substr($error,0,91);  if (!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php } 
      }
	  if($error==0){?><script language="JavaScript">muestra('GRABO EXITOSAMENTE');</script><?php } 
    }
