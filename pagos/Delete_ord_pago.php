<?php  include ("../class/seguridad.inc"); include ("../class/conects.php");  include ("../class/funciones.php");  include ("../class/configura.inc"); error_reporting(E_ALL);
...
if (pg_last_error($conn)) { ?> <script language="JavaScript"> muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');  </script> <?php }
 ...
 }$posicion=strpos($Mcamino,'S');if(is_numeric($posicion)){$Mcamino=$Mcamino;}else{$error=1;} if($Mcamino{6}=="S"){$error=0;}else{$error=1;} if($error==1){?><script language="JavaScript"> muestra(' NO TIENE DERECHOS PARA EJECUTAR ESTA OPCION'); </script><?php }
 ...
 if($error==0){ $l_cat=0; $sql="Select campo503,campo504,campo526 from SIA005 where campo501='05'";    $resultado=pg_query($sql);
  ...
  if ($filas==0){$error=1;?><script language="JavaScript">muestra('NUMERO DE ORDEN NO EXISTE');</script><?php }
     ...
     if (($error==0)and(substr($tipo_causado,0,1)=="A")){$error=1;?><script language="JavaScript">muestra('ORDEN DE PAGO NO PUEDE SER ELIMINADA');</script><?php }
     if ($error==0){$nmes=substr($fecha,3, 2);if ($SIA_Periodo>$nmes){$error=1;?><script language="JavaScript">muestra('FECHA DE ORDEN MENOR A ULTIMO PERIODO CERRADO');</script><?php } }
     if ($error==0){$sql="SELECT referencia_pago,tipo_pago FROM PRE008 WHERE (referencia_caus='$nro_orden') and (tipo_causado='$tipo_causado')"; $resultado=pg_exec($conn,$sql);  $filas=pg_numrows($resultado);
       if($filas>0){$error=1;?><script language="JavaScript">muestra('Tiene Pagos que Refieren al Causado, No puede ser Eliminado');</script><?php } }
     if ($error==0){$sql="SELECT cod_banco,nro_cheque FROM PAG007 WHERE (Nro_Orden='$nro_orden') and (tipo_causado='$tipo_causado')"; $resultado=pg_exec($conn,$sql);  $filas=pg_numrows($resultado);
       if($filas>0){$error=1;?><script language="JavaScript">muestra('Cheque que Refieren a la Orden, No puede ser Eliminado');</script><?php } }
     if ($error==0){$sql="SELECT referencia_ajuste,tipo_ajuste FROM PRE011 WHERE (referencia_caus='$nro_orden') and (tipo_causado='$tipo_causado')"; $resultado=pg_exec($conn,$sql);  $filas=pg_numrows($resultado);
       if ($filas>0){$error=1;?><script language="JavaScript">muestra('Tiene Ajustes que Refieren al Causado, No puede ser Eliminado');</script><?php } }
	 ...
	 if ($error==0){$sql="SELECT nro_planilla FROM BAN012 Where (referencia='$nro_orden') And (tipo_mov='O/P')";  $resultado=pg_exec($conn,$sql);  $filas=pg_numrows($resultado);
       if ($filas>0){$error=1;?><script language="JavaScript">muestra('Existe Comprobante de Retención Asociado a la Orden');</script><?php } }
     }
	 if ($error==0){$sql="SELECT nro_orden_ret,nro_cheque_r,tipo_pago_r from PAG004 Where (status_r='I') and (nro_orden_ret='$nro_orden')";  $resultado=pg_exec($conn,$sql);  $filas=pg_numrows($resultado);
       if ($filas>0){ $reg=pg_fetch_array($resultado); echo "Documento que Cancela: ".$reg["tipo_pago_r"]." Referencia:".$reg["nro_cheque_r"],"<br>";  $error=1;?><script language="JavaScript">muestra('Orden Tiene Retenciones Cancelada');</script><?php }
	 }
	 ...
	 if($error==0){$sfecha=formato_aaaammdd($fecha); $sqle="SELECT ELIMINA_ORDEN('$nro_orden','$tipo_causado','$sfecha')";
       ...
       if (!$resultado){echo $sqle; $error=1; ?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php }
		    ...
		    ?><script language="JavaScript">muestra('ELIMINO EXITOSAMENTE');</script><?php 
         $resultado=pg_exec($conn,"SELECT INCLUYE_SIA004('01','$usuario_sia','$usuario_sia','$equipo','Elimino','$sfecha','$desc_doc')");
         $error=pg_errormessage($conn);   $error=substr($error, 0, 121); if (!$resultado){?><script language="JavaScript">muestra('<?php echo $error;?>');</script><?php } }
    }
  }
}
