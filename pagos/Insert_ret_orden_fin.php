...
if (pg_last_error($conn)){ ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
  ...
  if ($filas>0){$error=1; ?> <script language="JavaScript"> muestra('RETENCION YA EXISTE EN LA ORDEN');</script><?php }
    ...
    if($error==0){$sSQL="Select * from PAG003 WHERE tipo_retencion='$tipo_retencion'"; $resultado=pg_query($sSQL);  $filas=pg_num_rows($resultado);
      if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('TIPO DE RETENCION NO EXISTE');</script><?php } else {$registro=pg_fetch_array($resultado); $cod_contable=$registro["cod_contable"];}
    }
