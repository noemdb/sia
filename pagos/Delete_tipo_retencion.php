<?php include ("../class/seguridad.inc"); include ("../class/conects.php");  include ("../class/funciones.php");  $tipo_retencion=$_GET["txttipo_retencion"];$equipo = getenv("COMPUTERNAME");$error=0;
...
if (pg_last_error($conn)) {$error=1; ?> <script language="JavaScript"> muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');  </script> <?php }
 ...
 if($error==0){$sSQL="Select * from PAG003 WHERE tipo_retencion='$tipo_retencion'";  $resultado=pg_exec($conn,$sSQL);  $filas=pg_numrows($resultado);
  if ($filas==0){ $error=1; ?>  <script language="JavaScript">  muestra('TIPO DE RETENCION NO EXISTE');  </script> <?php }
     ...
     if ($filas>0){$error=1; ?> <script language="JavaScript"> muestra('TIPO DE RETENCION TIENE ORDENES DE RETENCION, NO PUEDE SER ELIMINADO');</script><?php }
     if($error==0){ $sSQL = "SELECT tipo_ret FROM PAG011 WHERE (tipo_ret='$tipo_retencion')"; $resultado=pg_exec($conn,$sSQL);$filas=pg_numrows($resultado);
       if ($filas>0){$error=1; ?> <script language="JavaScript"> muestra('TIPO DE RETENCION TIENE ESTRUCTURA DE ORDEN, NO PUEDE SER ELIMINADO');</script><?php }
     }
