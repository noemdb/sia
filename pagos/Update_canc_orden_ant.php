...
if(pg_last_error($conn)) { ?> <script language="JavaScript"> muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');  </script> <?php }
  ...
  if($filas==0){$error=1;?><script language="JavaScript">muestra('NUMERO DE ORDEN NO EXISTE');</script><?php }
     ...
     */
     ...
     if($error==0){$sfecha=$fecha_causado;if(checkData($fecha_c)=='1'){$error=0;$afecha=formato_aaaammdd($fecha_c);} else{$error=1; ?> <script language="JavaScript">muestra('FECHA ANULACION NO ES VALIDA');</script><?php } }
     if($error==0){if($afecha<$sfecha){$error=1; ?> <script language="JavaScript">muestra('CANCELACION NO PUEDE SER MENOR A FECHA DE LA ORDEN');</script><?php } }
     if($error==0){$nmes=substr($fecha_c,3, 2); if($SIA_Periodo>$nmes){$error=1;?><script language="JavaScript">muestra('FECHA CANCELACION MENOR A ULTIMO PERIODO CERRADO');</script><?php } }
     if($error==0){
       $fecha_d=formato_ddmmaaaa($Fec_Ini_Ejer);  $fecha_h=formato_ddmmaaaa($Fec_Fin_Ejer);
       if (($afecha>$Fec_Fin_Ejer)or($afecha<$Fec_Ini_Ejer)){$error=1;?><script language="JavaScript">muestra('FECHA CANCELACION INVALIDA');</script><?php }
     }
     if($error==0){ $SQL="Update pag022 set status='$status',cod_banco='$cod_banco',nro_cheque='$nro_cheque',fecha_cheque='$fecha_c' where nro_orden='$nro_orden' and tipo_causado='$tipo_causado'"; 
       ...
       if(!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php } else{?><script language="JavaScript">muestra('<?php echo $mmens?>');</script><?php }
