...
if ($error==0){ $sfecha=formato_aaaammdd($fecha);  $rfecha=$sfecha; $campo_str2=""; $gen_comprobante="N"; 
  ...
  if($error==0){
    $resultado=pg_exec($conn,"SELECT ELIMINA_CON008('$codigo_mov')");  $mvalor=pg_errormessage($conn); $mvalor=substr($mvalor,0,91); if (!$resultado){ ?> <script language="JavaScript">  muestra('<?php  echo $mvalor; ?>'); </script> <?php }
    ...
    if ($filas>0){ $pasivo_comp="NO";
        while(($registro=pg_fetch_array($res))){ $tipo_retencion=$registro["tipo_retencion"];  $total=$total+$registro["monto_retencion"]; $monto_asiento=$registro["monto_retencion"];  $codigo_cuenta=$registro["cod_contable_ret"];
           ...
           if ($filas>0){$reg=pg_fetch_array($resultado);  $monto_asiento=cambia_coma_numero($monto_asiento);
              ...
              $err=pg_errormessage($conn);  $err="ERROR MODIFICANDO: ".substr($err,0,91); if (!$resultado){$error=1; ?><script language="JavaScript">muestra('<?php  echo $err; ?>');</script><?php } }
            else{
              $resultado=pg_exec($conn,"SELECT ACTUALIZA_PAG030 ('1','$codigo_mov','$codigo_cuenta','D','$monto_asiento')");
              $err=pg_errormessage($conn);  $err="ERROR GRABANDO: ".substr($err,0,91); if (!$resultado){$error=1; ?><script language="JavaScript">muestra('<?php  echo $err; ?>');</script><?php } }
       }
   }
  }
    ...
    if ($filas>0){ $pasivo_comp="NO";
        while(($registro=pg_fetch_array($res))){ $tipo_retencion=$registro["tipo_retencion"];  $total=$total+$registro["monto_retencion"]; $monto_asiento=$registro["monto_retencion"];  $codigo_cuenta=$registro["cod_contable_ret"];
		  ...
		  if ($filas>0){ $reg=pg_fetch_array($resultado);
             ...
             $mvalor=pg_errormessage($conn);  $mvalor="ERROR GRABANDO: ".substr($mvalor,0,91); if (!$resultado){?><script language="JavaScript">muestra('<?php  echo $mvalor; ?>');</script><?php } }
  ...
  if($total>0){$Status_1="S";} else {$error=1;?><script language="JavaScript">muestra('NO TIENE RETENCIONES SELECCIONADAS');</script><?php }
