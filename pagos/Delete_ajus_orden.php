...
if (pg_last_error($conn)) { ?> <script language="JavaScript"> muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');  </script> <?php }
  ...
  if ($filas==0){$error=1;?><script language="JavaScript">muestra('DOCUMENTO DE AJUSTE A COMPROMISO NO EXISTE');</script><?php } else{ $registro=pg_fetch_array($resultado); $tipo_ajuste_c=$registro["tipo_ajuste"]; }
  ...
  if ($filas==0){$error=1;?><script language="JavaScript">muestra('REFERENCIA AJUSTE DE ORDEN NO EXISTE');</script><?php }
     ...
     if (($error==0)and(substr($tipo_ajuste,0,1)=="A")){$error=1;?><script language="JavaScript">muestra('AJUSTE A ORDEN NO PUEDE SER ELIMINADA');</script><?php }
     if ($error==0){ $nmes=substr($fecha,3, 2);
        if ($SIA_Periodo>$nmes){$error=1;?><script language="JavaScript">muestra('FECHA AJUSTE DE ORDEN MENOR A ULTIMO PERIODO CERRADO');</script><?php }
     }
