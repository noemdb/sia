<?php include ("../class/conect.php");  include ("../class/funciones.php"); $formato_presup="XX-XX-XX-XXX-XX-XX-XX"; $error=0;
...
if ($monto_retencion==0) {$error=1; ?> <script language="JavaScript"> muestra('MONTO DE LA RETENCION INVALIDA');</script><?php }
...
if($error==0){ 
  ...
  if ($filas>0){$error=1; ?> <script language="JavaScript"> muestra('RETENCION YA EXISTE EN LA ORDEN');</script><?php }
   else{ $cod_contable="";
    ...
    $resultado=pg_query($sSQL);  $filas=pg_num_rows($resultado);
    if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO PRESUPUESTARIO NO EXISTE EN LA ORDEN');</script><?php }
    if($error==0){$sSQL="Select * from PRE099 WHERE ced_rif='$ced_rif'"; $resultado=pg_query($sSQL);   $filas=pg_num_rows($resultado);
      if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CEDULA/RIF DE BENEFICIARIO NO EXISTE');</script><?php }else{$registro=pg_fetch_array($resultado); $nombre=$registro["nombre"];}
    }
    if($error==0){ $sSQL="Select * from PAG003 WHERE tipo_retencion='$tipo_retencion'"; $resultado=pg_query($sSQL); $filas=pg_num_rows($resultado);
      if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('TIPO DE RETENCION NO EXISTE');</script><?php }
...
pg_close($conn); if (($error==0)or($error==1)){?><script language="JavaScript"> document.location='<?php  echo $url; ?>'; </script> <?php }
