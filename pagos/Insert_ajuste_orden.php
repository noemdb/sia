<?php include ("../class/conect.php");  include ("../class/funciones.php");  include ("../class/configura.inc"); error_reporting(E_ALL); $formato_presup="XX-XX-XX-XXX-XX-XX-XX";
...
if ($error==0){$sfecha=formato_aaaammdd($fecha);  $rfecha=$sfecha; $campo_str2="";
  ...
  if($error==0){ $sSQL="SELECT ced_rif,fecha FROM PAG001 where nro_orden='$nro_orden'";   $resultado=pg_exec($conn,$sSQL); $filas=pg_numrows($resultado);
    if ($filas==0){$error=1;?><script language="JavaScript">muestra('ORDEN DE PAGO A AJUSTAR NO EXISTE');</script><?php }
     else{ $reg=pg_fetch_array($resultado); $ced_rif=$reg["ced_rif"]; $fecha_orden=$reg["fecha"];
       if (($sfecha<$fecha_orden)){$error=1;?><script language="JavaScript">muestra('FECHA AJUSTE MENOR A FECHA ORDEN');</script><?php }
     }
  } $total=0;
  if($error==0){ $sql="Select * from CODIGOS_PRE026 where codigo_mov='$codigo_mov' order by cod_presup"; $res=pg_query($sql);
    while(($registro=pg_fetch_array($res))and($error==0)){
      ...
      if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('NO EXISTE EN EL CAUSADO EL CODIGO PRESUPUESTARIO:<?php  echo $registro["cod_presup"]; ?> FUENTE:<?php  echo $registro["fuente_financ"]; ?> REF.CREDITO:<?php  echo $registro["ref_imput_presu"]; ?>');</script><?php }
         ...
         if($monto_c>$compromiso){$error=1; ?> <script language="JavaScript"> muestra('MONTO AJUSTE MAYOR AL MONTO DEL CÓDIGO DEL CAUSADO POR AJUSTAR');</script><?php }
      }
    }
  ...
  if($error==0){$sql="SELECT * FROM CON008  where codigo_mov='$codigo_mov' order by debito_credito desc,cod_cuenta";    $res=pg_query($sql);  $t_debe=0; $t_haber=0;$balance=0;
    ...
    if ($balance==0){$error=0;}else{$error=1; echo $balance,"<br>"; ?> <script language="JavaScript"> muestra('COMPROBANTE CONTABLE NO CUADRA');</script><?php }
  ...
  if (($rmonto <> $t_debe) And ($monto > 0.001)) {$error=1;   ?> <script language="JavaScript"> muestra('COMPROBANTE CONTABLE NO CUADRA CON AJUSTE');</script><?php }
  if (($error==0)And($nro_aut="N")){ $sql="Select nro_orden from PAG019 WHERE referencia_aju_ord='$referencia_aju'";      $resultado=pg_exec($conn,$sql);  $filas=pg_numrows($resultado);
     if ($filas>0){$error=1;?><script language="JavaScript">muestra('NÚMERO DE AJUSTE A ORDEN YA EXISTE');</script><?php }
  }
  if($error==0){ $sfecha=formato_aaaammdd($fecha); $total=cambia_coma_numero($total);
     ...
     if (!$resultado){echo $sSQL; ?><script language="JavaScript"> muestra('<?php  echo $error; ?>');</script><?php  $error=1;} else{ $error=0;?><script language="JavaScript">  muestra('INCLUYO EXITOSAMENTE');</script><?php 
