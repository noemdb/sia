...
if (pg_last_error($conn)) { ?>  <script language="JavaScript"> muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS'); </script> <?php }
  ...
  if ($filas==0){ $error=1; ?>  <script language="JavaScript">muestra('C0DIGO DE ESTRUCTURA NO EXISTE'); </script><?php }
     ...
     if($error==0){ $sSQL="Select * from PAG008 WHERE tipo_orden='$cod_tipo_ord'";  $resultado=pg_exec($conn,$sSQL); $filas=pg_numrows($resultado);
       if ($filas==0){$error=1;?><script language="JavaScript">muestra('TIPO DE ORDEN NO EXISTE');</script><?php }
     }
     if($error==0){$sql="SELECT * FROM CODIGOS_PRE026 where codigo_mov='$codigo_mov' order by cod_presup"; $res=pg_query($sql);
       $total=0; while($registro=pg_fetch_array($res)){ $total=$total+1; }  if($total==0){$error=1;?><script language="JavaScript">muestra('NO HAY CODIGOS EN LA ESTRUCTURA');</script><?php }
     }
     if($error==0){ $dfecha=formato_aaaammdd($fecha_desde_est);$hfecha=formato_aaaammdd($fecha_hasta_est);
       ...
       if (!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php }  else{$error=0;?><script language="JavaScript">muestra('MODIFICO EXITOSAMENTE');</script><?php 
             ...
             $resultado=pg_exec($conn,"SELECT BORRAR_PAG028('$codigo_mov')");$error=pg_errormessage($conn); $error=substr($error, 0, 61);  if (!$resultado){ ?> <script language="JavaScript">  muestra('<?php  echo $error; ?>'); </script> <?php } }
    }
  }
}
pg_close($conn); error_reporting(E_ALL ^ E_WARNING);
if ($error==0){?><script language="JavaScript">document.location ='Act_estructura_orden.php?Gcod_estructura=<?php echo $cod_estructura?>';</script> <?php }
