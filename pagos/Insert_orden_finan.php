<?php include ("../class/conect.php");  include ("../class/funciones.php"); include ("../class/configura.inc");  include ("G_Comp_iva.php"); include ("G_Comp_islr.php");
...
if (strlen($nro_orden)==8){$error=$error;} else{$error=1; ?> <script language="JavaScript">muestra('NUMERO DE ORDEN VALIDA');</script><?php }
if ($error==0){ $sfecha=formato_aaaammdd($fecha);  $rfecha=$sfecha; $campo_str2="";
  ...
  if($error==0){ $fecha_d=formato_ddmmaaaa($Fec_Ini_Ejer);  $fecha_h=formato_ddmmaaaa($Fec_Fin_Ejer);
    if (($sfecha>$Fec_Fin_Ejer)or($sfecha<$Fec_Ini_Ejer)or($sfecha<$rfecha)){$error=1;?><script language="JavaScript">muestra('FECHA DE ORDEN INVALIDA');</script><?php }
  }
  if($error==0){$nmes=substr($fecha,3, 2);
    if ($SIA_Periodo>$nmes){$error=1;?><script language="JavaScript">muestra('FECHA DE ORDEN MENOR A ULTIMO PERIODO CERRADO');</script><?php }
  }
  $monto_am_ant=0;
  $sSQL="Select * from PAG036 WHERE codigo_mov='$codigo_mov'"; $resultado=pg_exec($conn,$sSQL);  $filas=pg_numrows($resultado);
  if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('INFORMACION DE LA ORDEN NO VALIDA');</script> <?php }
  ...
  if($error==0){ $nro_cod_ret=0;
    ...
    while(($registro=pg_fetch_array($res))){ $nro_cod_ret=$nro_cod_ret+1; $tipo_retencion=$registro["tipo_retencion"]; $totalr=$totalr+$registro["monto_retencion"];
      ...
      if ($descripcion_ret==""){ $error=1;?><script language="JavaScript">muestra('TIPO DE RETENCION NO INVALIDA: <?php  echo $tipo_retencion; ?>');</script> <?php }
    }
    if($totalr<0){$error=1;?><script language="JavaScript">muestra('MONTO TOTAL DE RETENCION INVALIDO');</script><?php }
    if($totalr>=$total) {$error=1;?><script language="JavaScript">muestra('TOTAL NETO DE LA ORDEN INVALIDO');</script><?php }
  ...
  $rmonto = $total;
  if (($rmonto <> $t_debe) And ($monto > 0.001)) {$error=1;   ?> <script language="JavaScript"> muestra('COMPROBANTE CONTABLE NO CUADRA CON CAUSADO');</script><?php }
...
if ($error==0){?><script language="JavaScript">document.location ='Act_orden_pago.php?Gcriterio=C<?php  echo $nro_orden.$tipo_causado; ?>';</script> <?php }
