...
if (pg_last_error($conn)){ ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
  ...
  if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('FACTURA NO EXISTE EN LA ORDEN');</script><?php }
   else{ $tmonto_s=$monto_sin_iva; if($tmonto_s<0){ $tmonto_s=$tmonto_s*-1;}   $tmontof=$monto_factura; if($tmontof<0){ $tmontof=$tmontof*-1;}
    if($tmonto_s>$tmontof){$error=1; ?> <script language="JavaScript"> muestra('MONTO CON IVA NO PUEDE SER MENOR A MONTO SIN IVA');</script><?php }
    ...
    if (checkData($fecha_factura)=='1'){$error=0;}else{$error=1; ?> <script language="JavaScript">muestra('FECHA NO ES VALIDA');</script><?php }
