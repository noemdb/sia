<?php include ("../class/conect.php");  include ("../class/funciones.php"); include ("G_Comp_Orden.php"); include ("../presupuesto/Ver_dispon.php");  include ("G_Comp_iva.php"); include ("G_Comp_islr.php");
...
if (checkData($fecha_desde)=='1'){$error=$error;} else{$error=1; ?> <script language="JavaScript">muestra('FECHA DESDE NO ES VALIDA');</script><?php }
if (checkData($fecha_hasta)=='1'){$error=$error;} else{$error=1; ?> <script language="JavaScript">muestra('FECHA HASTA NO ES VALIDA');</script><?php }
if (checkData($fecha_vencim)=='1'){$error=$error;} else{$error=1; ?> <script language="JavaScript">muestra('FECHA VENCIMIENTO NO ES VALIDA');</script><?php }
if (strlen($nro_orden)==8){$error=$error;} else{$error=1; ?> <script language="JavaScript">muestra('NUMERO DE CAUSADO INVALIDA');</script><?php }
if ($error==0){  $sfecha=formato_aaaammdd($fecha);  $rfecha=$sfecha; $campo_str2=""; $gen_comp_ret="N";
  ...
  if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('INFORMACION DE LA CAUSADO NO VALIDA');</script> <?php }
  ...
  if($error==0){  $sSQL="Select ref_compromiso from pre003 WHERE tipo_causado='$tipo_causado'";  $resultado=pg_exec($conn,$sSQL);  $filas=pg_numrows($resultado);
    if ($filas==0){$error=1;?><script language="JavaScript">muestra('DOCUMENTO DE CAUSADO NO EXISTE');</script><?php }
      ...
      else{ $registro=pg_fetch_array($resultado);  if(($caus_directo=="SI")and($registro["ref_compromiso"]=="SI")){echo $caus_directo; $error=1;?><script language="JavaScript">muestra('DOCUMENTO DE CAUSADO NO VALIDO');</script><?php }
        if(($caus_directo=="NO")and($registro["ref_compromiso"]=="NO")){echo $caus_directo; $error=1;?><script language="JavaScript">muestra('DOCUMENTO DE CAUSADO NO VALIDO');</script><?php }
      }}
  }
  if($error==0){if($caus_directo=="SI"){$referencia_comp=$nro_orden;$tipo_compromiso="0000";} }
  if (($error==0)And($nro_aut=="N")){  $sSQL="Select referencia_caus from pre007 WHERE referencia_caus='$nro_orden' and tipo_causado='$tipo_causado'";  $resultado=pg_exec($conn,$sSQL);  $filas=pg_numrows($resultado);    if ($filas>0){$error=1;?><script language="JavaScript">muestra('REFERENCIA DE CAUSADO YA EXISTE');</script><?php } }
  if($error==0){ $fecha_d=formato_ddmmaaaa($Fec_Ini_Ejer);  $fecha_h=formato_ddmmaaaa($Fec_Fin_Ejer);    if (($sfecha>$Fec_Fin_Ejer)or($sfecha<$Fec_Ini_Ejer)or($sfecha<$rfecha)){$error=1;?><script language="JavaScript">muestra('FECHA DE CAUSADO INVALIDA');</script><?php } }
  if($error==0){$nmes=substr($fecha,3, 2); if ($SIA_Periodo>$nmes){$error=1;?><script language="JavaScript">muestra('FECHA DE CAUSADO MENOR A ULTIMO PERIODO CERRADO');</script><?php } }
  if($error==0){  $sSQL="SELECT ced_rif FROM pre099 WHERE ced_rif='$ced_rif'";  $resultado=pg_exec($conn,$sSQL); $filas=pg_numrows($resultado);    if ($filas==0){$error=1;?><script language="JavaScript">muestra('CEDULA/RIF BENEFICIARIO NO EXISTE');</script><?php } }
  if(($error==0)and($pago_ces=="S")){ $sSQL="SELECT ced_rif FROM pre099 WHERE ced_rif='$ced_rif_ces'";  $resultado=pg_exec($conn,$sSQL); $filas=pg_numrows($resultado);    if ($filas==0){$error=1;?><script language="JavaScript">muestra('CEDULA/RIF CESIONARIO NO EXISTE');</script><?php } }
  if($error==0){ $StrSQL="select cod_contable_t,cod_banco_t from pag008 where tipo_orden='$tipo_orden'";    $res=pg_exec($conn,$StrSQL);$filas=pg_num_rows($res);
    if($filas>0){$reg=pg_fetch_array($res); $cod_contable=$reg["cod_contable_t"]; $cod_banco_t=$reg["cod_banco_t"];
     ...
     else {$error=1; ?> <script language="JavaScript"> muestra('TIPO DE ORDEN NO VALIDA');</script> <?php }
  }
    ...
    while($registro=pg_fetch_array($res)){ $nro_cod_pre=$nro_cod_pre+1;
      ...
      if($caus_directo=="SI"){ $fmonto=formato_monto($monto_c);
        if (verifica_disponibilidad($conn,$registro["cod_presup"],$registro["fuente_financ"],$fecha,$monto_c)==0){$error=0;}
         else{$error=1;?><script language="JavaScript">muestra('ERRRO EN EL CODIGO PRESUPUESTARIO:<?php  echo $registro["cod_presup"]; ?> FUENTE:<?php  echo $registro["fuente_financ"]; ?> REF.CREDITO:<?php  echo $registro["ref_imput_presu"]; ?>, MONTO: <?php  echo $fmonto; ?>');</script><?php }
        if(($error==0)and($tipo_imput_presu=="C")){
          ...
          if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO PRESUPUESTARIO:<?php  echo $registro["cod_presup"]; ?> FUENTE:<?php  echo $registro["fuente_financ"]; ?> REF.CREDITO:<?php  echo $registro["ref_imput_presu"]; ?> \n NO EXISTE EN LA EJECUCION DEL CREDITO ADICIONAL');</script><?php }
           else{$regc=pg_fetch_array($resc);
             if($regc["disponible"]<$monto_credito) {$error=1; $dispon=$regc["disponible"]; $dispon=formato_monto($dispon); ?> <script language="JavaScript"> muestra('CODIGO PRESUPUESTARIO:<?php  echo $reg["cod_presup"]; ?> FUENTE:<?php  echo $reg["fuente_financ"]; ?> REF.CREDITO:<?php  echo $reg["referencia_adicion"]; ?> \ncon Monto Mayor que Disponibilidad del Credito Adicional, Disponibilidad: <?php  echo $dispon; ?> ');</script><?php }
          }
        }
        ...
        if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('NO EXISTE EN EL COMPROMISO EL CODIGO PRESUPUESTARIO:<?php  echo $registro["cod_presup"]; ?> FUENTE:<?php  echo $registro["fuente_financ"]; ?> REF.CREDITO:<?php  echo $registro["ref_imput_presu"]; ?>');</script><?php }
           ...
           if(($monto_c>$compromiso)and($diferencia>0.001)){$error=1; echo $error_c,"<br>"; ?> <script language="JavaScript"> muestra('MONTO A CAUSAR MAYOR AL MONTO DEL CODIGO POR COMPROMETER');</script><?php }
         }
		if($error==0){
		   ...
		   if ($filas>0){echo "Referencia Compromiso: ".$referencia_comp; $error=1; ?> <script language="JavaScript"> muestra('EXISTEN CAUSADOS ANULADOS ASOCIADOS AL COMPROMISO CON FECHAS POSTERIOES ');</script><?php }
		}		 
      }
    } $total=$totalc;
    if($total<=0){$error=1;?><script language="JavaScript">muestra('MONTO TOTAL DEL CAUSADO INVALIDO');</script><?php }
  }
  ...
  if($error==0){ $nro_cod_ret=0; 
    ...
    while(($registro=pg_fetch_array($res))){ $nro_cod_ret=$nro_cod_ret+1; $tipo_retencion=$registro["tipo_retencion"];  $ced_rif_r=$registro["ced_rif_r"]; 
      ...
      if ($descripcion_ret==""){ $error=1;?><script language="JavaScript">muestra('TIPO DE RETENCION NO VALIDA: <?php  echo $tipo_retencion; ?>'); </script> <?php }
        ...
        if ($gen_pre_ret=="S"){  if($caus_directo=="SI"){ $referencia_comp="00000000"; }
			...
			if ($filasr==0){$error=1; echo $tipo_retencion." ".$cod_presup." ".$fuente_financ." ".$referencia_comp,"<br>"; echo $sSQL,"<br>"; ?> <script language="JavaScript"> muestra('CODIGO PRESUPUESTARIO DE LA RETENCION NO EXISTE EN EL CAUSADO');</script><?php }
			  ...
			  if (($gen_pre_ret=="S") and ($gen_ord_ret=="S")){ $monto_c=$monto_c-$monto_asiento;
			   if ($monto_c>=0) { if ($monto_credito>$monto_c) {$monto_credito=$monto_c;} $slqg="SELECT MOD_MONTO_PRE026('$codigo_mov','$cod_presup','$fuente_financ','$referencia_comp','$tipo_compromiso','$ref_imput_presu',$monto_c,$monto_credito)";
				  $resg=pg_exec($conn,$slqg);   $err=pg_errormessage($conn);  $err="ERROR MODIFICANDO: ".substr($err, 0, 61); if (!$resg){ $error=1; ?><script language="JavaScript">muestra('<?php  echo $err; ?>');</script><?php }
			   }else{$error=1;?><script language="JavaScript">muestra('CODIGO PRESUPUESTARIO DE RETENCION: <?php  echo $tipo_retencion; ?> INVALIDO' </script><?php }
			  }
			}
		}
      }
	  if ($ced_rif_r==""){ $error=1;?><script language="JavaScript">muestra('CEDULA/RIF, DEL TIPO DE RETENCION NO VALIDA: <?php  echo $tipo_retencion; ?>'); </script> <?php }
    }
    if($totalr<0){$error=1;?><script language="JavaScript">muestra('MONTO TOTAL DE RETENCION INVALIDO');</script><?php }
    if($totalr>=$total) {$error=1;?><script language="JavaScript">muestra('TOTAL NETO DEL CAUSADO INVALIDO');</script><?php }
  ...
  if($error==0){
    ...
    if ($filas>0){ $pasivo_comp="NO";
       ...
       if ($filas>0){$reg=pg_fetch_array($resultado); $monto_asiento=$monto_asiento+$reg["monto_pasivo"]; $monto_asiento=cambia_coma_numero($monto_asiento);
          ...
          $err=pg_errormessage($conn);  $err="ERROR MODIFICANDO: ".substr($err, 0, 61); if (!$resultado){$error=1; ?><script language="JavaScript">muestra('<?php  echo $err; ?>');</script><?php } }
        else{$resultado=pg_exec($conn,"SELECT ACTUALIZA_PAG030 ('1','$codigo_mov','$codigo_cuenta','D','$monto_asiento')");
          $err=pg_errormessage($conn);  $err="ERROR GRABANDO: ".substr($err, 0, 61); if (!$resultado){$error=1; ?><script language="JavaScript">muestra('<?php  echo $err; ?>');</script><?php } }
       }
       if($totaloc>0){$Status_2="S"; If ($totalr>0){$error=1;?><script language="JavaScript">muestra('NO PUEDE TENER RETENCIONES Y CANCELACION DE RETENCIONES');</script><?php } }
    }
  ...
  if($error==0){$sql="SELECT * FROM CON008 where codigo_mov='$codigo_mov' order by debito_credito desc,cod_cuenta";
    ...
    if (($t_debe==0)){$error=1; ?> <script language="JavaScript"> muestra('MONTO DEL COMPROBANTE CONTABLE NO VALIDO');</script><?php }
  ...
  if (($rmonto <> $t_debe) And ($balance>0.001)) {$error=1; $dif=0;
      ...
      if($error==1){echo $t_debe.' '.$rmonto.' '.formato_monto($balance)." ".$t_dpas." ".$pasivo_comp." ".formato_monto($dif),"<br>";  ?> <script language="JavaScript"> muestra('COMPROBANTE CONTABLE NO CUADRA CON CAUSADO');</script><?php } }
  if (($error==0)And($nro_aut=="N")){
     ...
     if ($filas>0){$error=1;?><script language="JavaScript">muestra('NUMERO DE ORDEN YA EXISTE');</script><?php }
        ...
        if ($filas>0){ $reg=pg_fetch_array($resultado); $ult_fecha=$reg["ult_fecha"]; $sfecha=formato_aaaammdd($fecha);
	      if($sfecha<$ult_fecha){$error=1; $fecha_up=formato_ddmmaaaa($ult_fecha); echo "Ultima Fecha:".$fecha_up; ?><script language="JavaScript">muestra('FECHA DE ORDEN MENOR A LA ANTERIOR');</script><?php }
	    }
	 }
  ...
  if($error==0){ 
    ...
    if ($filas>0){ $sql="Select min(fecha) as ult_fecha from PAG001 where nro_orden>'$nro_orden'  and tipo_causado<='9999'";  $resultado=pg_exec($conn,$sql);  $filas=pg_numrows($resultado);
	    ...
	    if($sfecha>$ult_fecha){$error=1; $fecha_up=formato_ddmmaaaa($ult_fecha); echo "Fecha proxima orden:".$fecha_up; ?><script language="JavaScript">muestra('FECHA DE ORDEN MAYOR A LA PROXIMA');</script><?php }
	}	
  }
  if($error==0){ 
  ...
  if($error==0){ if(($ord_per=="S") and ($per_permanente<=0)){ $error=1;?><script language="JavaScript">muestra('NUMERO PERIODO DE ORDEN PERMANENTE INVALIDO');</script><?php } }
  if($error==0){  $sfecha=formato_aaaammdd($fecha); $sfechad=formato_aaaammdd($fecha_desde); $sfechah=formato_aaaammdd($fecha_hasta); $sfechav=formato_aaaammdd($fecha_vencim);   $total=cambia_coma_numero($total);  $totalr=cambia_coma_numero($totalr); $totalop=cambia_coma_numero($totalop);
   ...
   //echo $statusc.'<br>'.$sSQL;exit;
     ...
     if (!$resultado){ $error=1; ?><script language="JavaScript"> muestra('<?php  echo $merror; ?>');</script><?php  $error=1;}  else{ $error=0;?><script language="JavaScript">  muestra('INCLUYO EXITOSAMENTE');</script><?php        
	   /*
       ...
       $resultado=pg_exec($conn,"SELECT ELIMINA_CON022('$codigo_mov')"); $error=pg_errormessage($conn); $error=substr($error, 0, 61); if (!$resultado){ ?> <script language="JavaScript">  muestra('<?php  echo $error; ?>'); </script> <?php }
...
//exit;
/*  */
if ($error==0){?><script language="JavaScript">document.location ='Act_orden_pago.php?Gcriterio=C<?php  echo $nro_orden.$tipo_causado; ?>';</script> <?php } else {?>  <script language="JavaScript">history.back();</script> <?php } 
