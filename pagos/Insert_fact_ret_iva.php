...
if (pg_last_error($conn)){ ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
    ...
    if($error==0){
      ...
      if ($filas>0){$error=1; echo $sSQL; ?> <script language="JavaScript"> muestra('NÚMERO DE OPERACIÓN YA EXISTE');</script><?php }
    }
    ...
    if($error==0){$sfechaf=formato_aaaammdd($fecha_factura);
      ...
      echo $sSQL; $resultado=pg_exec($conn,$sSQL); $error=pg_errormessage($conn); $error="ERROR GRABANDO: ".substr($error,0,91);  if (!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php }
    }
