...
if (pg_last_error($conn)) { ?> <script language="JavaScript">   muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');  </script> <?php }
  ...
  if ($filas==0){ $error=1; $url="Act_estructura_orden.php?Gcod_estructura==".$cod_estructura; ?><script language="JavaScript">muestra('CODIGO DE ESTRUCTURA NO EXISTE');</script> <?php }
   else{$registro=pg_fetch_array($resultado);     $bloqueada=$registro["bloqueada"];
     if($bloqueada=="S"){$url="Mod_estructura.php?Gcod_estructura=".$cod_estructura."&bloqueada=S&codigo_mov=".$codigo_mov;?><script language="JavaScript">muestra('ESTRUCTURA ESTA BLOQUEADA');</script> <?php }
