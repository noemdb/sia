...
if (pg_last_error($conn)) { ?>  <script language="JavaScript"> muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS'); </script> <?php }
  ...
  if ($filas>0){ $error=1; ?>  <script language="JavaScript">muestra('CÓDIGO DE ESTRUCTURA YA EXISTE'); </script><?php }
     ...
     if($error==0){  $sql="SELECT * FROM CODIGOS_PRE026 where codigo_mov='$codigo_mov' order by cod_presup"; $res=pg_query($sql);$total=0;
       while($registro=pg_fetch_array($res)){ $total=$total+1;}if($total==0){$error=1;?><script language="JavaScript">muestra('NO HAY CÓDIGOS EN LA ESTRUCTURA');</script><?php } }
