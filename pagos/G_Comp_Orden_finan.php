...
function g_comprobante_ord($mconn,$codigo_mov,$G_Comp_Ret,$G_Ord_Ret){ $mvalor=0; $error=0;
  ...
  if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('TIPO DE ORDEN NO VALIDA');</script> <?php }
    ...
    if($error==0){$resultado=pg_exec($mconn,"SELECT ELIMINA_CON008('$codigo_mov')");
      $error=pg_errormessage($mconn); $error=substr($error, 0, 61); if (!$resultado){ ?> <script language="JavaScript">  muestra('<?php  echo $error; ?>'); </script> <?php }
      ...
      while(($registro=pg_fetch_array($res))){
        ...
        if($registro["cargable"]=="C"){$monto_t=$monto_t-$monto_asiento;
          ...
          if ($filas>0){ $reg=pg_fetch_array($resultado);
             ...
             $error=pg_errormessage($mconn);  $error="ERROR GRABANDO: ".substr($error, 0, 61); if (!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php } } }
