...
if (($mes_libro=="")or(strlen($mes_libro)<2)){$error=1; ?> <script language="JavaScript">muestra('MES LIBRO NO VALIDO');</script><?php }
...
if (pg_last_error($conn)){ ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?php }
    ...
    if($error==0){ $sql="Select mes_libro from PAG032 where mes_libro='$mes_libro'";  $resultado=pg_query($sql);  $filas=pg_num_rows($resultado);
      if ($filas==0){$error=1; echo $sql; ?> <script language="JavaScript"> muestra('MES LIBRO DE COMPAS NO EXISTE');</script><?php }
       else{$sSQL="SELECT BORRAR_PAG032('$mes_libro')"; $resultado=pg_exec($conn,$sSQL); $error=pg_errormessage($conn); $error="ERROR ELIMINANDO: ".substr($error, 0, 61);  if (!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php } }
    }
    if($error==0){ $sSQL="SELECT GRABAR_PAG032('$codigo_mov','$minf_usuario')"; ECHO $sSQL;  $resultado=pg_exec($conn,$sSQL); $error=pg_errormessage($conn); $error="ERROR GRABANDO: ".substr($error, 0, 61);  if (!$resultado){?><script language="JavaScript">muestra('<?php  echo $error; ?>');</script><?php } else{?><script language="JavaScript">muestra('GRABO EXITOSAMENTE');</script><?php } }
